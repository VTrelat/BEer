  ...
  | @chpred α α' c_α =>
    ...
    · intro v hv
      ...
      · exact List.mem_union_iff.mpr
          (Or.inl (hvS! ▸ AList.mem_keys.mpr S!_in))
      · exact absurd hvx hv_ne_x
      · rcases List.mem_union_iff.mp (hT_fv hvT) with hΛ | hX
        · exact List.mem_union_iff.mpr (Or.inl (AList.mem_keys.mpr
            (AList.mem_of_subset S!_Λ_sub
...
/-- The equal option-function intersection is a direct lambda: it allocates
one local binder, erases it from the global context, and emits no helper. -/
theorem castInter_option_state
    (S T : SMT.Term) (γ δ : SMTType) {Λ : SMT.TypeContext} {n : ℕ}
    {used : List SMT.𝒱} {X : List SMT.𝒱} :
    ⦃ fun (⟨E, Λ'⟩ : EncoderState) ↦
        ⌜Λ' = Λ ∧ E.freshvarsc = n ∧ AList.keys Λ ⊆ E.usedVars ∧
          E.usedVars = used ∧ SMT.fv S ⊆ AList.keys Λ ∪ X ∧
          SMT.fv T ⊆ AList.keys Λ ∪ X⌝ ⦄
    castInter (S, SMTType.fun γ (SMTType.option δ))
      (T, SMTType.fun γ (SMTType.option δ))
    ⦃ ⇓? (⟨t', _σ⟩ : SMT.Term × SMTType)
        (⟨E', Γ'⟩ : EncoderState) => ⌜
      n ≤ E'.freshvarsc ∧
      Λ ⊆ Γ' ∧
      used ⊆ E'.usedVars ∧
  ...
  have hrun :
      castInter (S, SMTType.fun γ (SMTType.option δ))
          (T, SMTType.fun γ (SMTType.option δ)) = do
        let x ← SMT.freshVar γ "inter!"
        SMT.eraseFromContext x
        return (.lambda [x] [γ]
          (.ite (.eq (.app S (.var x)) (.app T (.var x)))
            (.app S (.var x)) (noneCast δ)),
          SMTType.fun γ (SMTType.option δ)) := by
    simp [castInter]
  rw [hrun]
  mintro pre ∀St₀
  mpure pre
  obtain ⟨rfl, rfl, keys_sub, rfl, hS_fv, hT_fv⟩ := pre
  mspec SMT.freshVar_spec
  next x =>
    mrename_i pre
    mintro ∀St₁
    mpure pre
    obtain ⟨St₁_types_eq, x_fresh, St₁_fvc, St₁_used_eq,
      x_not_used⟩ := pre
    mspec SMT.eraseFromContext_spec
    mrename_i pre
    mintro ∀StE
    mpure pre
    obtain ⟨StE_types_eq, StE_fvc, StE_used_eq⟩ := pre
    have StE_types_base : StE.types = St₀.types := by
    ...
    · intro v hv
      ...
      exact List.mem_cons_of_mem x hv
    · rw [StE_types_base, StE_used_eq, St₁_used_eq]
      exact List.subset_cons_of_subset x keys_sub
    · intro v hv
      simp only [noneCast, SMT.fv, List.mem_removeAll_iff, List.mem_append,
        List.mem_cons, List.not_mem_nil, or_false] at hv
      obtain ⟨hv_body, hv_ne_x⟩ := hv
...
set_option maxHeartbeats 4000000 in
/-- `castEq` records every loosened helper together with its Boolean
specification. Both the result and each specification mention only the input
terms and names declared by that delta. -/
theorem castEq_decl
    (A B : SMT.Term) (σA σB : SMTType) {Λ : SMT.TypeContext} {n : ℕ}
  ...
  · -- σA ⊑ σB : loosen A
    ...
    mspec Std.Do.Spec.pure
    mpure_intro
    refine ⟨helperSpecChunk A! σB A!_spec, ?_, ?_, ?_⟩
    · rw [hd_decl, A!_decl]
      simp [helperSpecChunk, List.concat_eq_append, List.append_assoc]
    · intro b hb
      simp only [specBodies_helperSpecChunk, List.mem_singleton] at hb
      subst b
      intro v hv
      simp only [declVars_helperSpecChunk, List.mem_union_iff,
        List.mem_singleton]
      rcases List.mem_union_iff.mp (A!_fv_sub hv) with hA | hA!
  ...
  · -- σB ⊑ σA : loosen B
    ...
    mspec Std.Do.Spec.pure
    mpure_intro
    refine ⟨helperSpecChunk B! σA B!_spec, ?_, ?_, ?_⟩
    · rw [hd_decl, B!_decl]
      simp [helperSpecChunk, List.concat_eq_append, List.append_assoc]
    · intro b hb
      simp only [specBodies_helperSpecChunk, List.mem_singleton] at hb
      ...
      simp only [declVars_helperSpecChunk, List.mem_union_iff,
        List.mem_singleton]
      rcases List.mem_union_iff.mp (B!_fv_sub hv) with hB | hB!
      · exact Or.inl (Or.inr hB)
      · exact Or.inr (List.mem_singleton.mp hB!)
    · intro v hv
      simp only [SMT.fv, List.mem_append, List.mem_cons, List.not_mem_nil, or_false,
        declVars_helperSpecChunk, List.mem_union_iff, List.mem_singleton] at hv ⊢
  ...
  | @«fun» α β α' β' hβ c_α c_β =>
    ...
    · rename_i σ _
      ...
      case post.success =>
        ...
        · intro v hv
          simp only [noneCast, SMT.fv, List.mem_removeAll_iff, List.mem_append,
            List.mem_cons, List.not_mem_nil, or_false, declVars,
            List.filterMap_cons, List.filterMap_nil, List.mem_union_iff] at hv ⊢
          obtain ⟨hv_body, hv_ne_p⟩ := hv
          rcases hv_body with
            (((hvS! | hvp) | (hvT | hvp')) | (hvS!' | hvp''))
          · exact Or.inr hvS!
...
/-- The direct equal option-function branch preserves the declaration stream;
its only fresh name is local to the returned lambda. -/
theorem castInter_option_decl
    (S T : SMT.Term) (γ δ : SMTType) {Λ : SMT.TypeContext} {n : ℕ}
    {used : List SMT.𝒱} {decl : SMT.Chunk} :
    ⦃ fun (⟨E, Λ'⟩ : EncoderState) ↦
        ⌜Λ' = Λ ∧ E.freshvarsc = n ∧ AList.keys Λ ⊆ E.usedVars ∧
  ...
  mintro pre ∀St₀
  mpure pre
  obtain ⟨rfl, rfl, keys_sub, rfl, rfl⟩ := pre
  mspec SMT.freshVar_decls
  next x =>
    mrename_i pref
    mintro ∀St₁
    ...
    · intro v hv
      simp only [noneCast, SMT.fv, List.mem_removeAll_iff, List.mem_append,
        List.mem_cons, List.not_mem_nil, or_false, declVars,
        List.filterMap_nil, List.mem_union_iff] at hv ⊢
      obtain ⟨hv_body, hv_ne_x⟩ := hv
      rcases hv_body with
        (((hvS | hvx) | (hvT | hvx')) | (hvS' | hvx''))
      · exact Or.inl hvS
...
theorem collectOption_subst_fv (D P : SMT.Term) (z : SMT.𝒱)
    ...
    ∀ v ∈ SMT.fv
      (SMT.substList vs
        ((toDestPair vs.dropLast (.var z)).concat (.the (.app D (.var z)))) P),
      v ≠ z → v ∈ SMT.fv P ∨ v ∈ SMT.fv D := by
  intro v hv hvz
  rcases SMT_mem_fv_substList hv with hP | ⟨t, ht, hvt⟩
  · exact .inl hP
  · rw [List.concat_eq_append] at ht
    rcases List.mem_append.mp ht with hdest | hthe
    · exact False.elim (hvz (SMT_fv_toDestPair_subset hdest hvt))
    · rw [List.mem_singleton] at hthe
      subst t
      simp only [SMT.fv, List.mem_append, List.mem_singleton] at hvt
      rcases hvt with hD | hz
      · exact .inr hD
...
theorem collectOption_lambda_fv (D P : SMT.Term) (z : SMT.𝒱)
    ...
    SMT.fv ((λˢ [z]) [alpha]
      (.ite (.and (.eq (.app D (.var z)) (.some (.the (.app D (.var z)))))
        (SMT.substList vs
          ((toDestPair vs.dropLast (.var z)).concat (.the (.app D (.var z)))) P))
        (.some (.the (.app D (.var z)))) (none$ beta))) ⊆
      SMT.fv D ∪ SMT.fv P := by
  intro v hv
  simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append,
    List.mem_singleton] at hv
  obtain ⟨hv, hvz⟩ := hv
  simp [noneCast, SMT.fv, hvz] at hv
  rw [List.mem_union_iff]
  rcases hv with (hD | hsub) | hD
  · exact .inl hD
  · have hsub' := collectOption_subst_fv D P z vs v
      (by simpa [List.concat_eq_append] using hsub) hvz
    rcases hsub' with hP | hD
    · exact .inr hP
    · exact .inl hD
  · exact .inl hD
...
theorem encodeTerm_combined
  ...
  | pow S ih =>
    ...
    · rename_i α γ heq
      ...
      mpure pre
      obtain ⟨_Sg_le, Sg_Λ_sub, _Sg_fresh, _Sg_not_used,
        Sg_used_sub, Sg_keys_sub, Sg_preserves, Sg_fv_sub,
        Sg_decl⟩ := pre
      mspec SMT.declareConst_addSpec_spec
      mrename_i pre
      mintro ∀StD
      mpure pre
      obtain ⟨Sg_helper_decl, _Sg_asserts, StD_fvc_eq,
        StD_used_eq, StD_types_eq⟩ := pre
      set ctx := StD.types with hctx
      mspec Std.Do.Spec.get_StateT
      mspec (Std.Do.Triple.and (SMT.freshVar (.pair α γ))
        (SMT.freshVar_spec (Γ := ctx) (τ := .pair α γ)
          (n := StD.env.freshvarsc) (used := StD.env.usedVars))
        (SMT.freshVar_decls (τ := .pair α γ)
          (decl := StD.env.declarations)))
      case post.success p =>
        mrename_i pre
        mintro ∀St₁
        mpure pre
        obtain ⟨⟨St₁_types_eq, p_fresh, St₁_fvc_eq,
          St₁_used_eq, p_not_used⟩, St₁_decl⟩ := pre
        mspec (Std.Do.Triple.and
          (SMT.freshVar (.fun (.pair α γ) .bool))
          (SMT.freshVar_spec
            (Γ := ctx.insert p (.pair α γ))
            (τ := .fun (.pair α γ) .bool)
            (n := St₁.env.freshvarsc) (used := St₁.env.usedVars))
          (SMT.freshVar_decls
            (τ := .fun (.pair α γ) .bool)
        ...
        case post.success R =>
          ...
          mintro ∀St₂
          mpure pre
          obtain ⟨⟨St₂_types_eq, R_fresh, St₂_fvc_eq,
            St₂_used_eq, R_not_used⟩, St₂_decl⟩ := pre
          simp [modify]
          mspec Std.Do.Spec.modifyGet_StateT
          mpure_intro
          let Dg := helperSpecChunk Sg
            (.fun (.pair α γ) .bool) Sg_spec
          have hvars_S_sub' :
              B.Term.vars S ⊆ B.Term.vars (𝒫ᴮ S) := hvars_S_sub
          have S_types_sub_ctx : σ_S.types ⊆ ctx := by
            intro v hv
            rw [StD_types_eq]
            apply Sg_Λ_sub
            rw [AList.entries_insert_of_notMem _Sg_fresh]
            exact List.mem_cons_of_mem _ hv
          have base_types_sub_ctx : σ.types ⊆ ctx :=
            AList.subset_trans S_Λ_sub S_types_sub_ctx
          have Sg_mem_ctx : Sg ∈ ctx := by
            rw [StD_types_eq]
            exact AList.mem_of_subset Sg_Λ_sub
              (AList.mem_insert _ |>.mpr (Or.inl rfl))
          refine ⟨⟨?_, base_types_sub_ctx, ?_, ?_, ?_, ?_⟩,
            ΔS ++ Dg, ?_, ?_, ?_⟩
          · intro v hv
            rw [St₂_used_eq, St₁_used_eq]
            exact List.mem_cons_of_mem _
              (List.mem_cons_of_mem _ (by
                simpa [StD_used_eq] using Sg_used_sub (S_used_sub hv)))
          · intro v hv
            rw [St₂_used_eq, St₁_used_eq]
            exact List.mem_cons_of_mem _
              (List.mem_cons_of_mem _ (by
                have hvL : v ∈ StL.types := by
                  simpa [ctx, StD_types_eq] using hv
                simpa [StD_used_eq] using Sg_keys_sub hvL))
          · intro v hv
            rw [B.fv] at hv
            rw [St₂_used_eq, St₁_used_eq]
            exact List.mem_cons_of_mem _
              (List.mem_cons_of_mem _ (by
                simpa [StD_used_eq] using Sg_used_sub (S_cov v hv)))
          · intro v hv
            have hvSg : v = Sg := by
              simp only [SMT.fv, List.mem_removeAll_iff,
                List.mem_append, List.mem_cons, List.not_mem_nil,
                or_false] at hv
              obtain ⟨⟨hv1, hnep⟩, hneR⟩ := hv
              rcases hv1 with (hR | hp) | hSg | hp'
              · exact (hneR hR).elim
              · exact (hnep hp).elim
              · exact hSg
              · exact (hnep hp').elim
            subst v
            exact List.mem_union_iff.mpr
              (Or.inl (AList.mem_keys.mpr Sg_mem_ctx))
          · intro v hv hΛ hvars
            have hnotL := Sg_preserves v (S_used_sub hv)
              (S_preserves v hv hΛ (fun h =>
                hvars (hvars_S_sub' h)))
            simpa [ctx, StD_types_eq] using hnotL
          · rw [St₂_decl, St₁_decl, Sg_helper_decl, Sg_decl,
              S_decl_eq]
            simp [Dg, helperSpecChunk, List.concat_eq_append,
              List.append_assoc]
          · intro b name hb
            rw [List.mem_append] at hb
            rcases hb with hb | hb
            · exact specBody_mono hvars_S_sub'
                (declVars_append .. ▸ List.subset_append_left ..)
                (S_specb b (mem_specBodies_define_fun.mpr ⟨name, hb⟩))
            · have hb' : b ∈ specBodies Dg :=
                mem_specBodies_define_fun.mpr ⟨name, hb⟩
              simp only [Dg, specBodies_helperSpecChunk,
                List.mem_singleton] at hb'
              subst b
              intro v hv
              rw [declVars_append]
              rcases List.mem_union_iff.mp (Sg_fv_sub hv) with hvS | hvSg
              · rcases List.mem_union_iff.mp (S_enc_fv_sub hvS) with
                  hvar | hdecl
                · exact List.mem_union_iff.mpr
                    (Or.inl (hvars_S_sub' hvar))
                · exact List.mem_union_iff.mpr
                    (Or.inr (List.mem_append_left _ hdecl))
              · have hv_eq : v = Sg := List.mem_singleton.mp hvSg
                subst v
                exact List.mem_union_iff.mpr (Or.inr
                  (List.mem_append_right _ (by
                    simp [Dg, declVars_helperSpecChunk])))
          · intro v hv
            have hvSg : v = Sg := by
              simp only [SMT.fv, List.mem_removeAll_iff,
                List.mem_append, List.mem_cons, List.not_mem_nil,
            ...
            rw [declVars_append]
            exact List.mem_union_iff.mpr (Or.inr
              (by simp [Dg, declVars_helperSpecChunk]))
    · mvcgen
  | cprod A C A_ih C_ih =>
    mstart
    mintro pre ∀σ
  ...
  | eq x y x_ih y_ih =>
    ...
    case post.success =>
      ...
      mpure pre
      obtain ⟨⟨h_le, h_Λ_sub, h_used_sub, h_keys_sub, h_fv_sub, h_preserves⟩,
        Dce, ce_decl_eq, ce_specb, ce_fv_decl_sub⟩ := pre
      mpure_intro
    ...
    · rename_i _ Senc1 _ Senc2 _ _ _ gamma delta heqA heqC
      ...
      case post.success x =>
        ...
        · intro v hv
          ...
          obtain ⟨hv_body, hv_ne_x⟩ := hv
          rw [StE_types_eq, St₁_types_eq]
          rcases hv_body with (((hvA | hvx) | (hvC | hvx')) | (hvA' | hvx''))
          · rcases List.mem_union_iff.mp (A_fv_sub hvA) with hk | hb
            · exact List.mem_union_iff.mpr (.inl (AList.mem_keys.mpr (AList.mem_erase.mpr
                ⟨hv_ne_x, AList.mem_insert _ |>.mpr
                  (Or.inr (AList.mem_of_subset C_Λ_sub (AList.mem_keys.mp hk)))⟩)))
    ...
    · -- function-`D` arm
      ...
      · -- arity matches
        ...
        · -- `encodeTerm P` returned a boolean
          ...
          subst St₃'
          mspec (Std.Do.Triple.and (SMT.freshVar α')
            (SMT.freshVar_spec (Γ := St₃.types) (τ := α')
              (n := St₃.env.freshvarsc) (used := St₃.env.usedVars))
            (SMT.freshVar_decls (τ := α') (decl := St₃.env.declarations)))
          case post.success z =>
          mrename_i pre4
          mintro ∀St₄
          mpure pre4
          ...
          have St₃_used_chain : St₃.env.usedVars ⊆ St₅.env.usedVars := by
            ...
            exact fun v hv => List.mem_cons_of_mem _ hv
          ...
          · intro v hv
            ...
            rcases List.mem_union_iff.mp
                (collectOption_lambda_fv D_enc P_enc z vs α' β' hv) with hD | hP
            · rcases List.mem_union_iff.mp (D_state_fv_sub hD) with hk | hb
              · exact List.mem_union_iff.mpr (.inl (AList.mem_keys.mp
                  (AList.mem_of_subset St₁_sub_St₃ (AList.mem_keys.mpr hk))))
              · exact List.mem_union_iff.mpr (.inr (hvars_D_sub hb))
            · rcases List.mem_union_iff.mp (P_state_fv_sub hP) with hk | hb
              · exact List.mem_union_iff.mpr (.inl hk)
          ...
          · intro v hv
            ...
            rcases List.mem_union_iff.mp
                (collectOption_lambda_fv D_enc P_enc z vs α' β' hv) with hD | hP
            · rcases List.mem_union_iff.mp (D_fv_sub hD) with h | h
              · exact List.mem_union_iff.mpr (.inl (hvars_D_sub h))
              · exact List.mem_union_iff.mpr (.inr (List.mem_append_left _ h))
            · rcases List.mem_union_iff.mp (P_fv_sub hP) with h | h
              · exact List.mem_union_iff.mpr (.inl (hvars_P_sub h))
              · exact List.mem_union_iff.mpr (.inr (List.mem_append_right _ h))
        · exact wp_bind_throw _ _ _ _
