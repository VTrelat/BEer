...
theorem inductive_sep {S} (P : ZFSet → Prop) (ind : inductive_set S) (h₀ : P ∅) (h₁ : ∀ n ∈ S, P n → P (insert n n)) : inductive_set <| S.sep P := by
...
theorem inductive_imp_transitive {E : ZFSet} : inductive_set E → inductive_set (E.sep transitive) := by
...
-/
theorem omega_inductive : inductive_set ω := ⟨omega_zero, λ _ => omega_succ⟩
