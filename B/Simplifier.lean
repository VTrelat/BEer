...
def subst (x : 𝒱) (e t : Term) : Term :=
  ...
  | .dom R => .dom (subst x e R)
  | .ran R => .ran (subst x e R)
  | .domRestrict n F R => .domRestrict n (subst x e F) (subst x e R)
  | .ranRestrict n R F => .ranRestrict n (subst x e R) (subst x e F)
  | .overload Q R => .overload (subst x e Q) (subst x e R)
  | .compose R S => .compose (subst x e R) (subst x e S)
  | .fold s f => .fold s (subst x e f)
  | .iterate R n => .iterate (subst x e R) (subst x e n)
  | .conc R ss => .conc R (subst x e ss)
  | .collect vs D P =>
...
def simplifier : Term → Term
  ...
  | Term.dom R => .dom (simplifier R)
  | Term.ran R => .ran (simplifier R)
  | Term.domRestrict n F R => .domRestrict n (simplifier F) (simplifier R)
  | Term.ranRestrict n R F => .ranRestrict n (simplifier R) (simplifier F)
  | Term.overload Q R => .overload (simplifier Q) (simplifier R)
  | Term.compose R S => .compose (simplifier R) (simplifier S)
  | Term.fold s f => .fold s (simplifier f)
  | Term.iterate R n => .iterate (simplifier R) (simplifier n)
  | Term.conc R ss => .conc R (simplifier ss)
