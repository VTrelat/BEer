...
theorem encodeTerm_state
  ...
  induction t generalizing E n used Λ α with
  | int i =>
    mstart
    mintro pre ∀St
    mpure pre
  ...
  | maplet x y x_ih y_ih =>
    ...
    have Λ_inv_x : ∀ v ∈ x.vars, v ∈ σ.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inl h)
    mspec x_ih (E := E) (Λ := σ.types) typ_x vars_used_x Λ_inv_x hx_bv_nodup
    clear x_ih
    rename_i out_x
    ...
    mspec y_ih (E := E) (Λ := σ_x.types) (used := σ_x.env.usedVars)
      typ_y (fun v hv => x_used_sub (vars_used_y v hv))
      (fun v hv hΛ => by
  ...
  | not x ih =>
    ...
    have Λ_inv_x : ∀ v ∈ x.vars, v ∈ σ.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simpa [B.Term.vars, B.fv, B.bv] using hv)
    mspec ih (E := E) (Λ := σ.types) typ_x vars_used_x Λ_inv_x hx_bv_nodup
    rename_i out_x
    obtain ⟨x_enc, σx⟩ := out_x
    mrename_i prex
    mintro ∀σ_x
  ...
  | pow S ih =>
    ...
    have Λ_inv_S : ∀ v ∈ S.vars, v ∈ σ.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simpa [B.Term.vars, B.fv, B.bv] using hv)
    mspec ih (E := E) (Λ := σ.types) typ_S vars_used_S Λ_inv_S hS_bv_nodup
    rename_i out_S
  ...
  | cprod A C A_ih C_ih =>
    ...
    have Λ_inv_A : ∀ v ∈ A.vars, v ∈ σ.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inl h)
    mspec A_ih (E := E) (Λ := σ.types) typ_A vars_used_A Λ_inv_A hA_bv_nodup
    rename_i out_A
  ...
  | mem x S x_ih S_ih =>
    ...
    mspec S_ih (E := E) (Λ := σ_x.types) (used := σ_x.env.usedVars)
      typ_S (fun v hv => x_used_sub (vars_used_S v hv))
      (fun v hv hΛ => by
        have hv_par : v ∈ (x ∈ᴮ S).vars := by
          simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
          rcases hv with h | h <;> [left; right] <;> exact .inr h
        by_cases hv_St : v ∈ σ.types
        · exact Λ_inv v hv_par hv_St
  ...
  | union A C A_ih C_ih =>
    ...
    mspec C_ih (E := E) (Λ := σ_A.types) (used := σ_A.env.usedVars)
      typ_C (fun v hv => A_used_sub (vars_used_C v hv))
      (fun v hv hΛ => by
        have hv_par : v ∈ (A ∪ᴮ C).vars := by
          simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
          rcases hv with h | h <;> [left; right] <;> exact .inr h
    ...
    · -- σA ⊑ σC : castUnionAux A_enc C_enc
      ...
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqA
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqC
      mspec (castUnionAux_state _ _ _ (X := []))
      case pre =>
        mpure_intro
        refine ⟨trivial, trivial, C_keys_sub, rfl, ?_, ?_⟩
        · intro v hv
          exact List.mem_union_iff.mpr (Or.inl (AList.mem_keys.mp (AList.mem_of_subset C_Λ_sub
            (AList.mem_keys.mpr (A_fv_sub hv)))))
        · intro v hv
          exact List.mem_union_iff.mpr (Or.inl (C_fv_sub hv))
      case post.success =>
        mrename_i hpost
        mintro ∀St'
        mpure hpost
        ...
        · intro v hv
          exact (List.mem_union_iff.mp (h_fv_sub hv)).resolve_right List.not_mem_nil
        · intro v hv hΛ hvars
          have hvA : v ∉ B.Term.vars A := fun h => hvars (by
            simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at h ⊢
            rcases h with h | h <;> [left; right] <;> exact .inl h)
  ...
  | inter A C A_ih C_ih =>
    ...
    · -- σA ⊑ σC : castInterAux A_enc C_enc
      ...
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqA
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqC
      mspec (castInterAux_state _ _ _ (X := []))
      case pre =>
        mpure_intro
        refine ⟨trivial, trivial, C_keys_sub, rfl, ?_, ?_⟩
        · intro v hv
  ...
  | app f x f_ih x_ih =>
    ...
    have Λ_inv_f : ∀ v ∈ f.vars, v ∈ σ.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inl h)
    mspec f_ih (E := E) (Λ := σ.types) typ_f vars_used_f Λ_inv_f hf_bv_nodup
    clear f_ih
    rename_i out_f
    ...
    mintro ∀σ_f
    mpure pre
    obtain ⟨f_used_sub, f_Λ_sub, f_keys_sub, f_cov, f_fv_sub, f_preserves⟩ := pre
    mspec x_ih (E := E) (Λ := σ_f.types) (used := σ_f.env.usedVars)
      typ_x (fun v hv => f_used_sub (vars_used_x v hv))
      (fun v hv hΛ => by
        have hv_par : v ∈ (B.Term.app f x).vars := by
          simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
          rcases hv with h | h <;> [left; right] <;> exact .inr h
        by_cases hv_St : v ∈ σ.types
        · exact Λ_inv v hv_par hv_St
    ...
    mpure pre
    obtain ⟨x_used_sub, x_Λ_sub, x_keys_sub, x_cov, x_fv_sub, x_preserves⟩ := pre
    mspec (castApp_state _ _ _ _ (X := []))
    case pre =>
      mpure_intro
      refine ⟨trivial, trivial, x_keys_sub, rfl, ?_, ?_⟩
      · intro v hv
        exact List.mem_union_iff.mpr (Or.inl (AList.mem_keys.mp (AList.mem_of_subset x_Λ_sub
          (AList.mem_keys.mpr (f_fv_sub hv)))))
      · intro v hv
        exact List.mem_union_iff.mpr (Or.inl (x_fv_sub hv))
    case post.success =>
      mrename_i hpost
      mintro ∀σ'
      mpure hpost
      ...
      · intro v hv
        rcases List.mem_union_iff.mp (h_fv_sub hv) with h | h
        · exact h
        · simp at h
      · intro v hv hΛ hvars
        have hvf : v ∉ B.Term.vars f := fun h => hvars (by
          simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at h ⊢
          rcases h with h | h <;> [left; right] <;> exact .inl h)
        have hvx : v ∉ B.Term.vars x := fun h => hvars (by
          simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at h ⊢
  ...
  | collect vs D P D_ih P_ih =>
    ...
    · -- function-`D` arm: `τD = .fun α' (.option β')`
      rename_i α' β' heq
      split
      · -- arity matches: real continuation
        rename_i harity
        set αs' := α'.fromProdl (vs.length - 2) with αs'_def
        mspec Std.Do.Spec.pure
        mspec encodeTerm_state.modifyTypes_forIn_spec
          (pairs := vs.zip (αs'.concat β'))
        mrename_i pre
        mintro ∀St₂
        mpure pre
        ...
        have vs_disj_St₁ : ∀ v ∈ vs, v ∉ St₁.types := by
          ...
          intro hv_St₀
          have hv_coll : v ∈ (Term.collect vs D P).vars := by
        ...
        have Λ_inv_P : ∀ v ∈ P.vars, v ∈ St₂.types → v ∈ E'.context := by
          ...
          · have v_in_St₁ : v ∈ St₁.types := by
            ...
            · have v_coll : v ∈ (Term.collect vs D P).vars := by
        ...
        mspec P_ih (E := E') (Λ := St₂.types) (α := .bool) typP vars_used_P_St₂ Λ_inv_P
          hP_bv_nodup
        clear P_ih
        rename_i out_P
        ...
        split
        · -- `encodeTerm P` returned a boolean: real continuation
          rename_i heqP
          obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. ▸ heqP
          mspec SMT.freshVarList_spec (τs := αs') (Γ := St₃.types)
            (used := St₃.env.usedVars)
          rename_i xs
          mrename_i pre
          mintro ∀St₄
          ...
          case pre =>
            mpure_intro
            refine ⟨trivial, trivial, St₄_keys_sub, rfl, ?_, ?_⟩
            · -- fv D_enc ⊆ keys St₄.types
              intro v hv
              exact List.mem_union_iff.mpr (Or.inl (AList.mem_keys.mp
                (AList.mem_of_subset St₁_sub_St₄ (AList.mem_keys.mpr (D_fv_sub hv)))))
            · -- fv ((xs.map var).toPairl) ⊆ keys St₄.types
              intro v hv
          ...
          case post.success =>
            ...
            obtain ⟨ca_le, ca_Λ_sub, ca_used_sub, ca_keys_sub, ca_fv_sub₀,
              ca_preserves⟩ := pre
            have ca_fv_sub : ∀ ⦃v⦄, v ∈ _ → v ∈ AList.keys St₅.types := fun v hv =>
              (List.mem_union_iff.mp (ca_fv_sub₀ hv)).resolve_right List.not_mem_nil
            mspec Std.Do.Spec.pure
            mpure_intro
            have St₀_sub_St₅ : St₀.types ⊆ St₅.types :=
            ...
            -- `xs` are fresh from everything before `St₃`, hence outside `used`.
            ...
            · -- 3. keys St₅.types ⊆ St₅.usedVars
              exact ca_keys_sub
            · -- 4. CoversUsedVars St₅.usedVars (collect vs D P)
              intro v hv
              rw [B.fv, List.mem_append] at hv
              rcases hv with hv | hv
            ...
            · -- 5. fv t' ⊆ keys St₅.types
              ...
              rcases hv_body with (hv_a | hv_b) | hv_c
              · -- v ∈ fv (substList vs (...) P_enc)
                rcases SMT_mem_fv_substList hv_a with hvP | ⟨t, ht, hvt⟩
                · exact St₃_keys_sub_St₅ (P_fv_sub hvP)
                · rw [List.concat_eq_append] at ht
                  ...
                  · rw [List.mem_singleton] at ht_eq
                    subst ht_eq; exact ca_fv_sub hvt
              · -- v ∈ fv r✝.1.some = fv r✝.1
                exact ca_fv_sub hv_b
              · -- v ∈ fv (none$β') = []
                simp only [noneCast, SMT.fv, List.not_mem_nil] at hv_c
            · -- 6. ∀ v ∈ used, v ∉ St₀.types → v ∉ (collect).vars → v ∉ St₅.types
        ...
        · -- `encodeTerm P` did not return a boolean: throw arm
          exact wp_bind_throw _ _ _ _
      · -- arity mismatch: throw arm
        mvcgen
    · -- set-`D` arm: `τD = .fun τ' .bool`
      rename_i τ' heq
      ...
      · -- `encodeTerm P` did not return a boolean: throw arm
        exact wp_bind_throw _ _ _ _
    · -- throw arm: `τD` neither a set nor a partial function
      mvcgen
...
appends a chunk `Δ` to `declarations`, and the encoded term `t'` has free
...
theorem encodeTerm_decl
  ...
  | maplet x y x_ih y_ih =>
    ...
    have Λ_inv_x : ∀ v ∈ x.vars, v ∈ St.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inl h)
    mspec (Std.Do.Triple.and (encodeTerm x E)
      (encodeTerm_state E (Λ := St.types) (n := St.env.freshvarsc) (used := used)
        typ_x vars_used_x Λ_inv_x hx_bv_nodup)
      (x_ih E (Λ := St.types) (n := St.env.freshvarsc) (used := used) (decl := decl)
        typ_x vars_used_x Λ_inv_x hx_bv_nodup))
    mrename_i pre
    mintro ∀σ_x
    mpure pre
    obtain ⟨⟨x_used_sub, x_Λ_sub, x_keys_sub, x_cov, x_fv_sub, x_preserves⟩,
      Δx, x_decl_eq, x_enc_fv_sub⟩ := pre
    rename_i out_x
    obtain ⟨x_enc, σx⟩ := out_x
    have Λ_inv_y : ∀ v ∈ y.vars, v ∈ σ_x.types → v ∈ E.context := fun v hv hΛ => by
      have hv_par : v ∈ (x ↦ᴮ y).vars := by
      ...
      · have hv_vars_x : v ∈ B.Term.vars x := by
        ...
        · rcases B.Term.mem_vars_iff.mp hv with hy_fv | hy_bv
          · exact absurd (_root_.B.Typing.typed_by_fv typ_y hy_fv)
              (_root_.B.Typing.bv_notMem_context typ_x v h)
          · exact absurd rfl (hxy_bv_disj v h v hy_bv)
    mspec (Std.Do.Triple.and (encodeTerm y E)
      (encodeTerm_state E (Λ := σ_x.types) (n := σ_x.env.freshvarsc)
        (used := σ_x.env.usedVars) typ_y
        (fun v hv => x_used_sub (vars_used_y v hv)) Λ_inv_y hy_bv_nodup)
      (y_ih E (Λ := σ_x.types) (n := σ_x.env.freshvarsc)
        (used := σ_x.env.usedVars) (decl := decl ++ Δx) typ_y
        (fun v hv => x_used_sub (vars_used_y v hv)) Λ_inv_y hy_bv_nodup))
    mrename_i pre
    mintro ∀σ_y
    mpure pre
    obtain ⟨⟨y_used_sub, y_Λ_sub, y_keys_sub, y_cov, y_fv_sub, y_preserves⟩,
      Δy, y_decl_eq, y_enc_fv_sub⟩ := pre
    rename_i out_y
    obtain ⟨y_enc, σy⟩ := out_y
    mpure_intro
    refine ⟨Δx ++ Δy, by rw [y_decl_eq, List.append_assoc], ?_⟩
    · intro v hv
      simp only [SMT.fv, List.mem_append] at hv
  ...
  | not x ih =>
    ...
    mintro ∀σ_x
    mpure pre
    obtain ⟨Δx, x_decl_eq, x_enc_fv_sub⟩ := pre
    rename_i out_x
    obtain ⟨x_enc, σx⟩ := out_x
    split
    · rename_i heq
  ...
  | pow S ih =>
    ...
    mintro ∀σ_S
    mpure pre
    obtain ⟨ΔS, S_decl_eq, S_enc_fv_sub⟩ := pre
    rename_i out_S
    obtain ⟨S_enc, σS⟩ := out_S
    split
    · rename_i heq
      ...
      mspec Std.Do.Spec.modifyGet_StateT
      mpure_intro
      refine ⟨ΔS, by rw [pre2, pre, S_decl_eq], ?_⟩
      · intro v hv
        simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append,
          List.mem_cons, List.not_mem_nil, or_false] at hv
        obtain ⟨⟨hv1, hv_ne_x⟩, hv_ne_ℰ⟩ := hv
    ...
    · rename_i heq
      ...
      mspec Std.Do.Spec.modifyGet_StateT
      mpure_intro
      refine ⟨ΔS, by rw [pre3, pre2, pre, S_decl_eq], ?_⟩
      · intro v hv
        simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append,
          List.mem_cons, List.not_mem_nil, or_false] at hv
        obtain ⟨⟨hv1, hv_ne_xy⟩, hv_ne_f⟩ := hv
  ...
  | cprod A C A_ih C_ih =>
    ...
    have vars_used_C : ∀ v ∈ C.vars, v ∈ used := fun v hv => vars_used v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inr h)
    have Λ_inv_A : ∀ v ∈ A.vars, v ∈ St.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inl h)
    mspec (Std.Do.Triple.and (encodeTerm A E)
      (encodeTerm_state E (Λ := St.types) (n := St.env.freshvarsc) (used := used)
        typ_A vars_used_A Λ_inv_A hA_bv_nodup)
      (A_ih E (Λ := St.types) (n := St.env.freshvarsc) (used := used) (decl := decl)
        typ_A vars_used_A Λ_inv_A hA_bv_nodup))
    mrename_i pre
    mintro ∀σ_A
    mpure pre
    obtain ⟨⟨A_used_sub, A_Λ_sub, A_keys_sub, A_cov, A_fv_sub, A_preserves⟩,
      ΔA, A_decl_eq, A_enc_fv_sub⟩ := pre
    rename_i out_A
    obtain ⟨A_enc, σA⟩ := out_A
    have Λ_inv_C : ∀ v ∈ C.vars, v ∈ σ_A.types → v ∈ E.context := fun v hv hΛ => by
      have hv_par : v ∈ (A ⨯ᴮ C).vars := by
    ...
    · rename_i heq
      ...
      subst hσe
      subst hAe
      mspec (Std.Do.Triple.and (encodeTerm C E)
        (encodeTerm_state E (Λ := σ_A.types) (n := σ_A.env.freshvarsc)
          (used := σ_A.env.usedVars) typ_C
          (fun v hv => A_used_sub (vars_used_C v hv)) Λ_inv_C hC_bv_nodup)
        (C_ih E (Λ := σ_A.types) (n := σ_A.env.freshvarsc)
          (used := σ_A.env.usedVars) (decl := decl ++ ΔA) typ_C
          (fun v hv => A_used_sub (vars_used_C v hv)) Λ_inv_C hC_bv_nodup))
      mrename_i pre
      mintro ∀σ_C
      mpure pre
      obtain ⟨⟨C_used_sub, C_Λ_sub, C_keys_sub, C_cov, C_fv_sub, C_preserves⟩,
        ΔC, C_decl_eq, C_enc_fv_sub⟩ := pre
      rename_i out_C
      obtain ⟨C_enc, σC⟩ := out_C
      split
      · rename_i heq2
        ...
        mspec Std.Do.Spec.pure
        mpure_intro
        refine ⟨ΔA ++ ΔC, by rw [pre3, pre2, pre, C_decl_eq, List.append_assoc], ?_⟩
        · intro v hv
          simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append,
            List.mem_cons, List.not_mem_nil, or_false] at hv
          obtain ⟨⟨hv1, hv_ne_ab⟩, hv_ne_p⟩ := hv
      ...
      · have hv_vars_x : v ∈ B.Term.vars x := by
        ...
        · rcases B.Term.mem_vars_iff.mp hv with hS_fv | hS_bv
          · exact absurd (_root_.B.Typing.typed_by_fv typ_S hS_fv)
              (_root_.B.Typing.bv_notMem_context typ_x v h)
          · exact absurd rfl (hxS_bv_disj v h v hS_bv)
    mspec (Std.Do.Triple.and (encodeTerm S E)
      (encodeTerm_state E (Λ := σ_x.types) (n := σ_x.env.freshvarsc)
        (used := σ_x.env.usedVars) typ_S
        (fun v hv => x_used_sub (vars_used_S v hv)) Λ_inv_S hS_bv_nodup)
      (S_ih E (Λ := σ_x.types) (n := σ_x.env.freshvarsc)
        (used := σ_x.env.usedVars) (decl := decl ++ Δx) typ_S
        (fun v hv => x_used_sub (vars_used_S v hv)) Λ_inv_S hS_bv_nodup))
    mrename_i pre
    mintro ∀σ_S
    mpure pre
    obtain ⟨⟨S_used_sub, S_Λ_sub, S_keys_sub, S_cov, S_fv_sub, S_preserves⟩,
      ΔS, S_decl_eq, S_enc_fv_sub⟩ := pre
    rename_i out_S
    obtain ⟨S_enc, σS⟩ := out_S
  ...
  | union A C A_ih C_ih =>
    ...
    rename_i out_C
    obtain ⟨C_enc, σC⟩ := out_C
    have hAfv : SMT.fv A_enc ⊆ AList.keys σ_C.types := fun v hv =>
      AList.mem_keys.mp (AList.mem_of_subset C_Λ_sub (AList.mem_keys.mpr (A_fv_sub hv)))
    unfold castUnion
    split <;> split <;> split <;> split
    · rename_i _ Senc1 _ Senc2 _ _ _ gamma heqA heqC
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqA
      ...
      mspec Std.Do.Spec.pure
      mpure_intro
      refine ⟨ΔA ++ ΔC, by rw [pref, C_decl_eq, List.append_assoc], ?_⟩
      · intro v hv
        simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append, List.mem_cons,
          List.not_mem_nil, or_false] at hv
        obtain ⟨hv_body, hv_ne_x⟩ := hv
    ...
    · rename_i _ Senc1 _ heqA _ Senc2 _ heqC _ _
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqA
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqC
      mspec castUnionAux_decl
      mrename_i pre
      mintro ∀σ'
      mpure pre
      obtain ⟨Dlt, cu_decl_eq, cu_spec_sub, cu_fv_sub⟩ := pre
      mpure_intro
      refine ⟨ΔA ++ ΔC ++ Dlt,
        by rw [cu_decl_eq, C_decl_eq]; simp only [List.append_assoc], ?_⟩
      · intro v hv
        have hv' := cu_fv_sub hv
        rw [B.fv, declVars_append, declVars_append]
        rcases List.mem_union_iff.mp hv' with h | h
        · rcases List.mem_union_iff.mp h with h | h
  ...
  | inter A C A_ih C_ih =>
    ...
    · rename_i _ Senc1 _ heqA _ Senc2 _ heqC _ _
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqA
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. |>.mp heqC
      mspec castInterAux_decl
      mrename_i pre
      mintro ∀σ'
      mpure pre
      obtain ⟨Dlt, ci_decl_eq, ci_spec_sub, ci_fv_sub⟩ := pre
      mpure_intro
      refine ⟨ΔA ++ ΔC ++ Dlt,
        by rw [ci_decl_eq, C_decl_eq]; simp only [List.append_assoc], ?_⟩
      · intro v hv
        have hv' := ci_fv_sub hv
        rw [B.fv, declVars_append, declVars_append]
        rcases List.mem_union_iff.mp hv' with h | h
        · rcases List.mem_union_iff.mp h with h | h
          · rcases List.mem_union_iff.mp (A_enc_fv_sub h) with h | h
  ...
  | pfun A C A_ih C_ih =>
    ...
    · rename_i heq
      ...
      · rename_i heq2
        ...
        mpure_intro
        refine ⟨ΔA ++ ΔC,
          by rw [pre4, pre3, pre2, pre, C_decl_eq, List.append_assoc], ?_⟩
        · intro v hv
          simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append,
            List.mem_cons, List.not_mem_nil, or_false] at hv
          obtain ⟨hv_body, hv_ne_R⟩ := hv
  ...
  | app f x f_ih x_ih =>
    ...
    have vars_used_x : ∀ v ∈ x.vars, v ∈ used := fun v hv => vars_used v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inr h)
    have Λ_inv_f : ∀ v ∈ f.vars, v ∈ St.types → v ∈ E.context := fun v hv => Λ_inv v (by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inl h)
    mspec (Std.Do.Triple.and (encodeTerm f E)
      (encodeTerm_state E (Λ := St.types) (n := St.env.freshvarsc) (used := used)
        typ_f vars_used_f Λ_inv_f hf_bv_nodup)
      (f_ih E (Λ := St.types) (n := St.env.freshvarsc) (used := used) (decl := decl)
        typ_f vars_used_f Λ_inv_f hf_bv_nodup))
    mrename_i pre
    mintro ∀σ_f
    mpure pre
    obtain ⟨⟨f_used_sub, f_Λ_sub, f_keys_sub, f_cov, f_fv_sub, f_preserves⟩,
      Δf, f_decl_eq, f_enc_fv_sub⟩ := pre
    rename_i out_f
    obtain ⟨f_enc, σf⟩ := out_f
    have Λ_inv_x : ∀ v ∈ x.vars, v ∈ σ_f.types → v ∈ E.context := fun v hv hΛ => by
      ...
      · have hv_vars_f : v ∈ B.Term.vars f := by
        ...
        · rcases B.Term.mem_vars_iff.mp hv with hx_fv | hx_bv
          ...
          · exact absurd rfl (hfx_bv_disj v h v hx_bv)
    mspec (Std.Do.Triple.and (encodeTerm x E)
      (encodeTerm_state E (Λ := σ_f.types) (n := σ_f.env.freshvarsc)
        (used := σ_f.env.usedVars) typ_x
        (fun v hv => f_used_sub (vars_used_x v hv)) Λ_inv_x hx_bv_nodup)
      (x_ih E (Λ := σ_f.types) (n := σ_f.env.freshvarsc)
        (used := σ_f.env.usedVars) (decl := decl ++ Δf) typ_x
        (fun v hv => f_used_sub (vars_used_x v hv)) Λ_inv_x hx_bv_nodup))
    mrename_i pre
    mintro ∀σ_x
    mpure pre
  ...
  | collect vs D P D_ih P_ih =>
    ...
    have Λ_inv_D : ∀ v ∈ D.vars, v ∈ St₀.types → v ∈ E.context := by
      ...
      · exact .inl (.inl hv)
      · exact .inr (.inr (.inl hv))
    mspec (Std.Do.Triple.and (encodeTerm D E)
      (encodeTerm_state E (Λ := St₀.types) (n := St₀.env.freshvarsc) (used := used)
        (α := .set τ) typ_D vars_used_D Λ_inv_D hD_bv_nodup)
      (D_ih E (Λ := St₀.types) (n := St₀.env.freshvarsc) (used := used)
    ...
    · -- function-`D` arm
      ...
      · -- arity matches
        ...
        have St₂_keys_sub : AList.keys St₂.types ⊆ St₂.env.usedVars := by
          ...
          exact encodeTerm_state.keys_foldl_insert_subset_of_fst_mem _ D_keys_sub
            (fun p hp => vs_sub_St₁_used p.1 (List.of_mem_zip hp).1)
        mspec (Std.Do.Triple.and (encodeTerm P E')
          (encodeTerm_state E' (Λ := St₂.types) (n := St₂.env.freshvarsc)
            (used := St₂.env.usedVars) (α := .bool) typP vars_used_P_St₂ Λ_inv_P hP_bv_nodup)
          (P_ih E' (Λ := St₂.types) (n := St₂.env.freshvarsc)
        ...
        · -- `encodeTerm P` returned a boolean
          ...
          mspec (Std.Do.Triple.and
            (castApp (D_enc, α'.fun β'.option) ((List.map SMT.Term.var xs).toPairl, αs'.toProdl))
            (castApp_state D_enc ((List.map SMT.Term.var xs).toPairl) (α'.fun β'.option)
              αs'.toProdl (Λ := St₄.types) (n := St₄.env.freshvarsc)
              (used := St₄.env.usedVars) (X := []))
            (castApp_decl D_enc ((List.map SMT.Term.var xs).toPairl) (α'.fun β'.option)
              αs'.toProdl (Λ := St₄.types) (n := St₄.env.freshvarsc)
          ...
          case post.success =>
            ...
            mpure pre
            obtain ⟨⟨ca_le, ca_Λ_sub, ca_used_sub, ca_keys_sub, ca_fv_sub,
              ca_preserves⟩, Δca, ca_decl_eq, ca_spec_sub, ca_fv_decl_sub⟩ := pre
            mspec Std.Do.Spec.pure
            mpure_intro
            refine ⟨ΔD ++ ΔP ++ Δca, by rw [ca_decl_eq]; simp only [List.append_assoc], ?_⟩
            have hBfv : B.fv (B.Term.collect vs D P) = B.fv D ++ (B.fv P).removeAll vs := by
              rw [B.fv]
            have hdv : declVars (ΔD ++ ΔP ++ Δca)
                = declVars ΔD ++ declVars ΔP ++ declVars Δca := by
              rw [declVars_append, declVars_append]
            -- `fv` of the `castApp` result lands in the target.
            have Dxs_fv : ∀ w ∈ SMT.fv Dxs, w ∉ xs →
                w ∈ B.fv (B.Term.collect vs D P) ∪ declVars (ΔD ++ ΔP ++ Δca) := by
              intro w hw hw_xs
              rw [hBfv, hdv]
              ...
              · rcases List.mem_union_iff.mp h with hD | hpairl
                · rcases List.mem_union_iff.mp (D_fv_sub hD) with hbd | hdd
                  · exact List.mem_union_iff.mpr (.inl (List.mem_append_left _ hbd))
            ...
            · rcases SMT_mem_fv_substList hv_subst with hvP | ⟨t, ht, hvt⟩
              · by_cases hvs : v ∈ vs
                ...
                · rw [hBfv, hdv]
                  ...
                  · exact List.mem_union_iff.mpr (.inl (List.mem_append_right _
                      (List.mem_removeAll_iff.mpr ⟨hfv, hvs⟩)))
                  · exact List.mem_union_iff.mpr (.inr (List.mem_append_left _
    ...
    · -- set-`D` arm
      ...
      · -- `encodeTerm P` returned a boolean
        ...
        mintro ∀St₄
        mpure pre4
        mspec SMT.eraseFromContext_decls
        mrename_i pre5
        mintro ∀St₅
        mpure pre5
        mspec Std.Do.Spec.pure
        mpure_intro
        refine ⟨ΔD ++ ΔP, ?_, ?_⟩
        · rw [pre5, pre4, P_decl_eq, List.append_assoc]
        · intro v hv
          simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append, List.mem_singleton,
            List.not_mem_nil, or_false] at hv
          obtain ⟨hv_body, hv_ne_z⟩ := hv
          rw [B.fv, declVars_append]
          have toDestPair_fv : ∀ t ∈ toDestPair vs (SMT.Term.var z), ∀ w ∈ SMT.fv t,
              w = z := fun t ht w hw => SMT_fv_toDestPair_subset ht hw
          rcases hv_body with (hvD | hvz1) | hvsubst
          · rcases List.mem_union_iff.mp (D_fv_sub hvD) with h | h
            · exact List.mem_union_iff.mpr (.inl (List.mem_append_left _ h))
            · exact List.mem_union_iff.mpr (.inr (List.mem_append_left _ h))
          · exact absurd hvz1 hv_ne_z
          · rcases SMT_mem_fv_substList hvsubst with hvP | ⟨t, ht, hvt⟩
            · rcases List.mem_union_iff.mp (P_fv_sub hvP) with hfv | hdv
              · have hv_notin_vs : v ∉ vs := by
                  intro hvs
                  exact SMT_not_mem_fv_substList_of_mem_vars
                    (toDestPair_len_ge vs (SMT.Term.var z))
                    hvs (fun t ht hvt => hv_ne_z (toDestPair_fv t ht v hvt)) hvsubst
                exact List.mem_union_iff.mpr (.inl (List.mem_append_right _
                  (List.mem_removeAll_iff.mpr ⟨hfv, hv_notin_vs⟩)))
  ...
  | lambda vs D P D_ih P_ih =>
    ...
    · rename_i τ' heq
      ...
      mspec (Std.Do.Triple.and (encodeTerm P E')
        (encodeTerm_state E' (Λ := St₂.types) (n := St₂.env.freshvarsc)
          (used := St₂.env.usedVars) (α := β) typP vars_used_P_St₂ Λ_inv_P hP_bv_nodup)
        (P_ih E' (Λ := St₂.types) (n := St₂.env.freshvarsc)
      ...
      · intro v hv
        ...
        obtain ⟨hv_body, hv_ne_xy⟩ := hv
        rw [B.fv, declVars_append]
        have toDestPair_fv : ∀ t ∈ toDestPair vs (SMT.Term.fst (.var xy)), ∀ w ∈ SMT.fv t,
            w = xy := by
          intro t ht w hw
          exact SMT_fv_toDestPair_subset_base (t₀ := SMT.Term.fst (.var xy))
            (by intro u hu; simp only [SMT.fv, List.mem_singleton] at hu; exact hu) ht hw
        ...
        · rcases SMT_mem_fv_substList hvsubst with hvP | ⟨t, ht, hvt⟩
          · rcases List.mem_union_iff.mp (P_fv_sub hvP) with hfv | hdv
            · have hv_notin_vs : v ∉ vs := by
                intro hvs
                exact SMT_not_mem_fv_substList_of_mem_vars
                  (toDestPair_len_ge vs (SMT.Term.fst (.var xy)))
                  hvs (fun t ht hvt => hv_ne_xy (toDestPair_fv t ht v hvt)) hvsubst
              exact List.mem_union_iff.mpr (.inl (List.mem_append_right _
                (List.mem_removeAll_iff.mpr ⟨hfv, hv_notin_vs⟩)))
            · exact List.mem_union_iff.mpr (.inr (List.mem_append_right _ hdv))
          · exact absurd (toDestPair_fv t ht v hvt) hv_ne_xy
...
the three components (source vars, declared helpers, result-term binders) embeds.
The workhorse for propagating the `decl_post` spec-body clause through compound
cases of `encodeTerm_combined`. -/
theorem specBody_mono {b : SMT.Term} {vx vt dx dt bx bt : List SMT.𝒱}
    (hv : vx ⊆ vt) (hd : dx ⊆ dt) (hb : bx ⊆ bt)
    (h : SMT.fv b ⊆ vx ∪ dx ∪ bx) : SMT.fv b ⊆ vt ∪ dt ∪ bt := by
  intro w hw
  rcases List.mem_union_iff.mp (h hw) with hvd | hbb
  · rcases List.mem_union_iff.mp hvd with hvv | hdd
    · exact List.mem_union_iff.mpr (.inl (List.mem_union_iff.mpr (.inl (hv hvv))))
    · exact List.mem_union_iff.mpr (.inl (List.mem_union_iff.mpr (.inr (hd hdd))))
  · exact List.mem_union_iff.mpr (.inr (hb hbb))

set_option maxHeartbeats 4000000 in
/-- Combined structural specification of `encodeTerm`: the `encodeTerm_state`
postcondition (with conjunct 5 weakened by `B.bv t` slack) **and** the
...
theorem encodeTerm_combined
  ...
  | maplet x y x_ih y_ih =>
    ...
    · intro v hv
      ...
      · rcases List.mem_union_iff.mp (x_fv_sub hv) with hk | hb
        ...
        · exact List.mem_union_iff.mpr (.inr (hbv_par ▸ List.mem_append_left _ hb))
      · rcases List.mem_union_iff.mp (y_fv_sub hv) with hk | hb
        · exact List.mem_union_iff.mpr (.inl hk)
        · exact List.mem_union_iff.mpr (.inr (hbv_par ▸ List.mem_append_right _ hb))
    · intro v hv hΛ hvars
    ...
    · intro b hb
      ...
      have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.pair x_enc y_enc) ∧
          SMT.bv y_enc ⊆ SMT.bv (SMT.Term.pair x_enc y_enc) := by
        constructor <;> intro w hw <;> rw [SMT.bv] <;>
          [exact List.mem_append_left _ hw; exact List.mem_append_right _ hw]
      rcases hb with hb | hb
      · exact specBody_mono hvars_x_sub (declVars_append .. ▸ List.subset_append_left ..)
          hbv_t'.1 (x_specb b hb)
      · exact specBody_mono hvars_y_sub (declVars_append .. ▸ List.subset_append_right ..)
          hbv_t'.2 (y_specb b hb)
    · intro v hv
      rw [SMT.fv, List.mem_append] at hv
      rw [declVars_append]
      rcases hv with hv | hv
  ...
  | add x y x_ih y_ih =>
    ...
    · rename_i heq
      ...
      · rename_i heq2
        ...
        mspec Std.Do.Spec.pure
        mpure_intro
        have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.add x_enc y_enc) ∧
            SMT.bv y_enc ⊆ SMT.bv (SMT.Term.add x_enc y_enc) := by
          constructor <;> intro w hw <;> rw [SMT.bv] <;>
            [exact List.mem_append_left _ hw; exact List.mem_append_right _ hw]
        refine ⟨⟨?_, ?_, ?_, ?_, ?_, ?_⟩, Δx ++ Δy, ?_, ?_, ?_⟩
        · exact fun v hv => y_used_sub (x_used_sub hv)
  ...
  | sub x y x_ih y_ih =>
    ...
    · rename_i heq
      ...
      · rename_i heq2
        ...
        mspec Std.Do.Spec.pure
        mpure_intro
        have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.sub x_enc y_enc) ∧
            SMT.bv y_enc ⊆ SMT.bv (SMT.Term.sub x_enc y_enc) := by
          constructor <;> intro w hw <;> rw [SMT.bv] <;>
            [exact List.mem_append_left _ hw; exact List.mem_append_right _ hw]
        refine ⟨⟨?_, ?_, ?_, ?_, ?_, ?_⟩, Δx ++ Δy, ?_, ?_, ?_⟩
        · exact fun v hv => y_used_sub (x_used_sub hv)
  ...
  | mul x y x_ih y_ih =>
    ...
    · rename_i heq
      ...
      · rename_i heq2
        ...
        mspec Std.Do.Spec.pure
        mpure_intro
        have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.mul x_enc y_enc) ∧
            SMT.bv y_enc ⊆ SMT.bv (SMT.Term.mul x_enc y_enc) := by
          constructor <;> intro w hw <;> rw [SMT.bv] <;>
            [exact List.mem_append_left _ hw; exact List.mem_append_right _ hw]
        refine ⟨⟨?_, ?_, ?_, ?_, ?_, ?_⟩, Δx ++ Δy, ?_, ?_, ?_⟩
        · exact fun v hv => y_used_sub (x_used_sub hv)
  ...
  | le x y x_ih y_ih =>
    ...
    have hvars_y_sub : B.Term.vars y ⊆ B.Term.vars (x ≤ᴮ y) := fun v hv => by
      simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hv ⊢
      rcases hv with h | h <;> [left; right] <;> exact .inr h
    have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.le x_enc y_enc) ∧
        SMT.bv y_enc ⊆ SMT.bv (SMT.Term.le x_enc y_enc) := by
      constructor <;> intro w hw <;> rw [SMT.bv] <;>
        [exact List.mem_append_left _ hw; exact List.mem_append_right _ hw]
    refine ⟨⟨?_, ?_, ?_, ?_, ?_, ?_⟩, Δx ++ Δy, ?_, ?_, ?_⟩
    · exact fun v hv => y_used_sub (x_used_sub hv)
  ...
  | and x y x_ih y_ih =>
    ...
    · rename_i heq
      ...
      · rename_i heq2
        ...
        mspec Std.Do.Spec.pure
        mpure_intro
        have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.and x_enc y_enc) ∧
            SMT.bv y_enc ⊆ SMT.bv (SMT.Term.and x_enc y_enc) := by
          constructor <;> intro w hw <;> rw [SMT.bv] <;>
            [exact List.mem_append_left _ hw; exact List.mem_append_right _ hw]
        refine ⟨⟨?_, ?_, ?_, ?_, ?_, ?_⟩, Δx ++ Δy, ?_, ?_, ?_⟩
        · exact fun v hv => y_used_sub (x_used_sub hv)
  ...
  | not x ih =>
    ...
    · rename_i heq
      ...
      mspec Std.Do.Spec.pure
      mpure_intro
      have hbv_t' : SMT.bv x_enc ⊆ SMT.bv (SMT.Term.not x_enc) := by
        intro w hw; rwa [SMT.bv]
      ...
      · intro b hb
        exact specBody_mono hvars_x_sub (fun w hw => hw) hbv_t' (x_specb b hb)
      · intro v hv
        simp only [SMT.fv] at hv
        rcases List.mem_union_iff.mp (x_enc_fv_sub hv) with h | h
        · exact List.mem_union_iff.mpr (.inl (hvars_x_sub h))
    ...
    · rename_i α heq
      ...
      case post.success x =>
        ...
        case post.success ℰ =>
          ...
          · intro b name hb
            have hbv_S' : SMT.bv S_enc ⊆ SMT.bv (SMT.Term.lambda [ℰ] [.fun α .bool]
                (.forall [x] [α] (.imp (.app (.var ℰ) (.var x))
                  (.app S_enc (.var x))))) := by
              intro w hw
              simp only [SMT.bv, List.mem_append, List.mem_cons]
              tauto
            exact specBody_mono hvars_S_sub (fun w hw => hw) hbv_S'
      ...
      case post.success x =>
        ...
        case post.success y =>
          ...
          case post.success f =>
            ...
            · intro b name hb
              have hbv_S' : SMT.bv S_enc ⊆ SMT.bv (SMT.Term.lambda [f]
                  [α.fun γ.option]
                  (.forall [x, y] [α, γ] (.imp
                    (.eq (.app (.var f) (.var x)) (.var y))
                    (.eq (.app S_enc (.var x)) (.var y))))) := by
                intro w hw
                simp only [SMT.bv, List.mem_append, List.mem_cons]
                tauto
              exact specBody_mono hvars_S_sub (fun w hw => hw) hbv_S'
