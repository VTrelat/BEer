...
instance : LT {x // x ∈ Int} where
  ...
  lt x y :=
    let f := Classical.choice (Function.Embedding.antisymm ZFInt.EmbeddingZFIntInt ZFInt.EmbeddingIntZFInt)
...
theorem ZFInt.exists_mono_bij_zero_eq.{u} :
  Nonempty {f : ZFInt.{u} → {x // x ∈ Int.{u}} // Function.Bijective f ∧ (∀ x y, f x < f y ↔ x < y) ∧ f 0 = ⟨ofInt 0, mem_ofInt_Int 0⟩}
  := by
  rw [nonempty_subtype]
  by_contra!
