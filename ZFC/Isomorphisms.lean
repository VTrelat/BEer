...
open Classical in
theorem bijective_of_injective_on_subset {A B : ZFSet} (B_sub : B ⊆ A) {u : ZFSet} {hu : A.IsFunc B u} (u_inj : IsInjective u) :
...
theorem isIso_of_prod {A B C D : ZFSet} (h : A ≅ᶻ C) (h' : B ≅ᶻ D) : A.prod B ≅ᶻ C.prod D := by
  ...
  · and_intros
    · intro ab a'b' cd hab ha'b' hcd habcd ha'b'cd
      ...
      obtain ⟨⟨⟨a, ha, b, hb, rfl⟩, c, hc, d, hd, rfl⟩, _, _, _, _, ⟨ab_eq, cd_eq⟩, ac_f₁, bd_f₂⟩ := habcd
...
theorem inv_Image_of_bijective {f A B : ZFSet} {hf : A.IsFunc B f} (bij : f.IsBijective) {X : ZFSet} (hX : X ⊆ A) :
    f⁻¹[(f[X])] = X := by
  ext1 x
...
theorem Image_inv_of_bijective {f A B : ZFSet} {hf : A.IsFunc B f} (bij : f.IsBijective hf) {X : ZFSet} (hX : X ⊆ B) :
    f[(f⁻¹[X])] = X := by
  let g := f⁻¹
...
theorem IsInjective_of_left_inverse {A B : ZFSet} {f : ZFSet} (hf : A.IsFunc B f) {g : ZFSet} (hg : B.IsFunc A g)
  (left_inv : g ∘ᶻ f = 𝟙A) : IsInjective f := by
...
theorem IsSurjective_of_right_inverse {A B : ZFSet} {f : ZFSet} (hf : A.IsFunc B f) {g : ZFSet} (hg : B.IsFunc A g)
  (right_inv : f ∘ᶻ g = 𝟙B) : IsSurjective f := by
  intro y yB
...
theorem isIso_of_two_sided_inverse {A B : ZFSet} {f : ZFSet} {hf : A.IsFunc B f} {g : ZFSet} {hg : B.IsFunc A g}
  (left_inv : g ∘ᶻ f = 𝟙A) (right_inv : f ∘ᶻ g = 𝟙B) : A ≅ᶻ B := by
  use f, hf
  and_intros
...
theorem isIso_powerset {A B : ZFSet} (h : A ≅ᶻ B) : A.powerset ≅ᶻ B.powerset := by
  ...
  have left_inv : F' ∘ᶻ F = 𝟙A.powerset := by
    ...
    · intro hX
      rw [mem_Id_iff] at hX
      obtain ⟨X, hX, rfl⟩ := hX
      simp only [mem_powerset, pair_inj, exists_eq_right_right', and_self, existsAndEq, and_true, exists_eq_left']
        ...
        · rw [lambda_spec]
          ...
          · conv_lhs => rw [lambda_eta hf]
            ...
            · intro a ha
              ...
              rw [fapply_lambda (ha := by rwa [mem_funs]), dite_cond_eq_true (eq_true ha), dite_cond_eq_true (eq_true ha),
        ...
        · rw [lambda_spec]
          ...
          · rw [dite_cond_eq_true (eq_true ?_)]
            · ext1 ab
              ...
              have fapp_eq : ↑(@ᶻG ⟨S, S_Gdom⟩) = (λᶻ : A → B.powerset | a ↦ if ha : a ∈ A then B.sep (fun b => a.pair b ∈ S) else ∅) := by
                  rw [fapply_lambda]
                  · intros
                    apply mem_funs_of_lambda
                    intro _ ha
      ...
      have bij' : IsBijective f' this := by
        ...
        · rw [bijective_exists1_iff] at bij
          ...
          · obtain ⟨x, ⟨x_mem_succ_n, x_k_f⟩, x_unq⟩ := this
            ...
            · nomatch y_ne_ℓ <| isfunc.2 _ (by rw [add_one_eq_succ]; exact lt_succ) |>.unique x_k_f hℓ
            · and_intros <;> beta_reduce
              · exact x_mem_n
      ...
      have k_iso : ↑k ≅ᶻ (↑(k+1) \ {ℓ}) := by
        ...
        · and_intros
          · intro x y z hz hy hz g_xz g_yz
            ...
            simp only [mem_sep, mem_prod, mem_diff, mem_singleton, pair_inj, exists_eq_right_right', π₁_pair, π₂_pair] at g_xz g_yz
            obtain ⟨⟨mem_x_k, mem_z_succ_k, z_ne_ℓ⟩, z_eq⟩ := g_xz
            obtain ⟨⟨mem_y_k, -, -⟩, z_eq'⟩ := g_yz

            -- reason into naturals
            have z_Nat : z ∈ Nat := mem_Nat_of_mem_mem_Nat (k.succ.prop) (by rwa [add_one_eq_succ] at mem_z_succ_k)
            have x_Nat : x ∈ Nat := mem_Nat_of_mem_mem_Nat (k.prop) mem_x_k
            have y_Nat : y ∈ Nat := mem_Nat_of_mem_mem_Nat (k.prop) hy
            have ℓ_Nat : ℓ ∈ Nat := mem_Nat_of_mem_mem_Nat (k.succ.prop) (by rwa [add_one_eq_succ] at ℓ_mem_m)
            let Z : ZFNat := ⟨z, z_Nat⟩
            let X : ZFNat := ⟨x, x_Nat⟩
            let Y : ZFNat := ⟨y, y_Nat⟩
            let L : ZFNat := ⟨ℓ, ℓ_Nat⟩
          ...
          · intro y hy
            ...
            · by_cases y_lt_ℓ : y ∈ ℓ
              · change Y < L at y_lt_ℓ
                use y, y_mem_k
                dsimp [g]
                simp only [mem_sep, mem_prod, mem_diff, mem_singleton, pair_inj, exists_eq_right_right', π₁_pair, π₂_pair]
                split_ifs
                · and_intros
                  · exact y_mem_k
                  · change Y < k + 1
