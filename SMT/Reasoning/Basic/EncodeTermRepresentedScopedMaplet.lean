...
/-! # Generated-helper contracts for maplets -/

namespace EncodeTermRepresentedMaplet

@[simp] theorem fv_smtTerm (x y : SMT.Term) :
    SMT.fv (SMT.Term.pair x y) = SMT.fv x ++ SMT.fv y := by
  simp [SMT.fv]

@[simp] theorem bv_smtTerm (x y : SMT.Term) :
    SMT.bv (SMT.Term.pair x y) = SMT.bv x ++ SMT.bv y := by
  simp [SMT.bv]

theorem smt_denote_inv.{u}
    {x y : SMT.Term} {Θ : SMT.RenamingContext.Context.{u}}
    (hcov : SMT.RenamingContext.CoversFV Θ (SMT.Term.pair x y))
    {d : SMT.Dom.{u}}
    (hden : ⟦(SMT.Term.pair x y).abstract Θ hcov⟧ˢ = some d) :
    ∃ (dx dy : SMT.Dom.{u}),
      ⟦x.abstract Θ (fun v hv => hcov v (by
        rw [fv_smtTerm, List.mem_append]
...
end EncodeTermRepresentedMaplet

set_option maxHeartbeats 4000000 in
theorem encodeTerm_rep_scoped.maplet_case_from.{u}
    (x y : B.Term)
  ...
  have vars_used_x : ∀ v ∈ x.vars, v ∈ used := by
    intro v hv
    apply vars_used v
    simp only [B.Term.vars, B.fv, B.bv, List.mem_union_iff,
      List.mem_append]
  ...
  have Λ_inv_y : ∀ v ∈ y.vars, v ∈ Stx.types → v ∈ E.context := by
    intro v hv hΓ
    have hv_parent : v ∈ (x ↦ᴮ y).vars := by
      simp only [B.Term.vars, B.fv, B.bv, List.mem_union_iff,
        List.mem_append]
      rcases B.Term.mem_vars_iff.mp hv with h | h
      · exact .inl (.inr h)
  ...
  · intro Δ_alt Δ_fv_alt Δ₀_alt related_alt wf_alt
    ...
    have hcov_op_alt : RenamingContext.CoversFV Δy_alt
      ...
      intro v hv
      rw [EncodeTermRepresentedMaplet.fv_smtTerm,
    ...
    refine ⟨Δy_alt, hcov_op_alt, denPairAlt, Δy_alt_ext₀,
    ...
    · simp [denPairAlt, SMT.Term.abstract, SMT.denote,
        hden_x_alt_final, hden_y_alt]
  ...
  · intro Γ_sup Γ_sub Δ_alt Δ_fv_alt Θ related_alt wf_alt
    ...
    injection pair_alt_eq with α_alt_eq β_alt_eq
    subst α_alt β_alt
    obtain ⟨⟨Xenc_alt, σX_alt, hXenc_alt⟩,
      ⟨Yenc_alt, σY_alt, hYenc_alt⟩, hden_x_target,
      hden_y_target, denOut_eq⟩ :=
      EncodeTermRepresentedMaplet.smt_denote_inv hcov hdenOut
    subst denOut
    dsimp at hdenOut_type
    injection hdenOut_type with σX_alt_eq σY_alt_eq
    subst σX_alt σY_alt
    have Γ_sub' :
    ...
    have X_rel_target := x_guard Γ_sup x_scope Δ_alt
      ...
      hcov_x_target ⟨Xenc_alt, σx, hXenc_alt⟩
      hden_x_target rfl
    have Y_rel_target := y_guard Γ_sup y_scope Δ_alt
      ...
      hcov_y_target ⟨Yenc_alt, σy, hYenc_alt⟩
      hden_y_target rfl
    simpa only [proof_irrel_heq] using
      RDomCastSupported.pair X_rel_target Y_rel_target
  · intro body hbody
  ...
  · constructor
    · intro Γ_sup Γ_sub result_bv_fresh
      ...
      have x_bv_fresh : ∀ v ∈ SMT.bv x_enc, v ∉ Γ_sup := by
        ...
        rw [EncodeTermRepresentedMaplet.bv_smtTerm,
          List.mem_append]
        exact Or.inl hv
      ...
      · exact x_sc_typing.1 Γ_sup x_scope x_bv_fresh
      · exact y_sc_typing.1 Γ_sup y_scope y_bv_fresh
