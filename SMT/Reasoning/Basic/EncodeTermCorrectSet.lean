...
theorem pow_denotation_aux.{u}
  ...
  have typ_S_enc_E_insert :
    ...
    Typing.weakening
      ...
      typ_S_enc
      (SMT.Typing.bv_notMem_insert_of_fresh typ_S_enc hE_not_bv_S_enc)
  have typ_S_enc_Ex_insert :
    ...
    Typing.weakening
      ...
      typ_S_enc_E_insert
      (SMT.Typing.bv_notMem_insert_of_fresh typ_S_enc_E_insert hx_not_bv_S_enc)
  have typ_pred :
  ...
  have hpred_total :
    ...
    have hDnot_ty : Dnot.snd.fst = SMTType.bool := by
      have hcov_not_rhs : SMT.RenamingContext.CoversFV
          (Function.update (Function.update Δ_ctx E (some F)) x (some W))
          (¬ˢ ((@ˢS_enc) (SMT.Term.var x))) := by
        intro v hv
        have hv' : v ∈ SMT.fv ((@ˢS_enc) (SMT.Term.var x)) := by simpa [SMT.fv] using hv
        have hv'' : v ∈ SMT.fv S_enc ∨ v = x := by simpa [SMT.fv] using hv'
        exact hcov_pred_upd F W v (by
          rw [hpred_def]; simp only [SMT.fv, List.mem_append, List.mem_singleton]
          exact Or.inr hv'')
      apply SMT.RenamingContext.denote_type_of_typing_fv typ_not_rhs _ hcov_not_rhs hden_not
      intro v σ hv hlk
      have hv_S_or_x : v ∈ SMT.fv S_enc ∨ v = x := by
        have hv' : v ∈ SMT.fv ((@ˢS_enc) (SMT.Term.var x)) := by simpa [SMT.fv] using hv
        simpa [SMT.fv] using hv'
      by_cases hvx : v = x
      · subst hvx
      ...
      · rw [AList.lookup_insert_ne hvx] at hlk
        have hv_S : v ∈ SMT.fv S_enc := hv_S_or_x.resolve_right hvx
  ...
  have hpred_ty :
    ...
    apply SMT.RenamingContext.denote_type_of_typing_fv typ_pred _ (hcov_pred_upd F W) hD
    intro v σ hv hlk
    -- v ∈ SMT.fv pred = SMT.fv (((@ˢE) x) ⇒ˢ ((@ˢS_enc) x))
    -- = {E, x} ∪ (SMT.fv S_enc ∪ {x}) = {E, x} ∪ SMT.fv S_enc
    have hv_cases : v = E ∨ v = x ∨ v ∈ SMT.fv S_enc := by
      rw [hpred_def] at hv
      simp only [SMT.fv, List.mem_append, List.mem_singleton, List.mem_cons,
    ...
    · rw [AList.lookup_insert_ne hvx] at hlk
      ...
      · subst hvE
        rw [AList.lookup_insert] at hlk
        cases hlk
        exact ⟨F, by rw [Function.update_of_ne hvx, Function.update_self], hF_ty⟩
      · rw [AList.lookup_insert_ne hvE] at hlk
        have hv_S : v ∈ SMT.fv S_enc := by
          rcases hv_cases with hE | hx | hvS
          · exact absurd hE hvE
          · exact absurd hx hvx
          · exact hvS
        obtain ⟨d, hd⟩ := Option.isSome_iff_exists.mp (hΔ_covers_S v hv_S)
  ...
  have hsome_tpow :
    ...
    · exfalso
      ...
      calc
        ...
        _ = SMTType.bool := by
            apply SMT.RenamingContext.denote_type_of_typing_fv typ_forall _ (hcov_forall_upd Fx) hDx
            intro v σ hv hlk
            have hv_cases : v = E ∨ v ∈ SMT.fv S_enc := by
              rw [hpred_def] at hv
              simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append, List.mem_singleton,
                List.mem_cons, List.mem_nil_iff, or_false] at hv
              obtain ⟨hv_inner, hv_ne_x⟩ := hv
              rcases hv_inner with ((hE | hx) | hvS | hx)
              · exact Or.inl hE
              · exact absurd hx hv_ne_x
              · exact Or.inr hvS
              · exact absurd hx hv_ne_x
            by_cases hvE : v = E
            · subst hvE
              rw [AList.lookup_insert] at hlk
              cases hlk
              exact ⟨Fx, by rw [Function.update_self], hFx_ty⟩
            · rw [AList.lookup_insert_ne hvE] at hlk
        ...
        _ = Dy.snd.fst := by
            symm
            apply SMT.RenamingContext.denote_type_of_typing_fv typ_forall _ (hcov_forall_upd Fy) hDy
            intro v σ hv hlk
            have hv_cases : v = E ∨ v ∈ SMT.fv S_enc := by
              rw [hpred_def] at hv
              simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append, List.mem_singleton,
            ...
            · subst hvE
              rw [AList.lookup_insert] at hlk
              cases hlk
              exact ⟨Fy, by rw [Function.update_self], hFy_ty⟩
            · rw [AList.lookup_insert_ne hvE] at hlk
              have hv_S : v ∈ SMT.fv S_enc := hv_cases.resolve_left hvE
              obtain ⟨d, hd⟩ := Option.isSome_iff_exists.mp (hΔ_covers_S v hv_S)
              refine ⟨d, ?_, Δctx_wt_fv hv_S hd hlk⟩
  ...
  have hden :
    ...
    have hτa : τa = .fun (.fun β.toSMTType .bool) .bool := by
      apply SMT.RenamingContext.denote_type_of_typing_fv typ_tpow _ hΔ_covers_tpow hD
      intro v σ hv hlk
      have hv_S : v ∈ SMT.fv S_enc := by
        rw [htpow_def, hpred_def] at hv
        simp only [SMT.fv, List.mem_removeAll_iff, List.mem_append, List.mem_singleton,
          List.mem_cons, List.mem_nil_iff, or_false, List.not_mem_nil] at hv
        obtain ⟨⟨hv_inner, hv_ne_x⟩, hv_ne_E⟩ := hv
        rcases hv_inner with ((hvE | hvx) | hvS | hvx)
        · exact absurd hvE hv_ne_E
          ...
          refine funUnaryForallEqZftrue
            ...
            (hbody_true := by
              ...
              · have hnot_isSome :
                ...
                have hDnot_ty : Dnot.snd.fst = SMTType.bool := by
                  have hcov_not_rhs : SMT.RenamingContext.CoversFV
                      (Function.update (Function.update Δ_ctx E (some (setDom hY_mem))) x (some W))
                      (¬ˢ ((@ˢS_enc) (SMT.Term.var x))) := by
                    intro v hv
                    have hv' : v ∈ SMT.fv ((@ˢS_enc) (SMT.Term.var x)) := by simpa [SMT.fv] using hv
                    have hv'' : v ∈ SMT.fv S_enc ∨ v = x := by simpa [SMT.fv] using hv'
...
theorem encodeTerm_spec.pow_case.{u} (fv_sub_typings : B.FvSubTypings) (S : B.Term)
                ...
                ∀ {T : ZFSet.{u}} {hT : T ∈ ⟦α⟧ᶻ},
                  ⟦S.abstract «Δ» Δ_fv⟧ᴮ = some ⟨T, ⟨α, hT⟩⟩ →
                    (∀ v ∈ S.vars, v ∈ used) →
                      ...
                      ((B.bv S).Nodup) →
                        B.RenamingContext.RespectsTypeContextOnFV (B.RenamingContext.toSMT «Δ») Λ S →
                        (∀ v ∈ B.fv S, v ∈ Λ) →
                        B.RenWF E.context «Δ» →
                        ∀ {n : ℕ},
                          ⦃fun x =>
  ...
  (Λ_inv : ∀ v ∈ ( 𝒫ᴮ S).vars, v ∈ Λ → v ∈ E.context)
  (bv_nodup : (B.bv ( 𝒫ᴮ S)).Nodup)
  (respects : B.RenamingContext.RespectsTypeContextOnFV (B.RenamingContext.toSMT «Δ») Λ (( 𝒫ᴮ S)))
  (fv_in_Λ : ∀ v ∈ B.fv (( 𝒫ᴮ S)), v ∈ Λ)
              ...
              Λ ⊆ Γ' ∧
                AList.keys Γ' ⊆ E'.usedVars ∧
                  CoversUsedVars E'.usedVars ( 𝒫ᴮ S) ∧
                    σ = α.toSMTType ∧
                      Γ' ⊢ˢ t' : σ ∧
                        (∀ v ∈ used, v ∉ Λ → v ∉ B.Term.vars ( 𝒫ᴮ S) → v ∉ Γ') ∧
                          ∃ Δ',
                            ∃ (Δ'_covers : RenamingContext.CoversFV Δ' t'),
                              RenamingContext.Extends Δ' Δ₀ ∧
  ...
  -- `B.denote` of `𝒫ᴮ'` pattern-matches its operand against `.set _`
  -- (`| failure` otherwise) and returns `α.set.set`; matching this against the
  -- known result type `β.set.set` pins `τX = β.set` — no `of_abstract` needed.
  cases τX <;> first | rw [Option.some_inj] at eq | exact absurd eq (by simp)
  rename_i τX
  injection eq with T_eq hτeq
  subst T_eq
  ...
  mspec (Std.Do.Triple.and _
    (ih (E := E) (Λ := St.types) (α := .set β) typ_S
      («Δ» := «Δ») Δ_fv_S
      ...
      (show (B.bv S).Nodup from bv_nodup)
      (respects.mono_fv (fun v hv => by simpa [B.fv] using hv))
      (fun v hv => fv_in_Λ v (by simpa [B.fv] using hv))
      wf
      (n := St.env.freshvarsc))
  ...
  obtain ⟨⟨St_used_sub_St', St_eq_St', St'_sub, S_cov_St', rfl, typ_S_enc, S_preserves,
    ...
    ⟨Senc, _, hSenc⟩, den_S_enc, ⟨rfl, retract_Senc_eq_X⟩, ih_total⟩,
    bv_S_enc_used, _⟩ := pre
            ...
            · constructor
              ...
              · set pred : SMT.Term :=
                ...
                have hE_not_bv_S_enc : E ∉ SMT.bv S_enc := by
                  intro hEbv
                  exact E_not_used (St₁_used_eq ▸ List.mem_cons_of_mem _ (bv_S_enc_used E hEbv))
                have hx_not_bv_S_enc : x ∉ SMT.bv S_enc := fun hxbv =>
                  x_not_used (bv_S_enc_used x hxbv)
                have typ_S_enc_E_insert :
                    ctx.insert E (.fun β.toSMTType .bool) ⊢ˢ S_enc : β.set.toSMTType :=
                  Typing.weakening
                    (h := SMT.TypeContext.entries_subset_insert_of_notMem hE_not_ctx)
                ...
                · constructor
                  · -- preserves_types
                    intro v hv h1 h2
                    rw [B.Term.notMem_vars_pow] at h2
                      ...
                      · constructor
                        ...
                        · refine ⟨hpow_cov, ?_⟩
                          ...
                          · -- alt universality for pow
                            ...
                            rw [B.Term.abstract, B.denote, Option.bind_eq_bind, Option.pure_def,
                              Option.bind_eq_some_iff] at den_t_alt
                            obtain ⟨⟨X_alt, α_alt, hX_alt⟩, den_S_alt, eq_alt⟩ := den_t_alt
                            -- `B.denote` of `𝒫ᴮ'` forces the operand tag to `.set _`
                            -- and returns `α.set.set`; matching the known result
                            -- type pins `α_alt = .set β` — no `of_abstract` needed.
                            cases α_alt <;> first | rw [Option.some_inj] at eq_alt | exact absurd eq_alt (by simp)
                            rename_i α_alt
                            injection eq_alt with hT_eq hτeq
                            subst hT_eq
                            simp only [heq_eq_eq, PSigma.mk.injEq, BType.set.injEq] at hτeq
                            obtain ⟨hτeq, _⟩ := hτeq
                            subst α_alt
                            ...
                            obtain ⟨Δ'_alt_S, hcov_alt_S, denT_S_alt, hext_alt_S,
                              ...
                              ih_total Δ_alt (fun v hv => Δ_fv_alt v (by simp [B.fv]; exact hv))
                                Δ₀_alt_S Δ₀_alt_S_ext wf_alt Δ₀_alt_S_none Δ₀_alt_S_wt X_alt hX_alt
...
theorem cprod_case_denotation_aux.{u_1}
    ...
    (Δctx_wt : ∀ v (d : SMT.Dom), Δ'' v = some d → ∀ τ, ctx.lookup v = some τ → d.snd.fst = τ)
    (p a b : SMT.𝒱)
    (p_fresh : p ∉ ctx)
    ...
    (hp_not_bv_S : p ∉ SMT.bv S_enc) (hp_not_bv_T : p ∉ SMT.bv T_enc)
    (ha_not_bv_S : a ∉ SMT.bv S_enc) (ha_not_bv_T : a ∉ SMT.bv T_enc)
    (hb_not_bv_S : b ∉ SMT.bv S_enc) (hb_not_bv_T : b ∉ SMT.bv T_enc)
    {Senc Tenc : ZFSet.{u_1}}
          ...
          ⟨X.prod Y, ⟨(αx ×ᴮ βx).set, hT⟩⟩ ≘ᶻ denT' := by
              ...
              have typ_S_enc_p :
                ...
                Typing.weakening
                  ...
                  typ_S_enc_T
                  (SMT.Typing.bv_notMem_insert_of_fresh typ_S_enc_T hp_not_bv_S)
              have typ_T_enc_p :
                  ctx.insert p (αx.toSMTType.pair βx.toSMTType) ⊢ˢ T_enc : βx.set.toSMTType :=
                Typing.weakening
                  (h := SMT.TypeContext.entries_subset_insert_of_notMem p_fresh)
                  typ_T_enc
                  (SMT.Typing.bv_notMem_insert_of_fresh typ_T_enc hp_not_bv_T)
              have typ_S_enc_pab :
                ...
                Typing.weakening
                  ...
                  (Typing.weakening
                    ...
                    typ_S_enc_p
                    (SMT.Typing.bv_notMem_insert_of_fresh typ_S_enc_p ha_not_bv_S))
                  (SMT.Typing.bv_notMem_insert_of_fresh
                    (Typing.weakening
                      (h := SMT.TypeContext.entries_subset_insert_of_notMem a_fresh)
                      typ_S_enc_p
                      (SMT.Typing.bv_notMem_insert_of_fresh typ_S_enc_p ha_not_bv_S)) hb_not_bv_S)
              have typ_T_enc_pab :
                  ((ctx.insert p (αx.toSMTType.pair βx.toSMTType)).insert a αx.toSMTType).insert b βx.toSMTType ⊢ˢ
                ...
                Typing.weakening
                  ...
                  (Typing.weakening
                    (h := SMT.TypeContext.entries_subset_insert_of_notMem a_fresh)
                    typ_T_enc_p
                    (SMT.Typing.bv_notMem_insert_of_fresh typ_T_enc_p ha_not_bv_T))
                  (SMT.Typing.bv_notMem_insert_of_fresh
                    (Typing.weakening
                      (h := SMT.TypeContext.entries_subset_insert_of_notMem a_fresh)
                      typ_T_enc_p
                      (SMT.Typing.bv_notMem_insert_of_fresh typ_T_enc_p ha_not_bv_T)) hb_not_bv_T)
              have typ_body :
                  ((ctx.insert p (αx.toSMTType.pair βx.toSMTType)).insert a αx.toSMTType).insert b βx.toSMTType ⊢ˢ
                    body : .bool := by
                dsimp [body]
              ...
              have hsome_tcprod :
                ...
                · exfalso
                  ...
                  calc
                    ...
                    _ = SMTType.bool := by
                      apply SMT.RenamingContext.denote_type_of_typing_fv typ_exists _ (hcov_exists_upd Wx) hDx
                      apply SMT.RenamingContext.respects_update_of_wt hWx_ty Δctx_wt
                      intro w hw hwp
                      have hw_tcprod : w ∈ SMT.fv tcprod := by
                        change w ∈ SMT.fv ((λˢ [p]) _ existsTerm)
                        simp only [SMT.fv, List.mem_removeAll_iff, List.mem_singleton]
                        exact ⟨hw, hwp⟩
                      exact hcov_tcprod w hw_tcprod
                    _ = Dy.snd.fst := by
                      symm
                      apply SMT.RenamingContext.denote_type_of_typing_fv typ_exists _ (hcov_exists_upd Wy) hDy
                      apply SMT.RenamingContext.respects_update_of_wt hWy_ty Δctx_wt
                      intro w hw hwp
                      have hw_tcprod : w ∈ SMT.fv tcprod := by
                        change w ∈ SMT.fv ((λˢ [p]) _ existsTerm)
                        simp only [SMT.fv, List.mem_removeAll_iff, List.mem_singleton]
              ...
              have hden :
                ...
                have hτa : τa = pairTy.fun SMTType.bool := by
                  apply SMT.RenamingContext.denote_type_of_typing_fv typ_tcprod_ctx _ hcov_tcprod hD
                  intro v σ hv hlk
                  obtain ⟨d, hd⟩ := Option.isSome_iff_exists.mp (hcov_tcprod v hv)
                ...
                · simp at hden'
                  ...
                  have hγ_eq_bool : γ = SMTType.bool := by
                    ...
                    calc
                      ...
                      _ = SMTType.bool := by
                        apply SMT.RenamingContext.denote_type_of_typing_fv typ_exists _
                          (hcov_exists_upd Wdefault) hDdefault
                        apply SMT.RenamingContext.respects_update_of_wt hWdefault_ty Δctx_wt
                        intro w hw hwp
                        have hw_tcprod : w ∈ SMT.fv tcprod := by
                          change w ∈ SMT.fv ((λˢ [p]) _ existsTerm)
                          simp only [SMT.fv, List.mem_removeAll_iff, List.mem_singleton]
                  ...
                  have hprod_of_body_true :
                    ...
                    obtain ⟨DSa', Dright, hden_Sa_from_body, hDSa'_true, hden_right, hDright_true⟩ :=
                      denoteAndTrueComponents
                        ...
                        (typ_p_bool := by
                          intro Dp hDp
                          apply SMT.RenamingContext.denote_type_of_typing_fv typ_Sa_term _ hcov_Sa_Δw hDp
                          apply SMT.RenamingContext.respects_update3_of_wt
                            (hd1_ty := rfl) (hd2_ty := rfl) (hd3_ty := rfl)
                            hp_ne_a hp_ne_b hb_ne_a.symm Δctx_wt
                          intro w hw hwp hwa hwb
                          have hw_S : w ∈ SMT.fv S_enc := by
                            simp only [SMT.fv, List.mem_append, List.mem_singleton] at hw
                            exact hw.resolve_right hwa
                          exact hΔ_S_final w hw_S)
                        (typ_q_bool := by
                          intro Dq hDq
                          apply SMT.RenamingContext.denote_type_of_typing_fv typ_right_term _ hcov_right hDq
                          apply SMT.RenamingContext.respects_update3_of_wt
                            (hd1_ty := rfl) (hd2_ty := rfl) (hd3_ty := rfl)
                            hp_ne_a hp_ne_b hb_ne_a.symm Δctx_wt
                          intro w hw hwp hwa hwb
                          -- right_term = T_enc @ b ∧ (var p = var a, var b); fv ⊆ fv(T_enc) ∪ {a, b, p}
                          -- With w ≠ p, w ≠ a, w ≠ b: w ∈ fv(T_enc)
                          simp only [SMT.fv, List.mem_append, List.mem_singleton, List.mem_cons,
                            List.mem_nil_iff, or_false] at hw
                          rcases hw with (hwT | hwb') | hwP | hwAB
                          · exact Δ''_covers_T w hwT
                          · exact absurd hwb' hwb
                          · exact absurd hwP hwp
                    ...
                    obtain ⟨DTb', Deq, hden_Tb_from_body, hDTb'_true, hden_eq, hDeq_true⟩ :=
                      denoteAndTrueComponents
                        ...
                        (typ_p_bool := by
                          intro Dp hDp
                          apply SMT.RenamingContext.denote_type_of_typing_fv typ_Tb_term _ hcov_Tb_Δw hDp
                          apply SMT.RenamingContext.respects_update3_of_wt
                            (hd1_ty := rfl) (hd2_ty := rfl) (hd3_ty := rfl)
                            hp_ne_a hp_ne_b hb_ne_a.symm Δctx_wt
                          intro w hw hwp hwa hwb
                          have hw_T : w ∈ SMT.fv T_enc := by
                            simp only [SMT.fv, List.mem_append, List.mem_singleton] at hw
                            exact hw.resolve_right hwb
                          exact Δ''_covers_T w hw_T)
                        (typ_q_bool := by
                          intro Dq hDq
                          apply SMT.RenamingContext.denote_type_of_typing_fv typ_eq_term _ hcov_eq hDq
                          apply SMT.RenamingContext.respects_update3_of_wt
                            (hd1_ty := rfl) (hd2_ty := rfl) (hd3_ty := rfl)
                            hp_ne_a hp_ne_b hb_ne_a.symm Δctx_wt
                          intro w hw hwp hwa hwb
                          -- eq_term = var p = var a, var b; fv ⊆ {p, a, b}
                          simp only [SMT.fv, List.mem_append, List.mem_singleton, List.mem_cons,
                            List.mem_nil_iff, or_false] at hw
                          rcases hw with hwP | hwAB
                          · exact absurd hwP hwp
                          · rcases hwAB with hwa' | hwb'
                            · exact absurd hwa' hwa
                            · exact absurd hwb' hwb)
...
theorem encodeTerm_spec.cprod_case.{u} (fv_sub_typings : B.FvSubTypings) (S T_1 : B.Term)
                ...
                ∀ {T : ZFSet.{u}} {hT : T ∈ ⟦α⟧ᶻ},
                  ⟦T_1.abstract «Δ» Δ_fv⟧ᴮ = some ⟨T, ⟨α, hT⟩⟩ →
                    (∀ v ∈ T_1.vars, v ∈ used) →
                      ...
                      ((B.bv T_1).Nodup) →
                        B.RenamingContext.RespectsTypeContextOnFV (B.RenamingContext.toSMT «Δ») Λ T_1 →
                        (∀ v ∈ B.fv T_1, v ∈ Λ) →
                        B.RenWF E.context «Δ» →
                        ∀ {n : ℕ},
                          ⦃fun x =>
                                      ...
                                      Λ ⊆ Γ' ∧
                                        AList.keys Γ' ⊆ E'.usedVars ∧
                                          CoversUsedVars E'.usedVars T_1 ∧
                                            σ = α.toSMTType ∧
                                              Γ' ⊢ˢ t' : σ ∧
                                                (∀ v ∈ used, v ∉ Λ → v ∉ B.Term.vars T_1 → v ∉ Γ') ∧
                                                  ∃ Δ',
                                                    ∃ (Δ'_covers : RenamingContext.CoversFV Δ' t'),
                                                      RenamingContext.Extends Δ' Δ₀ ∧
                                                        RenamingContext.ExtendsOnSourceFV Δ' «Δ» T_1 ∧
  ...
  (Λ_inv : ∀ v ∈ (S ⨯ᴮ T_1).vars, v ∈ Λ → v ∈ E.context)
  (bv_nodup : (B.bv (S ⨯ᴮ T_1)).Nodup)
  (respects : B.RenamingContext.RespectsTypeContextOnFV (B.RenamingContext.toSMT «Δ») Λ ((S ⨯ᴮ T_1)))
  (fv_in_Λ : ∀ v ∈ B.fv ((S ⨯ᴮ T_1)), v ∈ Λ)
  (wf : B.RenWF E.context «Δ» := by assumption)
  {n : ℕ} :
  ⦃fun x =>
    match x with
              ...
              Λ ⊆ Γ' ∧
                AList.keys Γ' ⊆ E'.usedVars ∧
                  CoversUsedVars E'.usedVars (S ⨯ᴮ T_1) ∧
                    σ = α.toSMTType ∧
                      Γ' ⊢ˢ t' : σ ∧
                        (∀ v ∈ used, v ∉ Λ → v ∉ B.Term.vars (S ⨯ᴮ T_1) → v ∉ Γ') ∧
                          ∃ Δ',
                            ∃ (Δ'_covers : RenamingContext.CoversFV Δ' t'),
                              RenamingContext.Extends Δ' Δ₀ ∧
  ...
  rw [B.Term.abstract, B.denote, Option.pure_def, Option.bind_eq_bind, Option.bind_eq_some_iff] at den_t
  obtain ⟨⟨X, τX, hX⟩, den_S, eq⟩ := den_t
  -- `B.denote` of `⨯ᴮ'` pattern-matches both operands against `.set _`
  -- (`| failure` otherwise) and returns `.set (α ×ᴮ β)`; matching this against
  -- the known result type pins `τX`/`τY` — no `of_abstract` needed.
  cases τX <;> first | rw [Option.bind_eq_some_iff] at eq | exact absurd eq (by simp)
  rename_i τX
  obtain ⟨⟨Y, τY, hY⟩, den_T, eq⟩ := eq
  cases τY <;> first | rw [Option.some_inj] at eq | exact absurd eq (by simp)
  rename_i τY
  injection eq with T_eq hτeq
  subst T_eq
  simp only [heq_eq_eq, PSigma.mk.injEq, BType.set.injEq, BType.prod.injEq] at hτeq
  obtain ⟨⟨hτXeq, hτYeq⟩, _⟩ := hτeq
  subst τX
  subst τY
  ...
  obtain ⟨⟨⟨St_used_sub_StS, St_eq_StS, StS_sub, S_cov_StS, rfl, typ_S_enc, S_preserves,
    Δ', Δ'_covers_S, Δ'_extends_Δ₀, Δ'_ext_S, Δ'_none_out,
    ⟨Senc, _, hSenc⟩, den_S_enc, ⟨rfl, retract_Senc_eq_X⟩, S_ih_total⟩,
    bv_S_enc_used, _⟩,
    bv_S_enc_notMem_used, _⟩ := pre
  ...
  rw [BType.toSMTType]
  mspec (Std.Do.Triple.and _
    (T_ih (E := E) (Λ := StS.types) (α := .set βx) typ_T
      («Δ» := «Δ») Δ_fv_T
      ...
      (fun v hv hΛ => by
        ...
        · have hv_vars_S : v ∈ B.Term.vars S := by
          ...
          rcases B.Term.mem_vars_iff.mp hv_vars_S with h | h
          · exact _root_.B.Typing.typed_by_fv typ_S h
          · rcases B.Term.mem_vars_iff.mp hv with hy_fv | hy_bv
            · exact absurd (_root_.B.Typing.typed_by_fv typ_T hy_fv)
                (_root_.B.Typing.bv_notMem_context typ_S v h)
            · exact absurd rfl (hST_bv_disj v h v hy_bv))
      hT_bv_nodup
      (respects.transport_fv (fun v hv => by rw [B.fv, List.mem_append]; exact Or.inr hv) St_eq_StS (fun v hv => fv_in_Λ v (by rw [B.fv, List.mem_append]; exact Or.inr hv)))
      (fun v hv => AList.mem_of_subset St_eq_StS (fv_in_Λ v (by rw [B.fv, List.mem_append]; exact Or.inr hv)))
      wf
      (n := StS.env.freshvarsc))
    (encodeTerm_bv_used E (t := T_1) (used := StS.env.usedVars)
  ...
  obtain ⟨⟨StS_used_sub_StT, StS_eq_StT, StT_sub, T_cov_StT, rfl, typ_T_enc, T_preserves,
    ...
    ⟨Tenc, _, hTenc⟩, den_T_enc, ⟨rfl, retract_Tenc_eq_Y⟩, T_ih_total⟩,
    bv_T_enc_used, _⟩ := pre

  have typ_S_enc_T : StT.types ⊢ˢ S_enc : αx.set.toSMTType :=
    Typing.weakening StS_eq_StT typ_S_enc
      (fun v hv => T_preserves v
        (bv_S_enc_used v hv)
        (SMT.Typing.bv_notMem_context typ_S_enc v hv)
        (fun hvars => bv_S_enc_notMem_used v hv (St_used_eq ▸ vars_used v (by
          simp only [B.Term.vars, List.mem_union_iff, B.fv, B.bv, List.mem_append] at hvars ⊢
          rcases hvars with h | h <;> [left; right] <;> exact .inr h))))
  ...
  case post.success p =>
    ...
    case post.success a =>
      ...
      case post.success b =>
        ...
        -- The encoder now erases the leaked λ/∃ binders `p`, `a`, `b` from the
        -- global type context before returning, so the result stays typeable
        -- there (the binders re-introduce them locally).
        ...
        mintro ∀StEp
        mpure preEp
        obtain ⟨StEp_types_eq, StEp_fvc, StEp_used_eq⟩ := preEp
        mspec SMT.eraseFromContext_spec
          (v := a) (Γ := StEp.types)
        ...
        mintro ∀StEa
        mpure preEa
        obtain ⟨StEa_types_eq, StEa_fvc, StEa_used_eq⟩ := preEa
        mspec SMT.eraseFromContext_spec
          (v := b) (Γ := StEa.types)
        ...
        mintro ∀StEb
        mpure preEb
        obtain ⟨StEb_types_eq, StEb_fvc, StEb_used_eq⟩ := preEb

        -- The final type context `StEb.types` is `ctx` with the (fresh) λ/∃ binders
        -- `p`, `a`, `b` erased. Hoist the bookkeeping facts that relate it to `ctx`.
        have a_notctx : a ∉ ctx := fun h =>
          a_fresh ((AList.mem_insert _).mpr (Or.inr h))
        have b_notctx : b ∉ ctx := fun h =>
          b_fresh ((AList.mem_insert _).mpr (Or.inr ((AList.mem_insert _).mpr (Or.inr h))))
        have StEb_types_eq' :
            StEb.types = AList.erase b (AList.erase a (AList.erase p St₃.types)) := by
          rw [StEb_types_eq, StEa_types_eq, StEp_types_eq]
        -- `ctx ⊆ StEb.types` (entries) — needed to lift typings/lookups into the result.
        have ctx_sub_StEb : ctx.entries ⊆ StEb.types.entries := by
          have base : ctx.entries ⊆ St₃.types.entries := by
            rw [St₃_types_eq]
          ...
          exact SMT.TypeContext.entries_subset_erase_of_notMem
            (SMT.TypeContext.entries_subset_erase_of_notMem
              (SMT.TypeContext.entries_subset_erase_of_notMem base p_fresh) a_notctx) b_notctx
        -- Any key of `StEb.types` lies in `ctx` (the only extra keys `p`/`a`/`b`
        -- have been erased), and there its lookup agrees with `ctx`.
        have StEb_mem_ctx : ∀ {v : SMT.𝒱}, v ∈ StEb.types → v ∈ ctx := by
          intro v hvErase
          have hv_ne_b : v ≠ b := (AList.mem_erase.mp (StEb_types_eq' ▸ hvErase)).1
          have hv_ne_a : v ≠ a :=
            (AList.mem_erase.mp (AList.mem_erase.mp (StEb_types_eq' ▸ hvErase)).2).1
          have hv_ne_p : v ≠ p :=
            (AList.mem_erase.mp
              (AList.mem_erase.mp (AList.mem_erase.mp (StEb_types_eq' ▸ hvErase)).2).2).1
          have hv_St₃ : v ∈ St₃.types :=
            AList.mem_keys.mpr (SMT.TypeContext.keys_erase_subset
              (SMT.TypeContext.keys_erase_subset
                (SMT.TypeContext.keys_erase_subset
                  (AList.mem_keys.mp (StEb_types_eq' ▸ hvErase)))))
          rw [St₃_types_eq] at hv_St₃
          iterate 3 rw [AList.mem_insert] at hv_St₃
          rcases hv_St₃ with rfl | rfl | rfl | hv_St₃
          · exact absurd rfl hv_ne_b
          · exact absurd rfl hv_ne_a
          · exact absurd rfl hv_ne_p
          · exact hv_St₃
        -- Lookup in `StEb.types` of a non-binder key reduces to lookup in `ctx`.
        have StEb_lookup_ctx : ∀ {v : SMT.𝒱} {τ : SMTType},
            AList.lookup v StEb.types = some τ → AList.lookup v ctx = some τ := by
          intro v τ hτ
          have hv_mem : v ∈ StEb.types := AList.lookup_isSome.mp (by rw [hτ]; rfl)
          have hv_ne_b : v ≠ b := (AList.mem_erase.mp (StEb_types_eq' ▸ hv_mem)).1
          have hv_ne_a : v ≠ a :=
            (AList.mem_erase.mp (AList.mem_erase.mp (StEb_types_eq' ▸ hv_mem)).2).1
          have hv_ne_p : v ≠ p :=
            (AList.mem_erase.mp
              (AList.mem_erase.mp (AList.mem_erase.mp (StEb_types_eq' ▸ hv_mem)).2).2).1
          rw [StEb_types_eq', AList.lookup_erase_ne hv_ne_b, AList.lookup_erase_ne hv_ne_a,
            AList.lookup_erase_ne hv_ne_p, St₃_types_eq, AList.lookup_insert_ne hv_ne_b,
            AList.lookup_insert_ne hv_ne_a, AList.lookup_insert_ne hv_ne_p] at hτ
          exact hτ
        ...
        · intro v hv
          exact ctx_sub_StEb (StS_eq_StT (St_eq_StS hv))
        · intro v hv
          rw [StEb_used_eq, StEa_used_eq, StEp_used_eq, St₃_used_eq, St₂_used_eq, St₁_used_eq]
        ...
        · have hp_ne_a : p ≠ a := by
          ...
          have hb_not_ctxpa :
            ...
            exact b_fresh
          -- bound vars of the encoded operands are all in `StT.usedVars`, so they
          -- avoid the freshly-created `p`, `a`, `b` (each `∉ usedVars` at creation).
          have bv_S_enc_StT : ∀ v ∈ SMT.bv S_enc, v ∈ StT.env.usedVars :=
            fun v hv => StS_used_sub_StT (bv_S_enc_used v hv)
          have hp_not_bv_S : p ∉ SMT.bv S_enc := fun hp => p_not_used (bv_S_enc_StT p hp)
          have hp_not_bv_T : p ∉ SMT.bv T_enc := fun hp => p_not_used (bv_T_enc_used p hp)
          have ha_not_bv_S : a ∉ SMT.bv S_enc := fun ha =>
            a_not_used (St₁_used_eq ▸ List.mem_cons_of_mem _ (bv_S_enc_StT a ha))
          have ha_not_bv_T : a ∉ SMT.bv T_enc := fun ha =>
            a_not_used (St₁_used_eq ▸ List.mem_cons_of_mem _ (bv_T_enc_used a ha))
          have hb_not_bv_S : b ∉ SMT.bv S_enc := fun hb =>
            b_not_used (St₂_used_eq ▸ List.mem_cons_of_mem _
              (St₁_used_eq ▸ List.mem_cons_of_mem _ (bv_S_enc_StT b hb)))
          have hb_not_bv_T : b ∉ SMT.bv T_enc := fun hb =>
            b_not_used (St₂_used_eq ▸ List.mem_cons_of_mem _
              (St₁_used_eq ▸ List.mem_cons_of_mem _ (bv_T_enc_used b hb)))
          have typ_S_enc_p :
              ctx.insert p (αx.toSMTType.pair βx.toSMTType) ⊢ˢ S_enc : αx.set.toSMTType :=
            Typing.weakening
              (h := SMT.TypeContext.entries_subset_insert_of_notMem p_fresh)
          ...
          have typ_tcprod_ctx :
            ...
            · have hupdate_p :
              ...
              rw [hupdate_p]
              exact typ_exists
          -- The leaked λ/∃ binders `p`, `a`, `b` have been erased from the global
          -- context, so we weaken `typ_tcprod_ctx` (typed in `ctx`) into the
          -- triple-erased final context — a sound direction since `ctx ⊆` it. The
          -- weakening's `hbv` premise holds because `bv tcprod` avoids `ctx` (by
          -- `bv_notMem_context`) and every key of `StEb.types` lies in `ctx`.
          exact Typing.weakening ctx_sub_StEb typ_tcprod_ctx
            (fun v hv hvErase =>
              SMT.Typing.bv_notMem_context typ_tcprod_ctx v hv (StEb_mem_ctx hvErase))
        · -- preserves_types
          intro v hv h1 h2
          obtain ⟨h2_S, h2_T⟩ := B.Term.notMem_vars_cprod.mp h2
          have hv_StS : v ∈ StS.env.usedVars := St_used_sub_StS (by simpa [St_used_eq] using hv)
          have hv_not_StS : v ∉ StS.types :=
            S_preserves v (by simpa [St_used_eq] using hv) h1 h2_S
          have hv_not_StT : v ∉ StT.types :=
            T_preserves v hv_StS hv_not_StS h2_T
          intro hv_in
          -- `StEb.types`'s keys are exactly `ctx`'s keys (binders erased), so the
          -- only surviving case is `v ∈ ctx = StT.types`.
          exact hv_not_StT (StEb_mem_ctx hv_in)
        · have hcov_tcprod : RenamingContext.CoversFV Δ'' tcprod := by
            intro v hv
            dsimp [tcprod] at hv
            rw [SMT.fv, List.mem_removeAll_iff] at hv
          ...
          · intro v hv
            exact Δ''_none_out v (by
              simp only [StEb_used_eq, StEa_used_eq, StEp_used_eq, St₃_used_eq, St₂_used_eq,
                St₁_used_eq, List.mem_cons, not_or] at hv
              exact hv.2.2.2)
          · have bv_S_enc_StT : ∀ v ∈ SMT.bv S_enc, v ∈ StT.env.usedVars :=
              fun v hv => StS_used_sub_StT (bv_S_enc_used v hv)
            have hp_not_bv_S : p ∉ SMT.bv S_enc := fun hp => p_not_used (bv_S_enc_StT p hp)
            ...
            obtain ⟨denT_main, hden_main, hRDom_main⟩ :=
              cprod_case_denotation_aux
                ...
                (Δctx_wt := SMT.RenamingContext.ExtendsOnSourceFV.wt Δ''_ext_T typ_T_enc)
                (p := p) (a := a) (b := b)
                (p_fresh := p_fresh) (a_fresh := a_fresh) (b_fresh := b_fresh)
                (hp_not_bv_S := hp_not_bv_S) (hp_not_bv_T := hp_not_bv_T)
                (ha_not_bv_S := ha_not_bv_S) (ha_not_bv_T := ha_not_bv_T)
                (hb_not_bv_S := hb_not_bv_S) (hb_not_bv_T := hb_not_bv_T)
            ...
            -- `B.denote` of `⨯ᴮ'` forces both operand tags to `.set _` and
            -- returns `.set (α ×ᴮ β)`; matching the known result type pins
            -- `τX_alt`/`τY_alt` — no `of_abstract` needed.
            cases τX_alt <;> first | rw [Option.bind_eq_some_iff] at eq_alt | exact absurd eq_alt (by simp)
            rename_i τX_alt
            obtain ⟨⟨Y_alt, τY_alt, hY_alt⟩, den_T_alt, eq_alt⟩ := eq_alt
            cases τY_alt <;> first | rw [Option.some_inj] at eq_alt | exact absurd eq_alt (by simp)
            rename_i τY_alt
            injection eq_alt with T_alt_eq hτeq_alt
            subst T_alt_eq
            simp only [heq_eq_eq, PSigma.mk.injEq, BType.set.injEq, BType.prod.injEq] at hτeq_alt
            obtain ⟨⟨hτXeq, hτYeq⟩, _⟩ := hτeq_alt
            subst τX_alt
            subst τY_alt
            ...
            have Δ₀_alt_S_wt : ∀ v (d : SMT.Dom), Δ₀_alt_S v = some d → ∀ τ, StS.types.lookup v = some τ → d.snd.fst = τ := by
              intro v d hv τ hτ; simp only [Δ₀_alt_S_def] at hv; split_ifs at hv with h; exact Δ₀_alt_wt v d hv τ (AList.lookup_of_subset ctx_sub_StEb (AList.lookup_of_subset StS_eq_StT hτ))
            -- Call S_ih_total
            obtain ⟨Δ'_alt_S, hcov_alt_S, denT_S_alt, hext_alt_S,
              ...
              S_ih_total Δ_alt
                ...
                Δ₀_alt_S Δ₀_alt_S_ext wf_alt Δ₀_alt_S_none Δ₀_alt_S_wt X_alt hX_alt den_S_alt
            ...
            have Δ₀_alt_T_wt : ∀ v (d : SMT.Dom), Δ₀_alt_T v = some d → ∀ τ, StT.types.lookup v = some τ → d.snd.fst = τ := by
              ...
              have hv_used : v ∈ StT.env.usedVars := by
                by_contra h; rw [if_neg h] at hv; nomatch hv
              rw [if_pos hv_used] at hv
              cases hΔ : Δ₀_alt v with
              | some d' =>
                simp [hΔ] at hv; subst hv
                exact Δ₀_alt_wt v d' hΔ τ (AList.lookup_of_subset ctx_sub_StEb hτ)
              | none =>
                simp [hΔ] at hv
            ...
            obtain ⟨Δ'_alt_T, hcov_alt_T, denT_T_alt, hext_alt_T,
              ...
              T_ih_total Δ_alt
                ...
                Δ₀_alt_T Δ₀_alt_T_ext wf_alt Δ₀_alt_T_none Δ₀_alt_T_wt Y_alt hY_alt den_T_alt
            ...
            obtain ⟨denT_cprod_alt, hden_cprod_alt, hRDom_cprod_alt⟩ :=
              cprod_case_denotation_aux
                ...
                (Δctx_wt := fun v d hv τ hτ => by
                  ...
                  match h : Δ₀_alt v with
                  | some d' =>
                    rw [h] at hv
                    have hd_eq : some d' = some d := hv
                    injection hd_eq with hd'
                    exact hd' ▸ Δ₀_alt_wt v d' h τ (AList.lookup_of_subset ctx_sub_StEb hτ)
                  | none =>
                    rw [h] at hv
                    exact Δ'_alt_T_wt_out v d hv τ hτ)
                (p := p) (a := a) (b := b)
                (p_fresh := p_fresh) (a_fresh := a_fresh) (b_fresh := b_fresh)
                (hp_not_bv_S := hp_not_bv_S) (hp_not_bv_T := hp_not_bv_T)
                (ha_not_bv_S := ha_not_bv_S) (ha_not_bv_T := ha_not_bv_T)
            ...
            · intro v hv; simp only [Δ'_alt_def]
              ...
              intro hmem; exact hv (by rw [StEb_used_eq, StEa_used_eq, StEp_used_eq, St₃_used_eq, St₂_used_eq, St₁_used_eq]; exact .tail _ (.tail _ (.tail _ hmem)))
            -- Well-typedness: output wt
            · intro v d hv τ hτ
              simp only [Δ'_alt_def] at hv
              cases hΔ : Δ₀_alt v with
              ...
              | none =>
                ...
                -- `hτ : lookup v StEb.types = some τ`; reduce to `lookup v ctx`.
                exact StEb_lookup_ctx hτ
            -- dom_out
            · intro v hv
              simp only [Δ'_alt_def] at hv
              cases hΔ : Δ₀_alt v with
              | some d =>
                exact fv_sub_typings (_root_.B.Typing.cprod typ_S typ_T)
                  (SMT.Typing.bool StEb.types true) v
                  (SMT.RenamingContext.ExtendsOnSourceFV.dom_sub_B_fv Δ₀_alt_ext v
                    (by rw [hΔ]; simp))
              | none =>
                simp [hΔ] at hv
                have h_StT := Δ'_alt_T_dom_out v hv
                -- `h_StT : v ∈ ctx`; lift into the (erased) final context.
                exact SMT.TypeContext.mem_of_entries_subset h_StT ctx_sub_StEb
