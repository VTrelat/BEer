...
theorem funs.nonempty {A B : ZFSet} (hB : B ≠ ∅) : ZFSet.funs A B ≠ ∅ := by
  ...
  letI f := (A.prod B).sep λ z => ∃ x ∈ A, z = x.pair b
...
theorem is_pfunc_func_exists {f A B : ZFSet} : f.IsPFunc A B → ∃ A' B', IsFunc A' B' f ∧ A' ⊆ A ∧ B' ⊆ B := by classical
  ...
  · let A' := A.sep (λ x => ∃ y, pair x y ∈ f)
    let B' := B.sep (λ y => ∃ x ∈ A', pair x y ∈ f)
    exists A', B'
    and_intros
    · intro z hz
    ...
    · intro x x_A
      ...
      have y_B : y ∈ B := by
        ...
        assumption
      exact ⟨y, pair, λ z hz => func x z hz y pair⟩
...
def IsMono {f A B : ZFSet} [LTA : Preorder {x // x ∈ A}] [LTB : Preorder {x // x ∈ B}] (_ : IsFunc A B f) :=
  ∀ x₁ x₂ y₁ y₂,
    (x₁_A : x₁ ∈ A) →
    (y₁_B :y₁ ∈ B) →
    x₁.pair y₁ ∈ f →
...
def IsStrictMono {f A B : ZFSet} [LTA : Preorder {x // x ∈ A}] [LTB : Preorder {x // x ∈ B}] (_ : IsFunc A B f) :=
  ∀ x₁ x₂ y₁ y₂,
    (x₁_A : x₁ ∈ A) →
    (y₁_B :y₁ ∈ B) →
    x₁.pair y₁ ∈ f →
...
theorem Id.IsBijective {A : ZFSet} : (𝟙A).IsBijective Id.IsFunc := by
  ...
  · intro x y z xA yA zA x_pair_y y_pair_z
    ...
    rfl
  · intro y yA
    simp_rw [Id, mem_sep, pair_mem_prod, pair_inj, exists_eq_right_right', existsAndEq, and_self, yA, and_true]
...
theorem IsInjective.composition_of_injective {f g : ZFSet} {A B C : ZFSet} {hf : A.IsFunc B f} {hg : B.IsFunc C g} (finj : f.IsInjective) (ginj : g.IsInjective) :
...
theorem IsSurjective.composition_of_surjective {f g : ZFSet} {A B C : ZFSet} {hf : A.IsFunc B f} {hg : B.IsFunc C g}
...
theorem IsBijective.composition_of_bijective {f g : ZFSet} {A B C : ZFSet} {hf : A.IsFunc B f} {hg : B.IsFunc C g} (fbij : f.IsBijective) (gbij : g.IsBijective) :
    (g ∘ᶻ f).IsBijective :=
  ⟨IsInjective.composition_of_injective fbij.1 gbij.1,
    IsSurjective.composition_of_surjective fbij.2 gbij.2⟩

theorem IsPermutation.composition_of_permutations {σ τ : ZFSet} {E : ZFSet} (hσ : σ.IsPermutation E) (hτ : τ.IsPermutation E) : (composition τ σ E E E).IsPermutation E := ⟨
...
theorem fapply_mem_range {f A B : ZFSet} (hf : f.IsPFunc A B) {x : ZFSet} (hx : x ∈ f.Dom) : (@ᶻf ⟨x, hx⟩).val ∈ B := by
...
theorem IsInjective.apply_inj {f A B : ZFSet} (hf : IsFunc A B f) (inj : f.IsInjective) : Function.Injective @ᶻf := by classical
  ...
  have choose_eq {z} (z_dom : z ∈ f.Dom) :
    Classical.choose (hf.right z (is_func_dom_eq hf ▸ z_dom)) = Classical.choose (fapply._proof_1 f (is_func_is_pfunc hf) z z_dom) := by
    congr
    funext w
    rw [propext_iff]
    constructor
...
theorem IsPFunc.mem_range_of_mem {f A B : ZFSet} (hf : f.IsPFunc A B) {x y : ZFSet} (hxy : x.pair y ∈ f) :
    y ∈ f.Range := by
  rw [mem_sep]
  and_intros
...
theorem IsPFunc.nonempty_range_of_nonempty_dom {f A B x y : ZFSet} (hf : f.IsPFunc A B) (hxy : x.pair y ∈ f) :
    f.Range ≠ ∅ := by
...
theorem IsInjective.apply_inj_pfun {f A B : ZFSet} (hf : IsPFunc f A B) (inj : f.IsInjective (is_func_of_pfunc f hf)) : Function.Injective @ᶻf := by
  rintro ⟨x, x_dom⟩ ⟨y, y_dom⟩ h
  congr
...
theorem IsInjective.apply_surj {f A B : ZFSet} (hf : IsFunc A B f) (surj : f.IsSurjective) : Function.Surjective @ᶻf := by
  rintro ⟨y, yB⟩
...
theorem IsInjective.apply_surj_pfun {f A B : ZFSet} (hf : IsPFunc f A B) (surj : f.IsSurjective (is_func_of_pfunc f hf)) : Function.Surjective @ᶻf := by
  rintro ⟨y, yB⟩
  obtain ⟨x, -, pair⟩ := surj y yB
...
theorem prod_sep_is_pfunc_mem {A B C D : ZFSet} (subAC : A ⊆ C) (subBD : B ⊆ D) : (A.prod B).powerset.sep (IsPFunc · A B) ∈ (C.prod D).powerset.powerset := by
  rw [mem_powerset]
  intro S hS
  rw [mem_sep] at hS
...
section
  ...
  def funZType : Parser :=
    ":" >> ppSpace >> termParser leadPrec >> ppSpace >> unicodeSymbol "→" "->" >> ppSpace >> termParser leadPrec
  ...
  partial def interpZBinder (x_def : TSyntax `term) (e : TSyntax `term) : TSyntax `funz_binder → MacroM (TSyntax `term)
  ...
  | `(term| λᶻ : $dom → $ran | $x ↦ $e) => do
    `(term| ZFSet.lambda $dom $ran λ a ↦ $(← interpZBinder (←`(term| a)) e x))
end
...
theorem mem_lambda {dom ran : ZFSet} {exp : ZFSet → ZFSet} {z : ZFSet} :
    (z ∈ λᶻ : dom → ran | x ↦ exp x) ↔ ∃ x y : ZFSet, z = x.pair y ∧ x ∈ dom ∧ y ∈ ran ∧ y = exp x where
  mp := by
    intro hz
...
theorem lambda_isFunc {A B : ZFSet} {f : ZFSet → ZFSet} (hf : ∀ {x}, x ∈ A → f x ∈ B) : A.IsFunc B (lambda A B f) := by
  and_intros
  · exact lambda_subset
  · intro x x_A
    exists f x
...
theorem fapply_lambda {A B : ZFSet} {f : ZFSet → ZFSet} (hf : ∀ {x}, x ∈ A → f x ∈ B) {a : ZFSet} (ha : a ∈ A) :
    fapply (λᶻ : A → B | x ↦ f x) (is_func_is_pfunc <| lambda_isFunc hf) ⟨a, by rwa [is_func_dom_eq (lambda_isFunc hf)]⟩ = f a := by
  rw [fapply]
...
-/
theorem inv_bijective_of_bijective {f A B : ZFSet} {f_is_func : A.IsFunc B f} (hf : f.IsBijective f_is_func) :
    f⁻¹.IsBijective := by
  and_intros
...
theorem composition_self_inv_of_bijective {f A B : ZFSet} {f_is_func : A.IsFunc B f} (hf : f.IsBijective) :
    f⁻¹ ∘ᶻ f = 𝟙A := by
  ext1 z
  constructor
  · intro hz
...
theorem composition_inv_self_of_bijective {f A B : ZFSet} {f_is_func : A.IsFunc B f} (hf : f.IsBijective) :
    (f ∘ᶻ f⁻¹) = 𝟙B := by
  set g := f⁻¹
...
theorem Image_of_singleton_pair_mem_iff {A B : ZFSet} {f : ZFSet} (hf : A.IsFunc B f) {a b : ZFSet} :
    a.pair b ∈ f ↔ f[{a}] = {b} := by
  constructor <;> intro h
  · ext1 z
...
theorem eq_singleton_of_bijective_inv_Image_of_singleton {A B : ZFSet} {f : ZFSet} {hf : A.IsFunc B f}
  (hbij : f.IsBijective) {b : ZFSet} (hb : b ∈ B) : ∃ a ∈ A, f⁻¹[{b}] = {a} := by
  obtain ⟨a, aA, fab⟩ := hbij.2 b hb
  use a, aA
  rwa [←Image_of_singleton_pair_mem_iff (inv_is_func_of_bijective hbij), mem_inv]

theorem Image_singleton_eq_fapply {A B : ZFSet} {f : ZFSet} (hf : A.IsFunc B f) {a : ZFSet} (ha : a ∈ A) :
    f[{a}] = { (@ᶻf ⟨a, by rwa [is_func_dom_eq hf]⟩).val } := by
  rw [←Image_of_singleton_pair_mem_iff hf]
  apply fapply.def

theorem fapply_eq_Image_singleton {A B : ZFSet} {f : ZFSet} (hf : A.IsFunc B f) {a : ZFSet} (ha : a ∈ A) :
    @ᶻf ⟨a, by rwa [is_func_dom_eq hf]⟩ = ⋂₀ (f[{a}]) := by
  rw [Image_singleton_eq_fapply hf ha, sInter_singleton]

theorem fapply_composition {g f : ZFSet} {A B C : ZFSet} (hg : B.IsFunc C g) (hf : A.IsFunc B f) {x : ZFSet} (xA : x ∈ A) :
...
-/
def IsFinite (x : ZFSet) := ∃ (n f : ZFSet) (_ : n ∈ Nat) (hf : f ∈ x.funs n), f.IsInjective (mem_funs.mp hf)

abbrev ZFFinSet := {x : ZFSet // x.IsFinite}

noncomputable def Max (S : ZFSet) [linord : LinearOrder {x // x ∈ S}] : ZFSet :=
  ε (S.sep λ x => (_ : x ∈ S) → ∀ y, (_ : y ∈ S) → linord.le ⟨y, ‹_›⟩ ⟨x, ‹_›⟩)
noncomputable def Min (S : ZFSet) [linord : LinearOrder {x // x ∈ S}] : ZFSet :=
  ε (S.sep λ x => (_ : x ∈ S) → ∀ y, (_ : y ∈ S) → linord.le ⟨x, ‹_›⟩ ⟨y, ‹_›⟩)

def LinearOrder.ofSubset {S T : ZFSet} (S_T : S ⊆ T) [linordT : LinearOrder {x // x ∈ T}] : LinearOrder {x // x ∈ S} :=
  LinearOrder.lift' (fun ⟨x, hx⟩ => (⟨x, S_T hx⟩:{x // x ∈ T})) (by rintro ⟨x, hx⟩ ⟨y, hy⟩ _; injections; congr)

example {x : ZFSet} {hx : x ∈ Nat} :
  @ZFSet.Max ({x} : ZFSet) (@LinearOrder.ofSubset ({x} : ZFSet) Nat (by intro; rw [mem_singleton]; rintro rfl; exact hx) ZFNat.instLinearOrder) = x := by
  unfold Max
  simp_rw [mem_singleton, mem_sep, mem_singleton]
  have : ∃ z, z = x ∧ ∀ (x_1 : z = x) (y : ZFSet) (x_2 : y = x), (⟨y, by rwa [x_2]⟩ : ZFNat) ≤ ⟨z, by rwa [x_1]⟩ := by
    exists x
    and_intros
    · rfl
    · rintro _ y rfl
...
theorem Min_mem (S : ZFFinSet) [linord : LinearOrder {x // x ∈ S.val}] (nempS : S.val.Nonempty) : S.val.Min ∈ S.val := by
  admit
...
theorem Max_mem (S : ZFFinSet) [linord : LinearOrder {x // x ∈ S.val}] (nempS : S.val.Nonempty) : S.val.Max ∈ S.val := by
  admit
...
theorem sep_mem_powerset {D T : ZFSet} {P : ZFSet → Prop} : D ∈ T.powerset → D.sep P ∈ T.powerset := by
  intro hD
  rw [mem_powerset, subset_def] at hD ⊢
  exact λ _ hz => hD (ZFSet.mem_sep.mp hz).1

theorem subset_of_𝔹_sInter (B : ZFSet) : B ⊆ ZFSet.𝔹 → (⋂₀ B : ZFSet) ∈ ZFSet.𝔹 := by
  intro h
  simp_rw [← ZFSet.mem_powerset, ZFSet.ZFBool.powerset_𝔹_def, ZFSet.mem_insert_iff, ZFSet.mem_singleton] at h
...
theorem sInter_sep_subset_of_𝔹_mem_𝔹 {D : ZFSet} {P : ZFSet → Prop} : D ⊆ ZFSet.𝔹 → (⋂₀ (D.sep P) : ZFSet) ∈ ZFSet.𝔹 := by
  intro h
...
theorem sUnion_sep_subset_of_𝔹_mem_𝔹 {D : ZFSet} {P : ZFSet → Prop} : D ⊆ ZFSet.𝔹 → (⋃₀ (D.sep P) : ZFSet) ∈ ZFSet.𝔹 := by
  intro h
...
theorem IsFunc.sep_on_eq {A B : ZFSet} {f : ZFSet → ZFSet} (hf : ∀ x ∈ A, f x ∈ B) : ZFSet.IsFunc A B <| (A.prod B).sep (λ z => ∃ x y : ZFSet, z = x.pair y ∧ y = f z.π₁) := by
  unfold IsFunc
  and_intros
  · exact sep_subset_self
  · intro x hx
...
theorem IsPFunc.empty_range_of_empty_dom {f A B : ZFSet}
  (hf : IsPFunc f A B) (dom_emp : f.Dom = ∅) : f.Range = ∅ := by
    ...
    simp only [notMem_empty, and_self, exists_const, sep_empty_iff, not_false_eq_true, implies_true, or_true]

theorem IsPFunc.exists_dom_of_mem_range {f A B : ZFSet} (hf : IsPFunc f A B) {y : ZFSet} (hy : y ∈ f.Range) :
  ∃ x ∈ A, pair x y ∈ f := by
  unfold Range at hy
  rw [mem_sep] at hy
...
theorem bijective_of_injective {f A B : ZFSet} (hf : IsFunc A B f) (inj : f.IsInjective hf) : f.IsBijective (A := A) (B := Range f) (IsFunc.is_func_on_range hf) := by
  constructor
...
theorem IsFunc.range_eq_of_surjective {f A B : ZFSet} (hf : IsFunc A B f) (surj : f.IsSurjective hf) :
  f.Range = B := by
  ext1 y
  constructor
  · intro hy
...
instance instPreorder_mem_Nat {n : ZFSet} (hn : n ∈ Nat) : Preorder {x // x ∈ n} where
  le := fun ⟨a, ha⟩ ⟨b, hb⟩ => (⟨a, ZFNat.mem_Nat_of_mem_mem_Nat hn ha⟩ : ZFNat) ≤ (⟨b, ZFNat.mem_Nat_of_mem_mem_Nat hn hb⟩ : ZFNat)
...
theorem IsFinite.subset {A B : ZFSet} (finB : B.IsFinite) (subAB : A ⊆ B) :
  ...
  · rw [mem_funs] at hf ⊢
    ...
    · intro z hz
      ...
      exact hz.1
    · intro x xA
      simp only [exists_and_left, mem_sep, mem_prod, pair_inj, exists_eq_right_right', exists_eq', and_true]
...
theorem IsFinite.disjoint_union {A B : ZFSet} (finA : A.IsFinite) (finB : B.IsFinite) (disjoint : A ∩ B = ∅):
  (A ∪ B).IsFinite := by
  ...
  · have n₁_ne_zero : n₁ ≠ ∅ := by
    ...
    let f' := fA ∪ (B.prod (⟨n₁, hn₁⟩ + ⟨n₂, hn₂⟩ : ZFNat)).sep fun z => ∃ (x y : ZFSet) (hy : y ∈ Nat), z = x.pair (⟨y, hy⟩ + ⟨n₁, hn₁⟩ : ZFNat) ∧ x.pair y ∈ fB
    ...
    · rw [mem_funs]
      ...
      · intro z hz
        ...
        · obtain ⟨a, z_a_fB, a_unq⟩ := mem_funs.mp hfB |>.2 z hz
          have a_Nat : a ∈ Nat := ZFNat.mem_Nat_of_mem_mem_Nat hn₂ <| And.right <| pair_mem_prod.mp <| mem_funs.mp hfB |>.1 z_a_fB
          ...
          · intro y hy
            ...
            · have zA := And.left <| pair_mem_prod.mp <| mem_funs.mp hfA |>.1 hy
              ...
              simp_rw [mem_inter, notMem_empty, iff_false] at contr
              nomatch contr ⟨zA, hz⟩
            · simp only [exists_and_right, mem_sep, mem_prod, pair_inj, exists_eq_right_right', exists_and_left] at hy
    ...
    · intro x y z xA yA hz xz yz
      ...
      · simp only [exists_and_right, mem_sep, mem_prod, pair_inj, exists_eq_right_right', exists_and_left] at xz
        nomatch contr x ⟨xA, xz.1.1⟩
      · obtain ⟨⟩ := pair_mem_prod.mp <| mem_funs.mp hfA |>.1 xz
        obtain ⟨⟩ := pair_mem_prod.mp <| mem_funs.mp hfA |>.1 yz
        apply injA <;> assumption
      · simp only [exists_and_right, mem_sep, mem_prod, pair_inj, exists_eq_right_right', exists_and_left] at yz
      ...
      · simp only [exists_and_right, mem_sep, mem_prod, pair_inj, exists_eq_right_right', exists_and_left] at xz yz
    ...
    · rw [Range, mem_sep, mem_insert_iff, eq_self, true_or, true_and,] at n_range
      ...
      have S'_is_func : S'.IsFunc n fS' := by
        ...
        · intro z hz
          ...
          · obtain ⟨⟩ := fS_inj a x y (And.left <| pair_mem_prod.mp <| fS_fun.1 an) xS (mem_insert y y) an hz.1
...
theorem IsFinite.exists_bij {A : ZFSet} (finA : A.IsFinite) :
  ...
  | empty =>
    ...
    · simp_rw [mem_funs, IsFunc, prod_empty_left, subset_refl, notMem_empty, existsUnique_false, imp_self, implies_true, and_self]
    · and_intros
      · intro _ _ _ h
  ...
  | insert S x ih x_not_mem_S =>
    injections
    subst_vars
    obtain ⟨n, f, hn, hf, bij⟩ := ih (IsFinite.subset (A := S) finA (fun _ => mem_insert_of_mem x)) rfl
  ...
  · extract_lets ex_bij
    ...
    have : (⟨n, hn⟩ : ZFNat) = 1 := by
      ...
      | succ m IH =>
        ...
        · rw [←@ZFNat.zero_add 1, ←ZFNat.add_one_eq_succ, ZFNat.add_right_cancel, ZFNat.nat_zero_eq, Subtype.ext_iff]
          dsimp
          symm
          apply k_unq
...
def Card.inductionOn {P : ZFFinSet → Prop}
  ...
  (succ : ∀ n : ZFNat, (∀ (S : ZFFinSet), n = Card S → P S) → ∀ (S' : ZFFinSet), Card S' = n + 1 → P S') :
  ∀ (S : ZFFinSet), P S := by
  intro ⟨S, hS⟩
  admit
  ...
  | succ n IH S cardS =>
    ...
    · obtain ⟨s, hs⟩ := (@nonempty_exists_iff S).mp hS
      ...
      have : S.val = Insert.insert s (S.val \ ({s} : ZFSet)) := by
        ext
        simp only [mem_insert_iff, mem_diff, mem_singleton, or_and_left, Classical.em, and_true, iff_or_self]
        rintro rfl
        assumption
      change Card ⟨S.val, S.property⟩ = n + 1 at cardS
      have : S = ⟨Insert.insert s (S.val \ ({s} : ZFSet)), IsFinite.subset finA <| subset_of_subset_of_eq (fun _ => id) this.symm⟩ :=
        Subtype.ext this
      rw [this] at cardS
      dsimp at cardS
...
theorem IsFinite.finite_funs (S T : ZFSet) (finS : S.IsFinite) (finT : T.IsFinite) : (S.funs T).IsFinite :=
  sep (powerset (prod finS finT)) (S.IsFunc T)

theorem IsFinite.exists_bij_mono_iff {S : ZFSet} [inst : Preorder {x // x ∈ S}] :
  S.IsFinite ↔ ∃ (n : ZFSet) (f : ZFSet) (_ : n ∈ Nat) (hf : f ∈ S.funs n), f.IsBijective (mem_funs.mp hf) ∧ @IsStrictMono f S n inst (instPreorder_mem_Nat ‹_ ∈ Nat›) (mem_funs.mp hf) := by
  constructor
  · intro Sfin
...
theorem Min_mem_of_non_empty_finite {S : ZFSet} [inst : LinearOrder {x // x ∈ S}]
  ...
  beta_reduce
  simp only [mem_sep]
  apply epsilon_spec ?_ (p := fun z => z ∈ S ∧ ∀ (x : z ∈ S) (y : ZFSet.{u_1}) (x_1 : y ∈ S), inst.le ⟨z, x⟩ ⟨y, x_1⟩) |>.1
  beta_reduce
  obtain ⟨n, f, hn, hf, bij, mono⟩ := IsFinite.exists_bij_mono_iff (inst := inst.toPreorder).mp S_fin
  have : n ≠ (0 : ZFNat).1 := by
...
theorem fprod_is_func {A B A' B' φ ψ : ZFSet} (hφ : A.IsFunc A' φ) (hψ : B.IsFunc B' ψ) :
  ...
  · intro z hz
    ...
    obtain ⟨x, rfl, ⟨a, ha, b, hb, rfl⟩, eq⟩ := hz
    let φa : ZFSet := @ᶻφ ⟨a, by rwa [is_func_dom_eq hφ]⟩
    let ψb : ZFSet := @ᶻψ ⟨b, by rwa [is_func_dom_eq hψ]⟩
    simp only [mem_prod, pair_inj, exists_eq_right_right', π₁_pair, π₂_pair] at eq
    simp only [mem_prod, pair_inj, exists_eq_right_right', π₁_pair, π₂_pair]
    and_intros
    · exact ha
    · exact hb
    · use φa, fapply_mem_range _ _, ψb, fapply_mem_range _ _
...
theorem fprod_bijective_of_bijective {A B A' B' φ ψ : ZFSet} {hφ : A.IsFunc A' φ} {hψ : B.IsFunc B' ψ}
  ...
  · intro y hy
    ...
    let φ_inv_a' : ZFSet := fapply φ⁻¹ (is_func_is_pfunc <| inv_is_func_of_bijective φ_bij) ⟨a', by rwa [is_func_dom_eq (inv_is_func_of_bijective φ_bij)]⟩
    let ψ_inv_b' : ZFSet := fapply ψ⁻¹ (is_func_is_pfunc <| inv_is_func_of_bijective ψ_bij) ⟨b', by rwa [is_func_dom_eq (inv_is_func_of_bijective ψ_bij)]⟩
    ...
    · simp only [fprod, mem_prod, lambda_spec, pair_inj, exists_eq_right_right', π₁_pair, π₂_pair]
      ...
      · exact ha'
      · exact hb'
      · rw [dite_cond_eq_true (eq_true (by rw [pair_mem_prod]; and_intros <;> apply fapply_mem_range)), pair_inj]
        and_intros
        · rw [←fapply_composition hφ (inv_is_func_of_bijective φ_bij) ha',
            fapply_eq_Image_singleton (IsFunc_of_composition_IsFunc hφ (inv_is_func_of_bijective φ_bij)) ha']
          conv =>
            enter [2, 1, 1]
            change φ ∘ᶻ φ⁻¹
        ...
        · rw [←fapply_composition hψ (inv_is_func_of_bijective ψ_bij) hb',
            fapply_eq_Image_singleton (IsFunc_of_composition_IsFunc hψ (inv_is_func_of_bijective ψ_bij)) hb']
          conv =>
            enter [2, 1, 1]
            change ψ ∘ᶻ ψ⁻¹
...
theorem fapply_fprod {A B C D f g a b : ZFSet} (hf : A.IsFunc C f) (hg : B.IsFunc D g)
  (ha : a ∈ A) (hb : b ∈ B) :
  @ᶻ(fprod f g) ⟨a.pair b, by rw [is_func_dom_eq (fprod_is_func hf hg), pair_mem_prod]; exact ⟨ha, hb⟩⟩ =
    let fa : ZFSet := @ᶻf ⟨a, by rwa [is_func_dom_eq hf]⟩
    let gb : ZFSet := @ᶻg ⟨b, by rwa [is_func_dom_eq hg]⟩
    fa.pair gb := by
  conv =>
    ...
    rw [
      ←fapply_eq_Image_singleton
        (lambda_isFunc (fun h ↦ by rw [dite_cond_eq_true (eq_true h), pair_mem_prod]; and_intros <;> apply fapply_mem_range))
        (by rw [pair_mem_prod]; exact ⟨ha, hb⟩),
      fapply_lambda (fun h ↦ by rw [dite_cond_eq_true (eq_true h), pair_mem_prod]; and_intros <;> apply fapply_mem_range) (by rw [pair_mem_prod]; exact ⟨ha, hb⟩),
      dite_cond_eq_true (eq_true (by rw [pair_mem_prod]; exact ⟨ha, hb⟩))]
    simp only [π₁_pair, π₂_pair]

open ZFSet Classical in
theorem composition_fprod_Image_bijective {A B A' B' φ ψ : ZFSet} {hφ : A.IsFunc A' φ} {hψ : B.IsFunc B' ψ}
  (φ_bij : φ.IsBijective) (ψ_bij : ψ.IsBijective) :
