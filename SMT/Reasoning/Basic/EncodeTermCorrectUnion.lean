...
/-- Erasing a freshly-inserted key recovers the original context: if `a ∉ s`
then `(s.insert a τ).erase a = s`. This is the algebraic fact behind the
sound-weakening `eraseFromContext` fix in `castUnion`. -/
private theorem erase_insert_self_union {a : SMT.𝒱} {τ : SMTType}
...
theorem castUnion_spec
  ...
  (bv_S_used : ∀ v ∈ SMT.bv S, v ∈ used) (bv_T_used : ∀ v ∈ SMT.bv T, v ∈ used)
       ...
       (∀ (den_S den_T : SMT.Dom),
         ...
         ∃ (Δ' : SMT.RenamingContext.Context) (Δ'_covers : RenamingContext.CoversFV Δ' t),
           ...
           ∃ den_t,
             ...
             (∀ (α : BType), γ = α.toSMTType →
               retract α.set den_t.1 = retract α.set den_S.1 ∪ retract α.set den_T.1)) ⌝ ⦄ := by
  -- Step 1: Reduce castUnion via the direct α = β path (sound-weakening fix:
  -- the leaked union! binder is erased from the context before the return)
  have hcastUnion :
  ...
  next z =>
    ...
    -- Step 2b: eraseFromContext z (sound-weakening fix: the leaked binder is
    -- removed before the pure return)
    mspec SMT.eraseFromContext_spec
    mrename_i preE
    mintro ∀StE
    mpure preE
    obtain ⟨StE_types_eq, StE_fvc, StE_used_eq⟩ := preE
    -- The erased context is `(St₀.types.insert z γ).erase z = St₀.types`.
    have StE_types_eq' : StE.types = St₀.types := by
      rw [StE_types_eq, St₁_types_eq, erase_insert_self_union z_fresh]
    ...
    · trivial
    -- (5) typing
    · have z_not_bv_S : z ∉ SMT.bv S := fun hbv => z_not_used (bv_S_used z hbv)
      have z_not_bv_T : z ∉ SMT.bv T := fun hbv => z_not_used (bv_T_used z hbv)
      ...
      · have hupdate : TypeContext.update St₀.types [z] [γ] rfl = St₀.types.insert z γ := by
        ...
        · apply SMT.Typing.app
          · exact Typing.weakening (TypeContext.entries_subset_insert_of_notMem z_fresh) typ_S
              (Typing.bv_notMem_insert_of_fresh typ_S z_not_bv_S)
        ...
        · apply SMT.Typing.app
          · exact Typing.weakening (TypeContext.entries_subset_insert_of_notMem z_fresh) typ_T
              (Typing.bv_notMem_insert_of_fresh typ_T z_not_bv_T)
          · exact SMT.Typing.var _ z γ (AList.lookup_insert St₀.types)
    -- (6) preserves_types: ∀ v ∈ used, v ∉ Λ → v ∉ Γ'
    · intro v hv hv_not_Λ
      first
      | exact hv_not_Λ
      | (rw [StE_types_eq']; exact hv_not_Λ)
    ...
    · intro den_S den_T h_den_S h_den_T
      ...
      · -- Wt + Denotation + RDom (direct path)
        ...
        obtain ⟨den_t, hden_t, htype_t, hretract_t, _⟩ :=
          castUnion_denotation_direct hS hT h_den_S h_den_T
...
theorem encodeTerm_spec.union_case.{u} (fv_sub_typings : B.FvSubTypings) (A B : B.Term)
  ...
  next out =>
    ...
    next out =>
      ...
      -- Inline castUnion for the direct α = β path (sound-weakening fix:
      -- the leaked union! binder is erased from the context before the return)
      have hcastUnion' :
      ...
      next z =>
        ...
        -- eraseFromContext z (sound-weakening fix: the leaked union! binder is
        -- removed before the pure return)
        mspec SMT.eraseFromContext_spec
        mrename_i preE
        mintro ∀StE
        ...
        -- The erased context is `(St₂.types.insert z γ).erase z = St₂.types`.
        have StE_types_eq' : StE.types = St₂.types := by
          rw [StE_types_eq, St_z_types_eq, erase_insert_self_union z_fresh]
        ...
        · intro v hv
          ...
          · rw [StE_used_eq, St_z_used_eq]; exact List.mem_cons_of_mem z (St₁_used_sub_St₂ (A_cov_St₁ v hv))
          · rw [StE_used_eq, St_z_used_eq]; exact List.mem_cons_of_mem z (B_cov_St₂ v hv)
        -- (5) τ = α.set.toSMTType
        · rfl
        -- (6) typing
        · rw [StE_types_eq']
          -- `z` is fresh w.r.t. `St₂.types` (`z_fresh`); bound variables of the
          -- encoded operands stay disjoint from the final context `St₂.types`.
          ...
          have hbv_Aenc_notMem_St₂ : ∀ v ∈ SMT.bv Aenc, v ∉ St₂.types := fun v hv =>
          ...
          have hbv_Aenc_notMem_St₂z : ∀ v ∈ SMT.bv Aenc, v ∉ St₂.types.insert z γ := by
            intro v hv hvin
            rcases (AList.mem_insert (s := St₂.types) (a := v) (a' := z) (b' := γ)).1 hvin with rfl | hvΓ
            · exact z_not_bv_Aenc hv
            · exact hbv_Aenc_notMem_St₂ v hv hvΓ
          have hsub_St₁_St₂z : St₁.types.entries ⊆ (St₂.types.insert z γ).entries :=
            fun e he => TypeContext.entries_subset_insert_of_notMem (τ := γ) z_fresh
              (St₁_sub_St₂ he)
          have typ_Aenc_St₂z : St₂.types.insert z γ ⊢ˢ Aenc : α.set.toSMTType :=
            Typing.weakening hsub_St₁_St₂z typ_Aenc hbv_Aenc_notMem_St₂z
          have typ_Benc_St₂z : St₂.types.insert z γ ⊢ˢ Benc : α.set.toSMTType :=
            Typing.weakening (TypeContext.entries_subset_insert_of_notMem (τ := γ) z_fresh)
        ...
        -- (7) preserves_types
        · intro v hv h1 h2
          obtain ⟨h2_A, h2_B⟩ := _root_.B.Term.notMem_vars_union.mp h2
          rw [StE_types_eq']
        ...
        · -- Use Δ'' as the renaming context; apply castUnion_denotation_direct
          ...
          have Δ''_none_out_z : ∀ v ∉ StE.env.usedVars, Δ'' v = none := by
            intro v hv; apply Δ''_none_out
            intro hmem; exact hv (StE_used_eq ▸ St_z_used_eq ▸ List.mem_cons_of_mem z hmem)
          ...
          · refine ⟨den_lambda, hden_lambda, ?_, ?_⟩
            ...
            · -- ih_total: alt universality
              ...
              have Δ₀_alt_A_wt : ∀ v (d : SMT.Dom), Δ₀_alt_A v = some d → ∀ τ, St₁.types.lookup v = some τ → d.snd.fst = τ := by
                intro v d hv τ hτ; simp only [Δ₀_alt_A_def] at hv; split_ifs at hv with h; exact Δ₀_alt_wt v d hv τ (by rw [StE_types_eq']; exact AList.mem_lookup_iff.mpr (St₁_sub_St₂ (AList.mem_lookup_iff.mp hτ)))
              -- Call A_ih_total
              ...
              have Δ₀_alt_B_wt : ∀ v (d : SMT.Dom), Δ₀_alt_B v = some d → ∀ τ, St₂.types.lookup v = some τ → d.snd.fst = τ := by
                ...
                · rw [if_pos h_used] at hv
                  ...
                  | some d' =>
                    ...
                    exact Δ₀_alt_wt v d' hΔ τ (by rw [StE_types_eq']; exact hτ)
                  | none =>
                    simp [hΔ] at hv
                    obtain ⟨h_mem, hv⟩ := hv
              ...
              · intro v d hv τ hτ
                ...
                | none =>
                  simp [hΔ] at hv
                  -- hv : Δ'_alt_B v = some d; the erased final context is St₂.types
                  rw [StE_types_eq'] at hτ
              ...
              · intro v hv
                ...
                | some d =>
                  exact fv_sub_typings (_root_.B.Typing.union typ_A typ_B)
                    (SMT.Typing.bool StE.types true) v
                    (SMT.RenamingContext.ExtendsOnSourceFV.dom_sub_B_fv Δ₀_alt_ext v
                      (by rw [hΔ]; simp))
                | none =>
                  simp [hΔ] at hv
                  -- Δ'_alt_B v ≠ none → v ∈ St₂.types = StE.types
