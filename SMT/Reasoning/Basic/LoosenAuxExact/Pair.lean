...
theorem loosenAux_prf_exact.pair («Δ» : RenamingContext.Context) {α β α' β' : SMTType} (pα : α ⇝ α')
  ...
  · rename_i out_fst
    ...
    mintro ∀St₃
    mpure pre
    obtain ⟨hrun_fst, hn₃, St₃_types_eq, fst!_fresh, fst!_not_used, used_sub₃, keys_sub₃, preserves_fst, typ_fst!, typ_fst!_spec, typ_fst!_St₃, typ_fst!_spec_St₃, fv_fst!_spec, den_fst⟩ := pre
    ...
    have typ_x_insfst : (St₂.types.insert fst! α') ⊢ˢ x : .pair α β :=
      SMT.Typing.weakening (h := SMT.TypeContext.entries_subset_insert_of_notMem fst!_fresh)
        typ_x_St₂ (SMT.Typing.bv_notMem_insert_of_fresh typ_x_St₂ fst!_not_bv_x)
    ...
    · rename_i out_snd
      ...
      have hbv_fst_St₄ : ∀ v ∈ bv fst!_spec, v ∉ St₄.types := by
        ...
        intro v hv
        exact preserves_snd v (bv_fst_used v hv)
                ...
                have hcontra : False := by
                  ...
                  have hall :
                    ...
                    have hDsnd_raw :
                      ...
                      simpa [SMT.RenamingContext.denote] using hDsnd
                    -- FV-restricted type compatibility for `fst!_spec` on the triple-updated
                    -- context (total branch, `Y` slot). On `fv x` use the top-level
                    -- `respects`; at `fst!` the slot carries tag `α'` by `hx0`.
                    have hcompat_fst :
                        SMT.RenamingContext.RespectsTypeContextOnFV
                    ...
                    have hDfst_ty : Dfst.2.1 = .bool :=
                      SMT.RenamingContext.denote_type_of_typing_fv typ_fst!_spec_St₃ hcompat_fst _ hDfst_raw
                    -- FV-restricted type compatibility for `snd!_spec` on the triple-updated
                    -- context (total branch, `Y` slot). On `fv x` use `respects`; at
                    -- `snd!` the slot carries tag `β'` by `hx1`.
                    have hcompat_snd :
                        SMT.RenamingContext.RespectsTypeContextOnFV
                          (Function.update (Function.update (Function.update «Δ» x! (some Y))
