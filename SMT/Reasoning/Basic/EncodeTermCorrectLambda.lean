...
private theorem lambda_chosen_totality.{u}
    ...
    (z_not_vs : z ∉ vs)
    (z_not_bv_D : z ∉ SMT.bv D_enc)
    (z_not_bv_P : z ∉ SMT.bv P_enc)
    (bv_D_disj_Γ_P : ∀ v ∈ SMT.bv D_enc, v ∉ Γ_P)
    (hcov_lambda : SMT.RenamingContext.CoversFV Δ'
      ((λˢ [z]) [τ.toSMTType.pair β.toSMTType]
        ((@ˢD_enc) (SMT.Term.var z).fst ∧ˢ
          ((SMT.Term.var z).snd =ˢ
    ...
    (P_enc_total :
      ∀ (Δ_alt : B.RenamingContext.Context) (Δ_fv_alt : ∀ v ∈ B.fv P, (Δ_alt v).isSome = true)
        ...
        B.RenWF (vs.zipToAList αs ∪ E_ctx) Δ_alt →
  ...
  obtain ⟨Δ_D_alt, hcov_D_alt, denD_alt, hext_D_alt, Δ_D_alt_none_out,
    ...
    D_total Δ_alt
      (fun v hv => Δ_fv_alt v (B.fv.mem_lambda (.inl hv)))
      Δ₀_alt_D hext_alt_D wf_alt hnone_alt_D hwt_alt_D
  ...
  have Δ_fv_P_alt : ∀ v ∈ B.fv P, (Δ_ext_alt v).isSome = true := by
    ...
    · simp [List.getElem_ofFn]
    · exact Δ_fv_alt v (B.fv.mem_lambda (.inr ⟨hv, hvs⟩))
  have hwf_P_alt : B.RenWF (vs.zipToAList αs ∪ E_ctx) Δ_ext_alt :=
    P_renwf Δ_alt f_alt wf_alt (fun i => (hf_alt_typ i).1)
  ...
  obtain ⟨Δ_P_alt, hcov_P_alt, denT_P_alt, Δ_P_alt_extends, Δ_P_alt_none_out_used_P,
    ...
    P_enc_total Δ_ext_alt Δ_fv_P_alt Δ₀_hybrid_alt Δ₀_hybrid_ext_P_alt hwf_P_alt
  ...
  have h_typ_Γ_P : Γ_P ⊢ˢ lambda_term :
    ...
    · -- Body typing under Γ_P.insert z (...).
      ...
      -- D_enc typed under Γ_P (entries subset + bv disjoint from Γ_P).
      have typ_D_enc_Γ_P : Γ_P ⊢ˢ D_enc : τ.toSMTType.fun SMTType.bool :=
        SMT.Typing.weakening Γ_D_entries_sub_Γ_P typ_D_enc bv_D_disj_Γ_P
      ...
      · -- D_app: weaken D_enc from Γ_P to Γ_z (single fresh binder z).
        exact SMT.Typing.app _ _ _ _ _
          (SMT.Typing.weakening h_ins typ_D_enc_Γ_P
            (SMT.Typing.bv_notMem_insert_of_fresh typ_D_enc_Γ_P z_not_bv_D))
          h_z_fst
      · -- eq_t: z.snd =ˢ substList vs (toDestPair vs z.fst) P_enc.
        ...
        · exact SMT.Typing.weakening h_ins typ_P_enc
            (SMT.Typing.bv_notMem_insert_of_fresh typ_P_enc z_not_bv_P)
  ...
  · -- retract (τ ×ᴮ β).set denT_alt.fst = T_alt
    ...
    · -- Chosen branch: 𝒟_alt ≠ ∅. Apply lambda_chosen_retract_eq.
      ...
      have hP_ty_eq : P_dom_alt.2.fst = β := by
        ...
        have hwf_P_choose : B.RenWF (vs.zipToAList αs ∪ E_ctx)
            (Function.updates Δ_alt vs (List.ofFn fun i => some (f_alt i))) :=
          P_renwf Δ_alt f_alt wf_alt (fun i => rfl)
        have h_wt := denote_welltyped_eq
          (t := P.abstract (Function.updates Δ_alt vs
            (List.ofFn fun i => some (f_alt i))) Δ_fv_alt_P)
          ⟨_, WFTC.of_abstract, β,
      ...
      case hbridge_alt =>
        ...
        have P_enc_total_simple :
            ∀ (Δ_a : B.RenamingContext.Context)
              ...
              B.RenWF (vs.zipToAList αs ∪ E_ctx) Δ_a →
          ...
          intro Δ_a Δ_fv_a Δ₀_a hext' wf' hnone' T_a hT_a hden'
          obtain ⟨Δ'_out, hcov_out, denT_out, hext_out, _, _, hden_out, hRDom_out, _⟩ :=
            P_enc_total Δ_a Δ_fv_a Δ₀_a hext' wf' hnone'
        ...
        · -- Case: xy.π₁ ∈ 𝒟_alt. Apply lambda_hbridge directly.
          ...
          · let xₙ : Fin 1 → SMT.Dom := fun _ => Wxy
            ...
            have hbridge_iff := lambda_hbridge vs_nemp vs_nodup τ_hasArity
              ...
              (fun x_fin htyp => P_renwf Δ_alt x_fin wf_alt htyp)
...
private theorem lambda_defaults_totality.{u}
  ...
  lambda_chosen_totality (vs := vs) (τ := τ) (β := β) (αs := αs) (E_ctx := E_ctx)
    ...
    (used' := used') vs_nemp vs_nodup h_toSMT typ_t typ_D typP P_renwf typ_D_enc typ_P_enc
    τ_hasArity vs_not_D_fv vs_disj_Γ_D vs_used_P vs_lookup_Γ_P z_not_Γ_P
    z_not_Γ' z_not_used_P z_not_fv_D z_not_fv_P z_not_vs
    z_not_bv_D z_not_bv_P bv_D_disj_Γ_P hcov_lambda hden_eq hden_ty
...
theorem encodeTerm_spec.lambda_case.{u} (fv_sub_typings : B.FvSubTypings)
  ...
  (Λ_inv : ∀ v ∈ (Term.lambda vs D P).vars, v ∈ Λ → v ∈ E.context)
  (bv_nodup : (B.bv (Term.lambda vs D P)).Nodup)
  (respects : B.RenamingContext.RespectsTypeContextOnFV (B.RenamingContext.toSMT «Δ») Λ ((Term.lambda vs D P)))
  (fv_in_Λ : ∀ v ∈ B.fv ((Term.lambda vs D P)), v ∈ Λ)
              ...
              Λ ⊆ Γ' ∧
                AList.keys Γ' ⊆ E'.usedVars ∧
                  CoversUsedVars E'.usedVars (Term.lambda vs D P) ∧
                    σ = α.toSMTType ∧
                      Γ' ⊢ˢ t' : σ ∧
                        (∀ v ∈ used, v ∉ Λ → v ∉ B.Term.vars (Term.lambda vs D P) → v ∉ Γ') ∧
                          ∃ Δ',
                            ∃ (Δ'_covers : RenamingContext.CoversFV Δ' t'),
                              RenamingContext.Extends Δ' Δ₀ ∧
  ...
  mspec (Std.Do.Triple.and _
    (Std.Do.Triple.and _
      (D_ih (E := E) (Λ := St₀.types) (α := .set τ) typ_D
        ...
        wf)
      (SMT.encodeTerm_bv_used E (t := D) (used := St₀.env.usedVars)
        (n := St₀.env.freshvarsc) (decl := St₀.env.declarations)))
    (SMT.encodeTerm_bv_notMem_used E (t := D) (used := St₀.env.usedVars)
      (n := St₀.env.freshvarsc) (decl := St₀.env.declarations)))
  clear D_ih
  rename_i out_D
  ...
  obtain ⟨⟨⟨used_sub_St₁, St₀_sub_St₁, St₁_keys_sub, covers_D, rfl, typ_D_enc,
    ...
    bvD_used, _⟩, bvD_notMem_used, _⟩ := pre
  ...
  · -- Chosen branch: 𝒟' ≠ ∅, valuation uses Classical.choose witness.
    ...
    have St₂_types_sub_E_ctx_on_P_vars : ∀ v ∈ P.vars, v ∈ St₂.types → v ∈ E'.context := by
      ...
      · right
        ...
        · have v_vars_D : v ∈ B.Term.vars D := by
          ...
          · -- v ∈ B.bv D. Case split on P.vars: fv P or bv P.
    ...
    mspec (Std.Do.Triple.and _
      ...
      (by
        ...
        · have hv_lambda : v ∈ B.fv (Term.lambda vs D P) := by
          ...
          have hToSMT_eq : (B.RenamingContext.toSMT Δ_ext) v
            ...
            unfold B.RenamingContext.toSMT
            rw [hΔ_ext_eq]
          have hv_Λ : v ∈ St₀.types := fv_in_Λ v hv_lambda
      ...
      (by
        ...
        · have hv_lambda : v ∈ B.fv (Term.lambda vs D P) := by
          ...
          have hτ'_St₂ : St₂.types.lookup v = some τ' := by
            rw [St₂_types]
            apply foldl_insert_preserves_lookup hτ'
            intro p hp heq
            exact hvs (heq ▸ (List.of_mem_zip hp).1)
          exact AList.lookup_isSome.mp (Option.isSome_of_mem hτ'_St₂))
      wf_P)
      (SMT.encodeTerm_bv_used E' (t := P) (used := St₂.env.usedVars)
        (n := St₂.env.freshvarsc) (decl := St₂.env.declarations)))
    clear P_ih
    rename_i out_P
    ...
    have St₅_types : St₅.types = St₄.types := rfl
    have St₅_fvc : St₅.env.freshvarsc = St₄.env.freshvarsc := rfl
    have St₅_used : St₅.env.usedVars = St₄.env.usedVars := rfl
    ...
    have St₆_sub_St₃ : St₆.types ⊆ St₃.types := by
      ...
      have hkv_ins := AList.erase_entries_subset z _ hkv
    ...
    have h_typ_St₆ : St₆.types ⊢ˢ
        ...
        Typing.strengthening_of_fv_subset St₆_sub_St₃ h_typ_St₃ (by
          have h_surv : ∀ v, v ∈ St₃.types → v ∉ vs → v ≠ z → v ∈ St₆.types := by
            ...
            exact AList.mem_keys.mpr (List.mem_map.mpr ⟨⟨v, τ_v⟩,
              List.mem_kerase_of_ne_key hv_ne_z h_ins, rfl⟩)
          intro v hv_lam
      ...
      -- bv-freshness facts for the sound weakenings (threaded via encodeTerm_bv specs).
      have z_not_bv_D : z ∉ SMT.bv D_enc := fun hbv =>
        z_not_used (St₂_sub_St₃ (St₁_sub_St₂_used (bvD_used z hbv)))
      have z_not_bv_P : z ∉ SMT.bv P_enc := fun hbv => z_not_used (bvP_used z hbv)
      have bv_D_disj_St₃ : ∀ v ∈ SMT.bv D_enc, v ∉ St₃.types := by
        intro v hv
        refine P_preserves_types v (St₁_sub_St₂_used (bvD_used v hv)) ?_ ?_
        · -- v ∉ St₂.types : not a vs-key (bv D_enc ∉ used) and not in St₁.types (own ctx).
          intro hv_St₂
          have hv_not_vs : v ∉ vs := fun hv_vs =>
            bvD_notMem_used v hv (St₀_used_eq ▸ vars_used_vs v hv_vs)
          have hv_St₁ : v ∈ St₁.types := by
            rw [St₂_types] at hv_St₂
            exact AList.mem_of_mem_foldl_insert' hv_St₂ (by
              intro h
              rw [List.mem_map] at h
              obtain ⟨⟨a, b⟩, hab, rfl⟩ := h
              exact hv_not_vs (List.of_mem_zip hab).1)
          exact SMT.Typing.bv_notMem_context typ_D_enc v hv hv_St₁
        · -- v ∉ P.vars : bv D_enc ∉ used, but P.vars ⊆ used.
          intro hv_P
          exact bvD_notMem_used v hv (St₀_used_eq ▸ vars_used_P v hv_P)
      have typ_D_enc_St₃ : St₃.types ⊢ˢ D_enc : τ.toSMTType.fun SMTType.bool :=
        SMT.Typing.weakening h_St₁_sub_St₃ typ_D_enc bv_D_disj_St₃
      ...
      · rw [hupdate]
        ...
        refine SMT.Typing.and _ _ _ ?_ ?_
        · exact SMT.Typing.app _ _ _ _ _
            (SMT.Typing.weakening h_ins typ_D_enc_St₃
              (SMT.Typing.bv_notMem_insert_of_fresh typ_D_enc_St₃ z_not_bv_D))
    ...
    · -- 2. St₀.types ⊆ St₆.types
      ...
      exact List.mem_kerase_of_ne_key hk_ne_z hk_St₃_kerase
    ...
    · -- 7. preserves types
      ...
      exfalso
      have v_notMem_St₁ := D_preserves_types v v_used v_notMem_St₀ v_notMem_vars_D
      ...
      exact P_preserves_types v (St₁_sub_St₂_used (used_sub_St₁ v_used))
        v_notMem_St₂ v_notMem_vars_P v_mem_St₃
      ...
      · -- Chosen-branch denotation/RDom/totality. Hcompat + denT' existence parallel
        ...
        · -- RDom: T = (𝒟.prod β.toZFSet).sep ℰ (non-empty in chosen branch).
          ...
          case hbridge =>
            ...
            have P_enc_total_simple :
              ...
              intro Δ_alt Δ_fv_alt Δ₀_alt hext' wf' hnone' T_alt hT_alt hden'
              obtain ⟨Δ'_out, hcov_out, denT_out, hext_out, _, _, hden_out, hRDom_out, _⟩ :=
                P_enc_total Δ_alt Δ_fv_alt Δ₀_alt hext' wf' hnone'
            ...
            · -- Case: xy.π₁ ∈ 𝒟. Apply lambda_hbridge directly.
              ...
              · let xₙ : Fin 1 → SMT.Dom := fun _ => Wxy
                ...
                have hbridge_iff := lambda_hbridge vs_nemp vs_nodup τ_hasArity
                  ...
                  (fun x_fin htyp => B.RenWF.updates_ofFn wf vs_nodup vs_Γ_disj vs_αs_len
                    (fun i => (htyp i).trans (_root_.BType.get_reduce αs_nemp vs_αs_len i)))
                  xy hx_mem hy_mem hxy_pair' hxy_π₁_arity hxy_𝒟'
                  body_val hbody_val
                -- Convert the iff.
        ...
        · -- Totality clause for alternative valuations.
          ...
          have St₃_lookup_sub_St₆_P : ∀ v, v ∉ vs → ∀ τ_v,
            ...
            have hentry_St₆ := List.mem_kerase_of_ne_key hv_ne_z h_ins
          ...
          have Δ'_wt_Γ' : ∀ v (d : SMT.Dom), Δ' v = some d →
            ...
            by_cases hvs : v ∈ vs
            · -- v ∈ vs ⇒ Δ' v = Δ_D v ⇒ v ∈ St₁ via Δ_D_dom, but vs_disj_St₁: contradiction
              exfalso
              have hΔ'_eq : Δ' v = Δ_D v := by rw [Δ'_def]; simp [hvs]
              have hΔ_D : Δ_D v = some d := hΔ'_eq ▸ hv
              exact vs_disj_St₁ v hvs
                (Δ_D_dom v (Option.ne_none_iff_exists.mpr ⟨d, hΔ_D.symm⟩))
            · have hΔ'_eq : Δ' v = Δ_P v := by rw [Δ'_def]; simp [hvs]
          ...
          have z_not_bv_D_aux : z ∉ SMT.bv D_enc := fun hbv =>
            z_not_used (St₂_sub_St₃ (St₁_sub_St₂_used (bvD_used z hbv)))
          have z_not_bv_P_aux : z ∉ SMT.bv P_enc := fun hbv => z_not_used (bvP_used z hbv)
          have bv_D_disj_St₃_aux : ∀ v ∈ SMT.bv D_enc, v ∉ St₃.types := by
            intro v hv
            refine P_preserves_types v (St₁_sub_St₂_used (bvD_used v hv)) ?_ ?_
            · intro hv_St₂
              have hv_not_vs : v ∉ vs := fun hv_vs =>
          ...
          exact lambda_chosen_totality (D := D) (P := P) (D_enc := D_enc)
            ...
            vs_nemp vs_nodup rfl typ_t typ_D typP
            (fun Δ_a f_a hwf_a htyp_a => B.RenWF.updates_ofFn hwf_a vs_nodup vs_Γ_disj vs_αs_len
              (fun i => (htyp_a i).trans (_root_.BType.get_reduce αs_nemp vs_αs_len i)))
            typ_D_enc typ_P_enc
            τ_hasArity vs_not_D_fv vs_disj_St₁ vs_used_St₃ vs_lookup_St₃
            z_fresh z_not_St₆ z_not_used
            z_not_fv_D_aux z_not_fv_P_aux z_not_vs_aux
            z_not_bv_D_aux z_not_bv_P_aux bv_D_disj_St₃_aux
