...
private theorem mem_retract_set_iff_app_canonical_eq_zftrue_inter
...
private theorem castInter_denotation_aux.{u_1}
    ...
    (hcov : RenamingContext.CoversFV (Function.update «Δ» S! (some X!))
      (.lambda [z] [γ] (.and (.app (.var S!) (.var z)) (.app T (.var z))))) :
    ∃ den_t : SMT.Dom.{u_1},
      ⟦(Term.lambda [z] [γ] (.and (.app (.var S!) (.var z)) (.app T (.var z)))).abstract
  ...
  set andBody := Term.and (.app (.var S!) (.var z)) (.app T (.var z)) with handBody_def
  ...
  -- Body denotation: for each valid W, the and-body denotes
  have hbody_den : ∀ (W : SMT.Dom) (hW_ty : W.2.1 = γ) (hW_mem : W.1 ∈ ⟦γ⟧ᶻ),
      ∃ Dbody : SMT.Dom,
        ∃ hcov_and : RenamingContext.CoversFV (Function.update Δ' z (some W)) andBody,
          ⟦andBody.abstract (Function.update Δ' z (some W)) hcov_and⟧ˢ = some Dbody ∧
          Dbody.2.1 = .bool ∧
          (Dbody.1 = zftrue ↔
    ...
    have hcov_and : RenamingContext.CoversFV (Function.update Δ' z (some W)) andBody := by
      intro v hv
      change v ∈ SMT.fv andBody at hv
      rw [handBody_def] at hv
    ...
    -- Case analysis on ZFBool values: .and denotes directly
    ...
    · -- Both false: and is false
      have hDand := denote_and_eq_zffalse_of_some_zffalse_left hden_S!_app hDS!_bool hDS!_false hden_T_app hDT!_bool
      refine ⟨⟨zffalse, .bool, ZFBool.zffalse_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · constructor
        · intro h; exact (ZFSet.zftrue_ne_zffalse h.symm).elim
        · intro ⟨h, _⟩; nomatch ZFSet.zftrue_ne_zffalse (h.symm.trans (hDS!_val.symm.trans hDS!_false))
    · -- S false, T true: and is false
      have hDand := denote_and_eq_zffalse_of_some_zffalse_left hden_S!_app hDS!_bool hDS!_false hden_T_app hDT!_bool
      refine ⟨⟨zffalse, .bool, ZFBool.zffalse_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · constructor
        · intro h; exact (ZFSet.zftrue_ne_zffalse h.symm).elim
        · intro ⟨h, _⟩; nomatch ZFSet.zftrue_ne_zffalse (h.symm.trans (hDS!_val.symm.trans hDS!_false))
    · -- S true, T false: and is false
      have hDand := denote_and_eq_zffalse_of_some_zffalse_right hden_S!_app hDS!_bool hden_T_app hDT!_bool hDT!_false
      refine ⟨⟨zffalse, .bool, ZFBool.zffalse_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · constructor
        · intro h; exact (ZFSet.zftrue_ne_zffalse h.symm).elim
        · intro ⟨_, h⟩; nomatch ZFSet.zftrue_ne_zffalse (h.symm.trans (hDT!_val.symm.trans hDT!_false))
    · -- Both true: and is true
      have hDand := denote_and_eq_zftrue_of_some_zftrue hden_S!_app hDS!_bool hDS!_true hden_T_app hDT!_bool hDT!_true
      refine ⟨⟨zftrue, .bool, ZFBool.zftrue_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · exact ⟨fun _ => ⟨hDS!_val ▸ hDS!_true, hDT!_val ▸ hDT!_true⟩, fun _ => rfl⟩

  -- 6. Lambda assembly + retract equality
  -- 6a. CoversFV for andBody under z-updates of Δ'
  have hcov_andBody_upd :
      ∀ W : SMT.Dom,
        RenamingContext.CoversFV (Function.update Δ' z (some W)) andBody := by
    intro W v hv
    change v ∈ SMT.fv andBody at hv
    rw [handBody_def] at hv
    simp only [SMT.fv, List.mem_append, List.mem_singleton] at hv
  ...
  have hgo_cov : ∀ v ∈ SMT.fv andBody, v ∉ [z] → (Δ' v).isSome = true := by
    intro v hv hvz
    have hvz' : v ≠ z := by simpa [List.mem_singleton] using hvz
    have := hcov_andBody_upd ⟨γ.defaultZFSet, γ, SMTType.mem_toZFSet_of_defaultZFSet⟩ v hv
    rwa [Function.update_of_ne hvz'] at this
  -- 6c. Body denotes for all valid W
  have hbody_total : ∀ W : SMT.Dom, W.snd.fst = γ →
      ⟦andBody.abstract (Function.update Δ' z (some W)) (hcov_andBody_upd W)⟧ˢ.isSome = true := by
    intro W hW_ty
    have hW_mem : W.fst ∈ ⟦γ⟧ᶻ := by rw [← hW_ty]; exact W.snd.snd
    obtain ⟨Dbody, hcov_and, hden_body, _⟩ := hbody_den W hW_ty hW_mem
    rw [show hcov_andBody_upd W = hcov_and from Subsingleton.elim _ _]
    exact Option.isSome_of_eq_some hden_body
  -- 6d. Body type is always .bool
  have hbody_ty : ∀ (W : SMT.Dom), W.snd.fst = γ → ∀ D,
      ⟦andBody.abstract (Function.update Δ' z (some W)) (hcov_andBody_upd W)⟧ˢ = some D →
      D.snd.fst = .bool := by
    intro W hW_ty D hden
    have hW_mem : W.fst ∈ ⟦γ⟧ᶻ := by rw [← hW_ty]; exact W.snd.snd
    obtain ⟨Dbody, hcov_and, hden_body, hDbody_ty, _⟩ := hbody_den W hW_ty hW_mem
    rw [show hcov_andBody_upd W = hcov_and from Subsingleton.elim _ _] at hden
    cases hden.symm.trans hden_body
    exact hDbody_ty
  -- 6e. Lambda isSome
  have hsome_lambda : ⟦((λˢ [z]) [γ] andBody).abstract Δ' hcov⟧ˢ.isSome = true := by
    ...
    · exfalso; apply den_t_typ_det
      ...
      have hgo_x := funAbstractGoSingle (Δctx := Δ') (P := andBody) (v := z) (τ := γ)
        hgo_cov hcov_andBody_upd x hx
      have hgo_y := funAbstractGoSingle (Δctx := Δ') (P := andBody) (v := z) (τ := γ)
        hgo_cov hcov_andBody_upd y hy
      ...
      have hden_x : ⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry x⟧ˢ = some Dx := by
        rw [hgo_x]; exact hDx
      have hden_y : ⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry y⟧ˢ = some Dy := by
        rw [hgo_y]; exact hDy
      calc (⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry x⟧ˢ.get
        ...
        _ = (⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry y⟧ˢ.get
    ...
    · exfalso; apply den_t_isSome
      ...
      let Wx : SMT.Dom := x ⟨0, by simp⟩
      have hWx_ty : Wx.snd.fst = γ := by simpa [Wx] using (hx ⟨0, by simp⟩).1
      rw [funAbstractGoSingle (Δctx := Δ') (P := andBody) (v := z) (τ := γ)
        hgo_cov hcov_andBody_upd x hx]
      exact hbody_total Wx hWx_ty
  -- 6f. Build the body function
  classical
  set bodyFun : ZFSet → ZFSet := fun x₁ =>
    if hx : x₁.hasArity 1 ∧ x₁ ∈ ⟦γ⟧ᶻ then
      let W : SMT.Dom := ⟨x₁, γ, hx.2⟩
      if hsome : ⟦andBody.abstract (Function.update Δ' z (some W))
          (hcov_andBody_upd W)⟧ˢ.isSome then
        (⟦andBody.abstract (Function.update Δ' z (some W))
            (hcov_andBody_upd W)⟧ˢ.get hsome).fst
  ...
  have hbodyFun_range : ∀ {x₁ : ZFSet}, x₁ ∈ ⟦γ⟧ᶻ → bodyFun x₁ ∈ 𝔹 := by
    ...
    obtain ⟨Dbody, hcov_and, hden_body, hDbody_ty, _⟩ := hbody_den W rfl hW_mem
    rw [show hcov_andBody_upd W = hcov_and from Subsingleton.elim _ _] at hsome ⊢
  ...
  have hbodyFun_eq : ∀ (w : ZFSet) (hw : w ∈ ⟦γ⟧ᶻ),
      ∃ Dbody : SMT.Dom,
        ⟦andBody.abstract (Function.update Δ' z (some ⟨w, γ, hw⟩))
          (hcov_andBody_upd ⟨w, γ, hw⟩)⟧ˢ = some Dbody ∧
        bodyFun w = Dbody.fst := by
    intro w hw
    let W : SMT.Dom := ⟨w, γ, hw⟩
    obtain ⟨Dbody, hcov_and, hden_body, _⟩ := hbody_den W rfl hw
    refine ⟨Dbody, ?_, ?_⟩
    · rw [show hcov_andBody_upd W = hcov_and from Subsingleton.elim _ _]; exact hden_body
    · simp only [bodyFun]
  ...
  have hlamVal_ty : lamVal.snd.fst = .fun γ .bool := by
    ...
    · let xd : Fin 1 → SMT.Dom := fun _ => ⟨γ.defaultZFSet, γ, SMTType.mem_toZFSet_of_defaultZFSet⟩
      ...
      have hgo_d := funAbstractGoSingle (Δctx := Δ') (P := andBody) (v := z) (τ := γ)
        hgo_cov hcov_andBody_upd xd hxd_spec
      obtain ⟨Dd, hDd⟩ := Option.isSome_iff_exists.mp (hbody_total (xd ⟨0, by simp⟩) rfl)
      have hden_d : ⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry xd⟧ˢ = some Dd := by
        rw [hgo_d]; exact hDd
      have hγ_out : (⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry xd⟧ˢ.get
          ...
          have h_pair_mem : w'.pair Dbody'.fst ∈ lamVal.fst := by
            ...
            · split_ifs with hw'_cond
              · let xₙ := fun i : Fin 1 => (⟨w'.get 1 i, [γ][↑i], hw'_cond.2 i⟩ : SMT.Dom)
                have hgo' := funAbstractGoSingle (Δctx := Δ') (P := andBody) (v := z) (τ := γ)
                  hgo_cov hcov_andBody_upd xₙ (fun i => ⟨rfl, hw'_cond.2 i⟩)
                have hxₙ_eq : xₙ ⟨0, Nat.zero_lt_one⟩ = ⟨w', γ, hw'⟩ := rfl
                have hden' : ⟦(SMT.Term.abstract.go andBody [z] Δ' hgo_cov).uncurry xₙ⟧ˢ = some Dbody' := by
  ...
  · intro α hα
    ...
    have fapply_iff (hx_α : x ∈ ⟦α⟧ᶻ)
      ...
      mem_retract_set_iff_app_canonical_eq_zftrue_inter hF_func rfl hx_α
    constructor
    · -- (→) x ∈ retract α.set lamVal.fst → x ∈ ... ∩ ...
      ...
      have hDbody_eq : Dbody = Dbody' := by
        rw [show hcov_andBody_upd ⟨mk_cx hx_α, α.toSMTType, hcx_mem⟩ = hcov_and from
      ...
      obtain ⟨hS_true, hT_true⟩ := hDbody'_iff.mp hx_mem
      constructor
      · rw [fapply_iff hx_α hdenS_func]
        rw [(ZFSet.is_func_ext_iff hX!_func hdenS_func).mp X!_val_eq (mk_cx hx_α) hcx_mem] at hS_true
        exact hS_true
      · rw [fapply_iff hx_α hdenT_func]; exact hT_true
...
/-!
### Direct-path denotation auxiliary for the new `castInter` (α = β case)

When both inputs have the same type `.fun γ .bool`, `castInter` builds
`λ z : γ. S(z) ∧ T(z)` directly (no loosening, no S! variable).
The renaming context is `Δ` unchanged.
-/
set_option maxHeartbeats 4000000 in
private theorem castInter_denotation_direct.{u_1}
    {γ : SMTType} {S T : SMT.Term}
    ...
    (hcov : RenamingContext.CoversFV «Δ»
      (.lambda [z] [γ] (.and (.app S (.var z)) (.app T (.var z))))) :
    ∃ den_t : SMT.Dom.{u_1},
      ⟦(Term.lambda [z] [γ] (.and (.app S (.var z)) (.app T (.var z)))).abstract
        «Δ» hcov⟧ˢ = some den_t ∧
      den_t.2.1 = .fun γ .bool ∧
      (∀ (α : BType), γ = α.toSMTType →
        retract α.set den_t.1 = retract α.set den_S.1 ∩ retract α.set den_T.1) := by
  ...
  have hbody_den : ∀ (W : SMT.Dom) (hW_ty : W.2.1 = γ) (hW_mem : W.1 ∈ ⟦γ⟧ᶻ),
    ...
    rcases DT!_mem_𝔹 with hDT!_false | hDT!_true
    · -- Both false: and is false
      have hDand := denote_and_eq_zffalse_of_some_zffalse_left hden_S_app hDS!_bool hDS!_false hden_T_app hDT!_bool
      refine ⟨⟨zffalse, .bool, ZFBool.zffalse_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · constructor
        · intro h; exact (ZFSet.zftrue_ne_zffalse h.symm).elim
    ...
    · -- S false, T true: and is false
      ...
      · constructor
        ...
        · intro ⟨h, _⟩; nomatch ZFSet.zftrue_ne_zffalse (h.symm.trans (hDS!_val.symm.trans hDS!_false))
    · -- S true, T false: and is false
      have hDand := denote_and_eq_zffalse_of_some_zffalse_right hden_S_app hDS!_bool hden_T_app hDT!_bool hDT!_false
      refine ⟨⟨zffalse, .bool, ZFBool.zffalse_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · constructor
        · intro h; exact (ZFSet.zftrue_ne_zffalse h.symm).elim
        · intro ⟨_, h⟩; nomatch ZFSet.zftrue_ne_zffalse (h.symm.trans (hDT!_val.symm.trans hDT!_false))
    · -- Both true: and is true
      have hDand := denote_and_eq_zftrue_of_some_zftrue hden_S_app hDS!_bool hDS!_true hden_T_app hDT!_bool hDT!_true
      refine ⟨⟨zftrue, .bool, ZFBool.zftrue_mem_𝔹⟩, hcov_and, ?_, rfl, ?_⟩
      · convert hDand using 1; simp only [handBody_def, SMT.Term.abstract]
      · exact ⟨fun _ => ⟨hDS!_val ▸ hDS!_true, hDT!_val ▸ hDT!_true⟩, fun _ => rfl⟩

  -- 2. Lambda assembly + retract equality
  have hcov_andBody_upd :
      ∀ W : SMT.Dom,
        RenamingContext.CoversFV (Function.update «Δ» z (some W)) andBody := by
    intro W v hv
  ...
  · intro α hα
    ...
    · -- (→) x ∈ retract α.set lamVal.fst → x ∈ ... ∩ ...
      ...
      rw [hfapply_eq, hDbody_eq] at hx_mem
      obtain ⟨hS_true, hT_true⟩ := hDbody'_iff.mp hx_mem
      exact ⟨(fapply_iff hx_α hdenS_func).mpr hS_true, (fapply_iff hx_α hdenT_func).mpr hT_true⟩
    · -- (←) x ∈ ... ∩ ... → x ∈ retract α.set lamVal.fst
      intro ⟨hx_S, hx_T⟩
      have hx_α := retract_mem_α hx_S
      rw [fapply_iff hx_α hdenS_func] at hx_S
...
theorem castInter_spec
       ...
       (∀ (den_S den_T : SMT.Dom),
         ...
         ∃ (Δ' : SMT.RenamingContext.Context) (Δ'_covers : RenamingContext.CoversFV Δ' t),
           ...
           ∃ den_t,
             ...
             (∀ (α : BType), γ = α.toSMTType →
               retract α.set den_t.1 = retract α.set den_S.1 ∩ retract α.set den_T.1)) ⌝ ⦄ := by
  -- Step 1: Reduce castInter via the direct α = β path
  have hcastInter :
      castInter (S, .fun γ .bool) (T, .fun γ .bool) = do
        let x ← freshVar γ "inter!"
        SMT.eraseFromContext x
        return (.lambda [x] [γ] (.and (.app S (.var x)) (.app T (.var x))), .fun γ .bool) := by
    unfold castInter
    simp
  rw [hcastInter]
  mintro pre ∀St₀
  ...
  next z =>
    ...
    -- Step 2b: erase the fresh lambda binder `z` from the returned context
    -- (the encoder now `erase`s `z` after re-binding it as the lambda binder).
    mspec SMT.eraseFromContext_spec
    mrename_i preE
    mintro ∀StE
    mpure preE
    obtain ⟨StE_types_eq, StE_fvc, StE_used_eq⟩ := preE
    -- The final context is `St₁.types.erase z = (insert z γ St₀.types).erase z = St₀.types`.
    have StE_eq_St₀ : StE.types = St₀.types := by
      rw [StE_types_eq, St₁_types_eq, encodeTerm_state.erase_insert_self z_fresh]
    ...
    and_intros
    -- (1) used ⊆ E'.usedVars
    · rw [StE_used_eq, St₁_used_eq]; exact List.subset_cons_of_subset z fun _ => id
    -- (2) Λ ⊆ Γ'
    · rw [StE_eq_St₀]
    -- (3) keys Γ' ⊆ E'.usedVars
    · intro v hv
    ...
    -- (5) typing
    · rw [StE_eq_St₀]
      have z_not_bv_S : z ∉ SMT.bv S := fun hbv => z_not_used (hbv_S z hbv)
      have z_not_bv_T : z ∉ SMT.bv T := fun hbv => z_not_used (hbv_T z hbv)
    ...
    · intro v hv hv_not_Λ
      rw [StE_eq_St₀]; exact hv_not_Λ
    -- (7) renaming context + denotation
    · intro den_S den_T h_den_S h_den_T
      -- In the direct path, Δ' = Δ (no new free variables from castInter)
      refine ⟨«Δ», ?_, ?_, ?_, ?_⟩
      · -- CoversFV Δ t
        intro v hv
      ...
      · -- ∀ v ∉ E'.usedVars, Δ v = none
        ...
        apply Δ_none_out
        intro hv_used
        exact hv (by rw [StE_used_eq, St₁_used_eq]; exact List.mem_cons_of_mem z hv_used)
      · -- Wt + Denotation + RDom (direct path)
        ...
        have hcov_lambda : RenamingContext.CoversFV «Δ»
            (.lambda [z] [γ] (.and (.app S (.var z)) (.app T (.var z)))) := by
          intro v hv
          simp only [SMT.fv, List.removeAll, List.mem_filter, List.mem_append,
            List.mem_cons, List.not_mem_nil, or_false] at hv
          obtain ⟨hv_body, hv_ne_z⟩ := hv
...
theorem encodeTerm_spec.inter_case.{u} (fv_sub_typings : B.FvSubTypings) (A B : B.Term)
                ...
                ∀ {T : ZFSet.{u}} {hT : T ∈ ⟦α⟧ᶻ},
                  ⟦A.abstract «Δ» Δ_fv⟧ᴮ = some ⟨T, ⟨α, hT⟩⟩ →
                    (∀ v ∈ A.vars, v ∈ used) →
                      ...
                      ((_root_.B.bv A).Nodup) →
                        _root_.B.RenamingContext.RespectsTypeContextOnFV (_root_.B.RenamingContext.toSMT «Δ») Λ A →
                        (∀ v ∈ _root_.B.fv A, v ∈ Λ) →
                        _root_.B.RenWF E.context «Δ» →
                        ∀ {n : ℕ},
                          ⦃fun x =>
                            match x with
                            | { env := E0, types := Λ' } =>
                                      ...
                                      Λ ⊆ Γ' ∧
                                        AList.keys Γ' ⊆ E'.usedVars ∧
                                          CoversUsedVars E'.usedVars A ∧
                                            σ = α.toSMTType ∧
                                              Γ' ⊢ˢ t' : σ ∧
                                                (∀ v ∈ used, v ∉ Λ → v ∉ _root_.B.Term.vars A → v ∉ Γ') ∧
                                                  ∃ Δ',
                                                              ...
                                                              ⟦t'.abstract Δ' Δ'_covers⟧ˢ = some denT' ∧
                                                                ⟨T, ⟨α, hT⟩⟩ ≘ᶻ denT' ∧
                                                                  ∀ (Δ_alt : _root_.B.RenamingContext.Context)
                                                                    ...
                                                                    _root_.B.RenWF E.context Δ_alt →
                ...
                ∀ {T : ZFSet.{u}} {hT : T ∈ ⟦α⟧ᶻ},
                  ⟦B.abstract «Δ» Δ_fv⟧ᴮ = some ⟨T, ⟨α, hT⟩⟩ →
                    (∀ v ∈ B.vars, v ∈ used) →
                      ...
                      ((_root_.B.bv B).Nodup) →
                        _root_.B.RenamingContext.RespectsTypeContextOnFV (_root_.B.RenamingContext.toSMT «Δ») Λ B →
                        (∀ v ∈ _root_.B.fv B, v ∈ Λ) →
                        _root_.B.RenWF E.context «Δ» →
                        ∀ {n : ℕ},
                          ⦃fun x =>
                                      ...
                                      Λ ⊆ Γ' ∧
                                        AList.keys Γ' ⊆ E'.usedVars ∧
                                          CoversUsedVars E'.usedVars B ∧
                                            σ = α.toSMTType ∧
                                              Γ' ⊢ˢ t' : σ ∧
                                                (∀ v ∈ used, v ∉ Λ → v ∉ _root_.B.Term.vars B → v ∉ Γ') ∧
                                                  ∃ Δ',
                                                    ∃ (Δ'_covers : RenamingContext.CoversFV Δ' t'),
                                                      RenamingContext.Extends Δ' Δ₀ ∧
                                                        RenamingContext.ExtendsOnSourceFV Δ' «Δ» B ∧
  ...
  (Λ_inv : ∀ v ∈ (A ∩ᴮ B).vars, v ∈ Λ → v ∈ E.context)
  (bv_nodup : (_root_.B.bv (A ∩ᴮ B)).Nodup)
  (respects : _root_.B.RenamingContext.RespectsTypeContextOnFV (_root_.B.RenamingContext.toSMT «Δ») Λ ((A ∩ᴮ B)))
  (fv_in_Λ : ∀ v ∈ _root_.B.fv ((A ∩ᴮ B)), v ∈ Λ)
  (wf : _root_.B.RenWF E.context «Δ» := by assumption)
  {n : ℕ} :
  ⦃fun x =>
    match x with
              ...
              Λ ⊆ Γ' ∧
                AList.keys Γ' ⊆ E'.usedVars ∧
                  CoversUsedVars E'.usedVars (A ∩ᴮ B) ∧
                    σ = α.toSMTType ∧
                      Γ' ⊢ˢ t' : σ ∧
                        (∀ v ∈ used, v ∉ Λ → v ∉ _root_.B.Term.vars (A ∩ᴮ B) → v ∉ Γ') ∧
                          ∃ Δ',
                            ∃ (Δ'_covers : RenamingContext.CoversFV Δ' t'),
                              RenamingContext.Extends Δ' Δ₀ ∧
  ...
  obtain ⟨α, rfl, typ_A, typ_B⟩ := Typing.interE typ_t
  clear typ_t
  ...
  -- `B.denote` of `∩ᴮ'` forces both operand tags to `.set _` (`| failure`
  -- otherwise) and returns `.set α`; matching the known result type pins the
  -- operand types — no `of_abstract` needed.
  cases α' <;> first | rw [Option.bind_eq_some_iff] at eq | exact absurd eq (by simp)
  rename_i αi
  obtain ⟨⟨B', β', hB'⟩, den_B, eq⟩ := eq
  cases β' <;> [exact absurd eq (by simp); exact absurd eq (by simp); skip; exact absurd eq (by simp)]
  rename_i βi
  dsimp only at eq
  split at eq
  on_goal 2 => exact absurd eq (by simp)
  rename_i hαβ
  ...
  have hA_bv_nodup : (_root_.B.bv A).Nodup := by
    have := bv_nodup; simp only [_root_.B.bv, List.nodup_append] at this; exact this.1
  have hB_bv_nodup : (_root_.B.bv B).Nodup := by
    have := bv_nodup; simp only [_root_.B.bv, List.nodup_append] at this; exact this.2.1
  have hAB_bv_disj : ∀ a ∈ _root_.B.bv A, ∀ b ∈ _root_.B.bv B, a ≠ b := by
  ...
  mspec (Std.Do.Triple.and _ (Std.Do.Triple.and _
    (A_ih (E := E) (Λ := St₀.types) (α := α.set) typ_A («Δ» := «Δ») Δ_fv_A
      ...
      (fun v hv => vars_used v (by
        simp only [_root_.B.Term.vars, List.mem_union_iff, _root_.B.fv, _root_.B.bv,
          List.mem_append] at hv ⊢; rcases hv with h | h <;> [left; right] <;> exact .inl h))
      (fun v hv => Λ_inv v (by
        simp only [_root_.B.Term.vars, List.mem_union_iff, _root_.B.fv, _root_.B.bv,
          List.mem_append] at hv ⊢; rcases hv with h | h <;> [left; right] <;> exact .inl h))
      hA_bv_nodup
      ...
      wf
      (n := St₀.env.freshvarsc))
    (encodeTerm_bv_used E (t := A) (used := St₀.env.usedVars)
      (n := St₀.env.freshvarsc) (decl := St₀.env.declarations)))
    (encodeTerm_bv_notMem_used E (t := A) (used := St₀.env.usedVars)
      (n := St₀.env.freshvarsc) (decl := St₀.env.declarations)))
  clear A_ih
  ...
  next out =>
    ...
    obtain ⟨⟨⟨St₀_used_sub_St₁, St₀_sub_St₁, St₁_sub, A_cov_St₁, rfl, typ_Aenc, A_preserves,
      ...
      ⟨Aenc', _, hAenc'⟩, den_Aenc, ⟨rfl, retract_Aenc'_eq_A'⟩, A_ih_total⟩,
      bv_Aenc_used, _⟩,
      bv_Aenc_notMem_used, _⟩ := pre

    have Δ'_ext_B : RenamingContext.ExtendsOnSourceFV Δ' «Δ» B :=
      RenamingContext.extendsOnSourceFV_of_extends Δ'_extends_Δ₀ Δ₀_ext_B
    mspec (Std.Do.Triple.and _ (Std.Do.Triple.and _
      (B_ih (E := E) (Λ := St₁.types) (α := α.set) typ_B («Δ» := «Δ») Δ_fv_B
        (Δ₀ := Δ') Δ'_ext_B (used := St₁.env.usedVars) Δ'_none_out (T := B') (hT := hB')
        den_B
        (fun v hv => St₀_used_sub_St₁ (vars_used v (by
          simp only [_root_.B.Term.vars, List.mem_union_iff, _root_.B.fv, _root_.B.bv,
            List.mem_append] at hv ⊢; rcases hv with h | h <;> [left; right] <;> exact .inr h)))
        (fun v hv hΛ => by
          ...
          · have hv_vars_A : v ∈ _root_.B.Term.vars A := by
              by_contra h_neg
              exact absurd hΛ (A_preserves v (vars_used v hv_mem) hv_St h_neg)
            rcases _root_.B.Term.mem_vars_iff.mp hv_vars_A with h | h
            · exact _root_.B.Typing.typed_by_fv typ_A h
            · rcases _root_.B.Term.mem_vars_iff.mp hv with hy_fv | hy_bv
              · exact absurd (_root_.B.Typing.typed_by_fv typ_B hy_fv)
                  (_root_.B.Typing.bv_notMem_context typ_A v h)
              · exact absurd rfl (hAB_bv_disj v h v hy_bv))
        hB_bv_nodup
        (respects.transport_fv (fun v hv => by rw [B.fv, List.mem_append]; exact Or.inr hv) St₀_sub_St₁ (fun v hv => fv_in_Λ v (by rw [B.fv, List.mem_append]; exact Or.inr hv)))
        (fun v hv => AList.mem_of_subset St₀_sub_St₁ (fv_in_Λ v (by rw [B.fv, List.mem_append]; exact Or.inr hv)))
        wf
        (n := St₁.env.freshvarsc))
      (encodeTerm_bv_used E (t := B) (used := St₁.env.usedVars)
        (n := St₁.env.freshvarsc) (decl := St₁.env.declarations)))
      (encodeTerm_bv_notMem_used E (t := B) (used := St₁.env.usedVars)
        (n := St₁.env.freshvarsc) (decl := St₁.env.declarations)))
    clear B_ih
    ...
    next out =>
      ...
      obtain ⟨⟨⟨St₁_used_sub_St₂, St₁_sub_St₂, St₂_sub, B_cov_St₂, rfl, typ_Benc, B_preserves,
        ...
        ⟨Benc', _, hBenc'⟩, den_Benc, ⟨rfl, retract_Benc'_eq_B'⟩, B_ih_total⟩,
        bv_Benc_used, _⟩,
        bv_Benc_notMem_used, _⟩ := pre
      dsimp at den_Benc

      ---
      -- Apply castInter directly (inline the direct α = β path)
      ...
      -- Inline castInter for the direct α = β path
      have hcastInter' :
          castInter (Aenc, .fun γ .bool) (Benc, .fun γ .bool) = do
            let z ← freshVar γ "inter!"
            SMT.eraseFromContext z
            return (.lambda [z] [γ] (.and (.app Aenc (.var z)) (.app Benc (.var z))), .fun γ .bool) := by
        unfold castInter; simp
      rw [show castInter (Aenc, α.set.toSMTType) (Benc, α.set.toSMTType) =
              castInter (Aenc, .fun γ .bool) (Benc, .fun γ .bool) from rfl,
          hcastInter']
      mspec freshVar_spec
      next z =>
        mrename_i ci_pre
        mintro ∀St_z
        mpure ci_pre
        obtain ⟨St_z_types_eq, z_fresh, St_z_fvc, St_z_used_eq, z_not_used⟩ := ci_pre

        have z_not_fv_Aenc : z ∉ SMT.fv Aenc :=
          funNotMemFvOfNotMemContext typ_Aenc (fun h => z_fresh (AList.mem_of_subset St₁_sub_St₂ h))
        have z_not_fv_Benc : z ∉ SMT.fv Benc :=
          funNotMemFvOfNotMemContext typ_Benc z_fresh

        -- Δ'' covers fv(lambda [z] [γ] (and (app Aenc (var z)) (app Benc (var z))))
        have Δ''_cov_lambda : SMT.RenamingContext.CoversFV Δ''
            (.lambda [z] [γ] (.and (.app Aenc (.var z)) (.app Benc (.var z)))) := by
          intro v hv
          simp only [SMT.fv, List.removeAll, List.mem_filter, List.mem_append,
            List.mem_cons, List.not_mem_nil, or_false] at hv
          obtain ⟨hv_body, hv_ne_z⟩ := hv
        ...
        -- Erase the fresh lambda binder `z` from the returned context
        -- (the encoder now `erase`s `z` after re-binding it as the lambda binder).
        mspec SMT.eraseFromContext_spec
        mrename_i preE
        mintro ∀St_e
        mpure preE
        obtain ⟨St_e_types_eq, St_e_fvc, St_e_used_eq⟩ := preE
        -- The final context is `St₂.types.erase z` since
        -- `St_z.types = insert z γ St₂.types` and `z ∉ St₂.types` (`z_fresh`).
        have St_e_eq_St₂ : St_e.types = St₂.types := by
          rw [St_e_types_eq, St_z_types_eq, encodeTerm_state.erase_insert_self z_fresh]
        ...
        -- (1) used ⊆ E'.usedVars
        · intro v hv
          rw [St_e_used_eq, St_z_used_eq]
          exact List.mem_cons_of_mem z (St₁_used_sub_St₂ (St₀_used_sub_St₁ hv))
        -- (2) Λ ⊆ Γ'
        · rw [St_e_eq_St₂]
          exact fun v hv => St₁_sub_St₂ (St₀_sub_St₁ hv)
        -- (3) keys Γ' ⊆ E'.usedVars
        · intro v hv
          rw [St_e_eq_St₂] at hv
          rw [St_e_used_eq, St_z_used_eq]
        ...
        · intro v hv
          ...
          · rw [St_e_used_eq, St_z_used_eq]; exact List.mem_cons_of_mem z (St₁_used_sub_St₂ (A_cov_St₁ v hv))
          · rw [St_e_used_eq, St_z_used_eq]; exact List.mem_cons_of_mem z (B_cov_St₂ v hv)
        -- (5) τ = α.set.toSMTType
        · rfl
        -- (6) typing
        · rw [St_e_eq_St₂]
          -- `bv Aenc` avoids `St₂.types`: each `v ∈ bv Aenc` is used at stage `St₁`
          -- (`bv_Aenc_used`), is not in its own typing context `St₁.types`
          -- (`bv_notMem_context typ_Aenc`), and is not a variable of `B` (its fresh
          -- name predates `St₀.usedVars`, so `B_preserves` keeps it out of `St₂`).
          have bvAenc_notMem_St₂ : ∀ v ∈ SMT.bv Aenc, v ∉ St₂.types := fun v hv =>
            B_preserves v (bv_Aenc_used v hv)
              (SMT.Typing.bv_notMem_context typ_Aenc v hv)
              (by
                rw [_root_.B.Term.notMem_vars_iff]
                refine ⟨?_, ?_⟩
                · intro hfB
                  exact SMT.Typing.bv_notMem_context typ_Aenc v hv
                    (AList.mem_of_subset St₀_sub_St₁
                      (fv_in_Λ v (by rw [B.fv, List.mem_append]; exact Or.inr hfB)))
                · intro hbB
                  refine bv_Aenc_notMem_used v hv ?_
                  exact vars_used v (by
                    simp only [_root_.B.Term.vars, List.mem_union_iff, _root_.B.fv, _root_.B.bv,
                      List.mem_append]
                    right; right; exact hbB))
          -- `z ∉ bv Aenc` / `z ∉ bv Benc`: `z` is freshly created *after* both
          -- encodings (`z_not_used : z ∉ St₂.usedVars`), and the bound variables of
          -- `Aenc`/`Benc` were registered during those encodings
          -- (`bv Aenc ⊆ St₁.usedVars`, `bv Benc ⊆ St₂.usedVars`).
          have z_not_bv_Aenc : z ∉ SMT.bv Aenc := fun hbv =>
            z_not_used (St₁_used_sub_St₂ (bv_Aenc_used z hbv))
          have z_not_bv_Benc : z ∉ SMT.bv Benc := fun hbv =>
            z_not_used (bv_Benc_used z hbv)
          -- `Aenc` is well-typed in the (larger) `St₂.types`.
          have typ_Aenc_St₂ : St₂.types ⊢ˢ Aenc : α.set.toSMTType :=
            Typing.weakening St₁_sub_St₂ typ_Aenc bvAenc_notMem_St₂
          refine Typing.lambda St₂.types [z] [γ] _ .bool ?_ ?_ ?_ ?_ ?_
          · intro v hv; rw [List.mem_singleton] at hv; subst hv; exact z_fresh
          · intro v hv; rw [List.mem_singleton] at hv; subst hv
            simp only [SMT.bv, List.append_nil, List.mem_append]
          ...
          · have hupdate : TypeContext.update St₂.types [z] [γ] rfl = St₂.types.insert z γ := by
            ...
            · apply SMT.Typing.app
              · exact Typing.weakening (TypeContext.entries_subset_insert_of_notMem (τ := γ) z_fresh)
                  typ_Aenc_St₂ (SMT.Typing.bv_notMem_insert_of_fresh typ_Aenc_St₂ z_not_bv_Aenc)
              · exact SMT.Typing.var _ z γ (AList.lookup_insert St₂.types)
            · apply SMT.Typing.app
              · exact Typing.weakening (TypeContext.entries_subset_insert_of_notMem (τ := γ) z_fresh)
                  typ_Benc (SMT.Typing.bv_notMem_insert_of_fresh typ_Benc z_not_bv_Benc)
              · exact SMT.Typing.var _ z γ (AList.lookup_insert St₂.types)
        -- (7) preserves_types
        · intro v hv h1 h2
          obtain ⟨h2_A, h2_B⟩ := _root_.B.Term.notMem_vars_inter.mp h2
          rw [St_e_eq_St₂]
        ...
        · -- Use Δ'' as the renaming context; apply castInter_denotation_direct
          ...
          have Δ''_none_out_z : ∀ v ∉ St_e.env.usedVars, Δ'' v = none := by
            intro v hv; apply Δ''_none_out
            intro hmem; exact hv (St_e_used_eq ▸ St_z_used_eq ▸ List.mem_cons_of_mem z hmem)
          ...
          · refine ⟨den_lambda, hden_lambda, ?_, ?_⟩
            ...
            · -- ih_total: alt universality
              ...
              -- returns `.set α`; matching the known result type pins them.
              cases α_alt' <;> first | rw [Option.bind_eq_some_iff] at eq_alt | exact absurd eq_alt (by simp)
              rename_i αi
              obtain ⟨⟨B_alt, β_alt', hB_alt⟩, den_B_alt, eq_alt⟩ := eq_alt
              cases β_alt' <;> [exact absurd eq_alt (by simp); exact absurd eq_alt (by simp); skip; exact absurd eq_alt (by simp)]
              rename_i βi
              dsimp only at eq_alt
              split at eq_alt
              on_goal 2 => exact absurd eq_alt (by simp)
              rename_i hαβ
              rw [Option.some_inj] at eq_alt
              injection eq_alt with T_eq hτeq
              subst T_eq
              simp only [heq_eq_eq, PSigma.mk.injEq, BType.set.injEq] at hτeq
              obtain ⟨hτeq, _⟩ := hτeq
              subst αi
              ...
              have Δ₀_alt_A_wt : ∀ v (d : SMT.Dom), Δ₀_alt_A v = some d → ∀ τ, St₁.types.lookup v = some τ → d.snd.fst = τ := by
                intro v d hv τ hτ; simp only [Δ₀_alt_A_def] at hv; split_ifs at hv with h; exact Δ₀_alt_wt v d hv τ (by rw [St_e_eq_St₂]; exact AList.mem_lookup_iff.mpr (St₁_sub_St₂ (AList.mem_lookup_iff.mp hτ)))
              -- Call A_ih_total
              obtain ⟨Δ'_alt_A, hcov_alt_A, denT_A_alt, hext_alt_A,
                ...
                A_ih_total Δ_alt
                  ...
                  Δ₀_alt_A Δ₀_alt_A_ext wf_alt Δ₀_alt_A_none Δ₀_alt_A_wt A_alt hA_alt den_A_alt
              ...
              obtain ⟨Δ'_alt_B, hcov_alt_B, denT_B_alt, hext_alt_B,
                ...
                B_ih_total Δ_alt
                  ...
                  Δ₀_alt_B Δ₀_alt_B_ext wf_alt Δ₀_alt_B_none Δ₀_alt_B_wt B_alt hB_alt den_B_alt
              ...
              · intro v hv
                ...
                | some d =>
                  rw [St_e_eq_St₂]
                  exact fv_sub_typings (_root_.B.Typing.inter typ_A typ_B)
