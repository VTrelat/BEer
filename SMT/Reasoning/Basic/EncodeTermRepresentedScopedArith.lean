...
/-! # Generated-helper contracts for integer arithmetic -/

namespace EncodeTermRepresentedArith.CheckedOp

@[simp] theorem fv_smtTerm (op : CheckedOp) (x y : SMT.Term) :
    SMT.fv (op.smtTerm x y) = SMT.fv x ++ SMT.fv y := by
  cases op <;> simp [smtTerm, SMT.fv]

@[simp] theorem bv_smtTerm (op : CheckedOp) (x y : SMT.Term) :
    SMT.bv (op.smtTerm x y) = SMT.bv x ++ SMT.bv y := by
  cases op <;> simp [smtTerm, SMT.bv]
...
theorem smt_denote_inv.{u} (op : CheckedOp)
    ...
    ∃ X, ∃ hX : X ∈ ⟦SMTType.int⟧ᶻ,
      ...
      ∃ Y, ∃ hY : Y ∈ ⟦SMTType.int⟧ᶻ,
        ...
        d = ⟨op.eval X Y, SMTType.int, op.eval_mem hX hY⟩ := by
  cases op
  all_goals
...
end EncodeTermRepresentedArith.CheckedOp

set_option maxHeartbeats 4000000 in
theorem encodeTerm_rep_scoped.checked_int_case_from.{u}
    ...
    (typ_t : E.context ⊢ᴮ op.term x y : BType.int)
    ...
    ⦃⇓? (⟨t', σ⟩ : SMT.Term × SMTType) ⟨E', Γ'⟩ =>
      ⌜EncodeTermRepScopedPostFrom.{u} (op.term x y) E BType.int
  ...
  · intro Δ_alt Δ_fv_alt Δ₀_alt related_alt wf_alt
    ...
    have hcov_op_alt : RenamingContext.CoversFV Δy_alt
      ...
      intro v hv
      rw [EncodeTermRepresentedArith.CheckedOp.fv_smtTerm,
  ...
  · intro Γ_sup Γ_sub Δ_alt Δ_fv_alt Θ related_alt wf_alt
    ...
    have X_rel_target := x_guard Γ_sup x_scope Δ_alt
      ...
      hcov_x_target ⟨Xenc_alt, SMTType.int, hXenc_alt⟩
      hden_x_target rfl
    have Y_rel_target := y_guard Γ_sup y_scope Δ_alt
      ...
      hcov_y_target ⟨Yenc_alt, SMTType.int, hYenc_alt⟩
      hden_y_target rfl
    subst denOut
    refine ⟨⟨?_, trivial⟩, .int⟩
  ...
  · constructor
    · intro Γ_sup Γ_sub result_bv_fresh
      ...
      have x_bv_fresh : ∀ v ∈ SMT.bv x_enc, v ∉ Γ_sup := by
        ...
        rw [EncodeTermRepresentedArith.CheckedOp.bv_smtTerm,
          List.mem_append]
        exact Or.inl hv
